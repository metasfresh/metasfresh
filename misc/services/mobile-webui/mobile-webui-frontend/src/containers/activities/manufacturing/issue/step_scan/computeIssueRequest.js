 ...
 *
 * Pure on purpose: the weight predicate below is easier to get wrong than to read, so it is
 * unit-tested instead of being buried in the component's `onResult`.
 *
...
export const computeIssueRequest = ({ qty = 0, qtyRejected = 0, reason = null, resolvedBarcodeData }) => {
  ...
  // qtyRejected / qtyRejectedReasonCode travel unconditionally: the reason list is offered on every
  // step (computeStepScanPropsFromActivity.js), so whatever the operator declared must reach the
  // backend, including on a step that takes only part of the HU.
  //
  // The WEIGHT field answers a different question: "did the operator physically weigh the whole
  // container?" That is a fact about what was actually measured, so it is decided by the TYPED qty.
  // Entering less than the HU's capacity is a short entry, not a weighing of the whole container:
  // sending it as the gross weight makes the backend re-weigh the HU down to that qty BEFORE the
  // issue, the issue then consumes everything, and the leftover write-off that the rejected-qty
  // reason asked for never happens.
