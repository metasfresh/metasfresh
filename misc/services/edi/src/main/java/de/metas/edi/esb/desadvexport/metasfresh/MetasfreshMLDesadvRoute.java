...
package de.metas.edi.esb.desadvexport.metasfresh;
...
public class MetasfreshMLDesadvRoute extends AbstractEDIRoute
{
	public static final String ROUTE_ID = "MF-Desadv-To-STEPCOM-XML-Desadv";

	public static final String EP_EDI_METASFRESH_XML_DESADV_CONSUMER = "direct:edi.desadv.metasfresh-xml.consumer";
	...
	public void configureEDIRoute(@NonNull final DataFormat jaxb, @NonNull final DecimalFormat decimalFormat)
	{
		...
		final String ownerId = Util.resolveProperty(getContext(), MetasfreshMLDesadvRoute.EDI_XML_OWNER_ID);
		final String supplierGln = Util.resolveProperty(getContext(), MetasfreshMLDesadvRoute.EDI_XML_SUPPLIER_GLN);

		final String defaultEDIMessageDatePattern = Util.resolveProperty(getContext(), MetasfreshMLDesadvRoute.EDI_ORDER_EDIMessageDatePattern);
		...
		{
			endPointURIs = new String[] { MetasfreshMLDesadvRoute.OUTPUT_DESADV_LOCAL };
		}
		else
		{
			endPointURIs = new String[] { MetasfreshMLDesadvRoute.OUTPUT_DESADV_LOCAL, remoteEndpoint };
		}

		from(MetasfreshMLDesadvRoute.EP_EDI_METASFRESH_XML_DESADV_CONSUMER)
				.routeId(ROUTE_ID)

				.log(LoggingLevel.INFO, "Setting defaults as exchange properties...")
				.setProperty(MetasfreshMLDesadvRoute.EDI_XML_OWNER_ID).constant(ownerId)
				.setProperty(MetasfreshMLDesadvRoute.EDI_XML_SUPPLIER_GLN).constant(supplierGln)
				.setProperty(MetasfreshMLDesadvRoute.EDI_ORDER_EDIMessageDatePattern).constant(defaultEDIMessageDatePattern)
				...
				.log(LoggingLevel.INFO, "Sending metasfresh-XML to the endpoint(s):\r\n" + body())
				.multicast()
				.stopOnException()
				.to(endPointURIs)
				.end()
				...
				.process(new EDIXmlSuccessFeedbackProcessor<>(EDIDesadvFeedbackType.class, MetasfreshMLDesadvRoute.EDIDesadvFeedback_QNAME, MetasfreshMLDesadvRoute.METHOD_setEDIDesadvID))
