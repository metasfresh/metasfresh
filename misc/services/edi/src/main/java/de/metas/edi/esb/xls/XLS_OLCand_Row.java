 ...
 * @task 08839
 */
public class XLS_OLCand_Row
{
	public static final XLS_OLCand_Row_Builder builder()
	{
		return new XLS_OLCand_Row_Builder();
	}

	public static final Map<Integer, XLS_OLCand_Row> indexByLineNo(final Collection<XLS_OLCand_Row> rows)
	{
		final Map<Integer, XLS_OLCand_Row> lineNo2row = new HashMap<Integer, XLS_OLCand_Row>(rows.size());
		for (XLS_OLCand_Row row : rows)
		{
	...
	public static final List<XLS_OLCand_Row> ofMaps(final List<Map<String, Object>> mapsList)
	{
		final List<XLS_OLCand_Row> rows = new ArrayList<XLS_OLCand_Row>(mapsList.size());
		for (final Map<String, Object> rowData : mapsList)
		{
			final XLS_OLCand_Row row = ofMap(rowData);
			rows.add(row);
		}
