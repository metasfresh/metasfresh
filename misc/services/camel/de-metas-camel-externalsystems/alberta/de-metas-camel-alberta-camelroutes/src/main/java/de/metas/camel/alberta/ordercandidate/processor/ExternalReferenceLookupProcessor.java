...
public class ExternalReferenceLookupProcessor implements Processor
{
	...
	public void process(final Exchange exchange)
	{
		...
		final DoctorApi doctorApi = ProcessorHelper.getPropertyOrThrowError(exchange, ROUTE_PROPERTY_DOCTOR_API, DoctorApi.class);
		final PharmacyApi pharmacyApi = ProcessorHelper.getPropertyOrThrowError(exchange, ROUTE_PROPERTY_ALBERTA_PHARMACY_API, PharmacyApi.class);
			...
			@NonNull final DoctorApi doctorApi)
	{
		...
		exchange.setProperty(ROUTE_PROPERTY_EXTERNAL_BP_IDENTIFIER_TO_API, externalBPId2Api);
