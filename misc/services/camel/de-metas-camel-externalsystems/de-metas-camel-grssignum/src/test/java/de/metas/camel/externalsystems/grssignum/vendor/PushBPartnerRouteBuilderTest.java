...
package de.metas.camel.externalsystems.grssignum.vendor;
...
import static de.metas.camel.externalsystems.grssignum.vendor.PushBPartnerRouteBuilder.PUSH_BPARTNERS_PROCESSOR_ID;
...
public class PushBPartnerRouteBuilderTest extends CamelTestSupport
{
	...
	void pushBPartner() throws Exception
	{
		...
		//when
		template.sendBody("direct:" + PUSH_BPARTNERS_ROUTE_ID, requestBodyAsString);
	...
	{
		AdviceWith.adviceWith(context, PUSH_BPARTNERS_ROUTE_ID,
							  advice -> {
								  advice.weaveById(PUSH_BPARTNERS_PROCESSOR_ID)
										  .after()
