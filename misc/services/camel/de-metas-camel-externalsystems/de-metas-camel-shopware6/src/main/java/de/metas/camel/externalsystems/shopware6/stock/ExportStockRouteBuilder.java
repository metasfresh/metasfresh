...
public class ExportStockRouteBuilder extends RouteBuilder
{
	...
	private void exportStockToShopware(@NonNull final Exchange exchange)
	{
		...
		shopwareClient.exportStock(jsonStock, exportStockRouteContext.getJsonAvailableAvailableForSales().getProductIdentifier().getExternalReference());
	}
	...
	private void createJsonStock(@NonNull final Exchange exchange)
	{
		...
		final BigDecimal stockBD = exportStockRouteContext.getJsonAvailableAvailableForSales()
				.getStock()
	...
	private void buildAndAttachRouteContext(@NonNull final Exchange exchange)
	{
		...
		final JsonAvailableAvailableForSales jsonAvailableAvailableForSales = getJsonAvailableStock(request);
	...
	@NonNull
	private JsonAvailableAvailableForSales getJsonAvailableStock(@NonNull final JsonExternalSystemRequest request)
	{
		try
		{
			return objectMapper.readValue(request.getParameters().get(ExternalSystemConstants.PARAM_JSON_AVAILABLE_FOR_SALES), JsonAvailableAvailableForSales.class);
		}
