...
winpty docker exec -it ${BRANCH_NAME}_db  psql -U postgres -c "alter database metasfresh rename to ${BRANCH_NAME}_infrastructure;"
...
echo ""
echo "winpty docker exec -it ${BRANCH_NAME}_db  psql -U postgres -c \"UPDATE pg_database SET datistemplate='false' WHERE datname='${BRANCH_NAME}_infrastructure';\""
echo "winpty docker exec -it ${BRANCH_NAME}_db  psql -U postgres -c \"drop database if exists ${BRANCH_NAME}_infrastructure;\""
echo ""
