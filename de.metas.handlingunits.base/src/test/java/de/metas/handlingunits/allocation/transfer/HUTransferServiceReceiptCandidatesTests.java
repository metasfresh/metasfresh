...
public class HUTransferServiceReceiptCandidatesTests
{
	/**
	...
	public void testMultipleActionsIssue1177()
	{
		...
		final List<I_M_HU> secondLUs = HUTransferService.get(data.helper.getHUContext())
		...
		final List<I_M_HU> singleNewTUs = HUTransferService.get(data.helper.getHUContext())
		...
		HUTransferService.get(data.helper.getHUContext()).cuToExistingTU(newCU, BigDecimal.ONE, singleNewTU);
		// ..but all in all the qtys are unchanged
		verifyQuantities(new BigDecimal("40"), new BigDecimal("9"), firstLU, aggregateTU, newCU, secondLU, singleNewTU);
	}
