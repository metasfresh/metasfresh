...
CREATE OR REPLACE VIEW "de.metas.monitoring".async_workpackage_performance_v AS 
 SELECT pp.classname,
   ...
   FROM c_queue_workpackage wp
     JOIN c_queue_block wb ON wb.c_queue_block_id = wp.c_queue_block_id
     JOIN c_queue_packageprocessor pp ON pp.c_queue_packageprocessor_id = wb.c_queue_packageprocessor_id
