...
public class ForecastLineEventListener implements MaterialEventListener
{
	private static final transient Logger logger = LogManager.getLogger(ForecastLineEventListener.class);

	private final CommonDemandHandler commonDemandHandler;

	public ForecastLineEventListener(@NonNull final CommonDemandHandler commonDemandHandler)
	{
	...
	{
		if (!(event instanceof ForecastLineMaterialEvent))
		{
			return;
		}
		logger.info("Received event {}", event);

		final ForecastLineMaterialEvent materialDemandEvent = (ForecastLineMaterialEvent)event;
