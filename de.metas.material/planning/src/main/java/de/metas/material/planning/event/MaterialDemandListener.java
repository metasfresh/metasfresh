...
public class MaterialDemandListener implements MaterialEventListener
{
	...
	private IMRPNotesCollector mkMRPNotesCollector()
	{
		...
		{
			@Override
			public IMRPNoteBuilder newMRPNoteBuilder(IMaterialPlanningContext mrpContext, String mrpErrorCode)
			{
				return new SimpleMRPNoteBuilder(this, mrps -> Collections.emptySet());
			}

			@Override
			public void collectNote(IMRPNoteBuilder noteBuilder)
			{
				noteBuilder.collect();
			}
		};
