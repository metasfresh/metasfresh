...
CREATE OR REPLACE FUNCTION de_metas_endcustomer_fresh_reports.average_product_prices(IN p_datefrom date,
                                                                                     IN p_dateto date,
                                    ...
                                    COALESCE(
                                            (SELECT avg(
                                                                (CASE
                                                                    ...
                                                                    -- way for the user to know why this price was negated
                                                                    -- as a column with "reason Credit Memo" can't be added easily
                                                                    -- due to the the multiple invoices case for one invoice candidate
                                                                     WHEN dt.docbasetype IN ('APC', 'ARC')
                                                                         THEN -1
                                                                         ELSE 1
                                                                 END)
                                                                * il.PriceActual
...
FROM C_Invoice_Candidate ic
         ...
         LEFT OUTER JOIN C_OrderLine ol
                         ON ic.Record_ID = ol.C_OrderLine_ID
                             AND ic.AD_Table_ID = get_Table_ID('C_OrderLine')
         ...
         INNER JOIN c_bpartner ioBP ON io.c_bpartner_id = ioBP.c_bpartner_id
         INNER JOIN c_bpartner_location ioLOC ON io.c_bpartner_location_id = ioLOC.c_bpartner_location_id
         LEFT OUTER JOIN PP_Order pp
                         ON ic.Record_ID = pp.PP_Order_ID
                             AND ic.AD_Table_ID = get_Table_ID('PP_Order')

         LEFT OUTER JOIN C_Order o ON ol.C_Order_ID = o.C_Order_ID

         LEFT OUTER JOIN C_UOM_Conversion uconv
                         ON
                                 uconv.C_UOM_ID = iol.C_UOM_ID
                                 AND uconv.C_UOM_To_ID = price_uom.C_UOM_ID
                                 AND p.M_Product_ID = uconv.M_Product_ID


WHERE ic.isSOTrx = p_issotrx
...
-- Example:
-- select * from de_metas_endcustomer_fresh_reports.average_product_prices ('2019-12-30'::date, '2019-12-31'::date, 'N' )
