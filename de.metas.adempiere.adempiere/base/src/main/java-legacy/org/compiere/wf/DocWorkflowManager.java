 ...
 */
public final class DocWorkflowManager implements DocWorkflowMgr
{
	...
	private static final transient DocWorkflowManager instance = new DocWorkflowManager();
	private static final Logger logger = LogManager.getLogger(DocWorkflowManager.class);
	...
	public String toString()
	{
		return MoreObjects.toStringHelper(this)
				.add("called", m_noCalled)
				.add("started", m_noStarted)
				.toString();
	}
	...
	public boolean process(final PO document)
	{
		...
		MWorkflow[] wfs = MWorkflow.getDocValue(document.getCtx(),
				document.getAD_Client_ID(), AD_Table_ID, document.get_TrxName() // Bug 1568766 Trx should be kept all along the road
		);
		...
		for (final MWorkflow wf : wfs)
		{
			...
			{
				logger.error("Workflow has no Logic - {}", wf);
				continue;
			}
			...
			{
				logger.debug("SQL Logic evaluated to false ({})", logic);
				continue;
			}
			if (!sql && !Evaluator.evaluateLogic(document, logic))
			{
				logger.debug("Logic evaluated to false ({})", logic);
				continue;
			}
			...
			if (wf.start(pi) != null)
			{
				logger.info("Workflow {} started for {}", wf, document);
				m_noStarted++;
				started = true;
			}
		}
	...
	private boolean testStart(MWorkflow wf, PO document)
	{
		...
		{
			logger.error("Tables with more then one key column not supported - "
