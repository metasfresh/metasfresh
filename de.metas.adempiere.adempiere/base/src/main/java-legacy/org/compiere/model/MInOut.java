...
import static org.adempiere.util.CustomColNames.M_InOut_BPARTNERADDRESS;
...
import static org.adempiere.util.CustomColNames.M_InOut_ISUSE_BPARTNER_ADDRESS;
...
public class MInOut extends X_M_InOut implements DocAction
{
			...
			Timestamp minGuaranteeDate, boolean complete, String trxName)
	{
		...
		for (int i = 0; i < oLines.length; i++)
		{
			...
			if (product != null && product.get_ID() != 0 && Services.get(IProductBL.class).isStocked(product))
			{
	...
	private static MInOut copyHeader(MInOut from, Timestamp dateDoc, Timestamp dateAcct, int C_DocType_ID, boolean isSOTrx, boolean counter, String trxName, boolean setOrder)
	{
	...
	private void copyAdditionalCols(final Object order)
	{
		...
		poService.copyValue(order, this, M_InOut_INCOTERM);
		poService.copyValue(order, this, M_InOut_INCOTERMLOCATION);
		poService.copyValue(order, this, M_InOut_DESCRIPTION_BOTTOM);

		poService.setValue(this, M_InOut_BPARTNERADDRESS, poService.getValue(order, de.metas.adempiere.model.I_C_Order.COLUMNNAME_BPartnerAddress));
		poService.setValue(this, M_InOut_ISUSE_BPARTNER_ADDRESS, poService.getValue(order, de.metas.adempiere.model.I_C_Order.COLUMNNAME_IsUseBPartnerAddress));
	}
	...
	public int copyLinesFrom(MInOut otherShipment, boolean counter, boolean setOrder)
	{
		if (isProcessed() || isPosted() || otherShipment == null)
			return 0;
		MInOutLine[] fromLines = otherShipment.getLines();
		int count = 0;
	...
	protected boolean beforeSave(boolean newRecord)
	{
		...
		// metas
		final IPOService poService = Services.get(IPOService.class);
		if ("".equals(poService.getValue(this, M_InOut_INCOTERM)))
		{
			poService.setValue(this, M_InOut_INCOTERMLOCATION, "");
		}
	...
	public String completeIt()
	{
		...
		{
			final MInOutLine sLine = lines[lineIndex];
...
// @formatter:on
	...
	private static void sortByProductAndASI(final MInOutLine[] lines)
	{
		...
		c.addComparator(new Comparator<MInOutLine>()
		{
			...
			{
				return Integer.compare(o1.getM_Product_ID(), o2.getM_Product_ID());
			}
		});
		c.addComparator(new Comparator<MInOutLine>()
		{
			...
			public int compare(MInOutLine o1, MInOutLine o2)
			{
				return Integer.compare(o1.getM_AttributeSetInstance_ID(), o2.getM_AttributeSetInstance_ID());
			}
		});
		Arrays.sort(lines, c);
	}
...
// @formatter:on
	...
	private MInOut createCounterDoc()
	{
		...
		// metas
		final IPOService poService = Services.get(IPOService.class);
		poService.copyValue(this, counter, M_InOut_INCOTERM);
		poService.copyValue(this, counter, M_InOut_INCOTERMLOCATION);
		poService.copyValue(this, counter, M_InOut_DESCRIPTION_BOTTOM);
		// metas end
		...
		// Update copied lines
		MInOutLine[] counterLines = counter.getLines();
	...
	public boolean reverseCorrectIt()
	{
		...
		for (int i = 0; i < rLines.length; i++)
		{
			...
			// De-Activate Asset
			MAsset asset = MAsset.getFromShipment(getCtx(), sLine.getM_InOutLine_ID(), get_TrxName());
			if (asset != null)
			{
	...
	public boolean reActivateIt()
	{
		...
		for (I_M_InOutConfirm confirmation : confirmations)
		{
