...
public class ZoomInfoFactory
{
	...
	public static final class ZoomInfo
	{
		...
		public String toString()
		{
			...
			sb.append(", RecordCount=").append(query == null ? "<no query>" : query.getRecordCount());
			sb.append("]");
			return sb.toString();
		}
	...
	/**
	 * Adds {@link ZoomInfo} instances from {@link MRelationType}s and {@link GenericZoomProvider}.
	 *
	 * First it looks for matching {@link MRelationType} instances and adds their {@link MRelationType#retrieveZoomInfos(PO)} results.
	 * Then it adds the {@link GenericZoomProvider}'s results unless there is already one
	 * from a MRelationType that has an equal {@link ZoomInfo#destinationDisplay} value.
	 *
	 * @param po the record for which we want to retrieve zoom infos
	 * @param AD_Window_ID the windo for which we want to retrieve zoom infos
	 * @return
	 */
	public static List<ZoomInfoFactory.ZoomInfo> retrieveZoomInfos(
			final PO po,
			final int AD_Window_ID)
	{

		logger.info("PO=" + po + " - AD_Window_ID=" + AD_Window_ID);
		...
		for (final ZoomInfo zoomInfo : MRelationType.retrieveZoomInfos(po, AD_Window_ID))
		{
		...
		for (final ZoomInfo zoomInfo : genericZoomProvider.retrieveZoomInfos(po))
		{
			if (alreadySeen.add(zoomInfo.destinationDisplay))
			{
				logger.debug("Adding zoomInfo " + zoomInfo + " from " + GenericZoomProvider.class.getSimpleName());
				result.add(zoomInfo);
			}
			else
			{
