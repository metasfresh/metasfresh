...
public class RequestImportProcess extends AbstractImportProcess<I_I_Request>
{
	...
	{
		return I_I_Request.COLUMNNAME_I_Request_ID;
	}
	...
	protected void updateAndValidateImportRecords()
	{
		...
		// Update R_RequestType_ID by name
		{
			...
			markAsError("Flatrate conditions not found", I_I_Request.COLUMNNAME_R_RequestType_ID + " IS NULL"
					+ "\n AND " + sqlImportWhereClause);
		}
		...
		{
			final String sqlSelectByValue = "select MIN(s." + I_R_Status.COLUMNNAME_R_Status_ID + ")"
					+ " from " + I_R_Status.Table_Name + " r "
					+ " where r." + I_R_Status.COLUMNNAME_Value + "=i." + I_I_Request.COLUMNNAME_Status
		...
		// Flag missing status
		markAsError("Product not found", I_I_Request.COLUMNNAME_R_Status_ID + " IS NULL"
				+ "\n AND " + sqlImportWhereClause);
	}
