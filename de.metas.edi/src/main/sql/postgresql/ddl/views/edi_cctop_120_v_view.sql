...
CREATE OR REPLACE VIEW edi_cctop_120_v AS 
 SELECT i.c_invoice_id AS edi_cctop_120_v_id, i.c_invoice_id, i.c_invoice_id AS edi_cctop_invoic_v_id, 
        CASE
            WHEN tc.tax_count < 2 THEN max(t.rate)
            ELSE NULL::numeric
        END AS singlevat, 
        CASE
            WHEN max(t.rate) = 0::numeric THEN 'Y'::text
            ELSE 'N'::text
        END AS taxfree, c.iso_code, pt.netdays, 
        CASE
            WHEN pt.netdays IS NOT NULL THEN i.dateinvoiced::timestamp with time zone + pt.netdays
            ELSE NULL::date
        END AS netdate, i.ad_client_id, i.ad_org_id, i.created, i.createdby, i.updated, i.updatedby, i.isactive
   ...
   LEFT JOIN ( SELECT sit.c_invoice_id, count(DISTINCT st.rate) AS tax_count
