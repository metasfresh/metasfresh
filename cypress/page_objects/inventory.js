...
class Inventory extends Metasfresh {
  ...
  toMatchSnapshots(newInventoryRecord, labelPrefix) {
    cy.log(`InventoryId=${newInventoryRecord.documentId}`);
    cy.request('GET', `${config.API_URL}/window/${inventory.windowId}/${newInventoryRecord.documentId}`).then(
      response => {
    ...
    cy.request(
      'GET',
      `${config.API_URL}/window/${inventory.windowId}/${newInventoryRecord.documentId}/${inventory.inventoryLineTabId}`
    ).then(response => {
