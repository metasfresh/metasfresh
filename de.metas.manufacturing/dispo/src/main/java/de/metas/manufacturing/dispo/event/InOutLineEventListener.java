...
public class InOutLineEventListener implements IEventListener
{
private static final Logger logger = LogManager.getLogger(ReceiptScheduleEventListener.class);
	...
	{
		final BigDecimal movementQty = event.getProperty("MovementQty");
		logger.info("movementQty={}", movementQty);

		final Date movementDate = event.getProperty("MovementDate");
		logger.info("movementQty={}", movementQty);
		...
		if (timing == ModelValidator.TYPE_AFTER_NEW)
		{
			...
			final Candidate candidate = Candidate.builder()
					...
					.product(product)
					.warehouse(warehouse)
					.quantity(new Quantity(movementQty, product.getC_UOM()))
					.date(movementDate)
					.build();
			candidateChangeHandler.onStockCandidateNew(candidate);
		}
		else if (timing == ModelValidator.TYPE_AFTER_CHANGE)
		{
			...
			candidateChangeHandler.onStockCandidateChange(event.getRecord(), movementQty, movementDate);
		}
		else if (timing == ModelValidator.TYPE_AFTER_DELETE)
		{
			candidateChangeHandler.onStockCandidateDelete(event.getRecord());
		}
	}
}
