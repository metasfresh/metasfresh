 ...
 */
@Service
public class ShipmentScheduleEventListener implements IEventListener
{
	private static final Logger logger = LogManager.getLogger(ShipmentScheduleEventListener.class);
	...
	public void onEvent(final IEventBus eventBus, final Event event)
	{
		...
		final Date preparationDate = event.getProperty("PreparationDate");
		logger.info("preparationDate={}", preparationDate);

		if (qtyOrdered == null && preparationDate == null)
		{
			return; // nothing to be done, this event doesn't interest us
		}
		
		// create or update a demand record
		...
		if (timing == ModelValidator.TYPE_AFTER_NEW)
		{
			...
			final Candidate candidate = Candidate.builder()
					...
					.build();
			candidateChangeHandler.onDemandCandidateNew(candidate);
		}
		else if (timing == ModelValidator.TYPE_AFTER_CHANGE)
		{
			...
			candidateChangeHandler.onDemandCandidateChange(event.getRecord(), qtyOrdered, preparationDate);
		}
		else if (timing == ModelValidator.TYPE_AFTER_DELETE)
		{
			candidateChangeHandler.onDemandCandidateDelete(event.getRecord());
		}
	}
}
