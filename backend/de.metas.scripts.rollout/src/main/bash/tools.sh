...
check_java_version()
{
        ...
        if [[ -f "/usr/bin/java" ]]; then
			...
			local JAVA_VERSION_NEW="$(/usr/bin/java -version 2>&1 | awk -F '"' '/version/ {print $2}' | cut -d "." -f1)"
            if [[ ! ($JAVA_VERSION == "8" || $JAVA_VERSION_NEW == "11") ]]; then
				...
				if [[ $JAVA_VERSION == "8" ]]; then
					...
					trace check_java_version "OK: JAVA_VERSION=${JAVA_VERSION_NEW}"
				fi
            fi
        else
