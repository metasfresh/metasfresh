...
--
-- Part 2: RV_ReceiptDisposition_DeliveryPlanning (AD_Table 542644) is a report VIEW, and the label
-- has to say the same thing its TableName already does. AD_Table has no AD_Element_ID -- like
-- AD_Process, it is self-owned, so its Name/Description are set directly on AD_Table + every
-- AD_Table_Trl row (never inferred from the window's element). Description states what one row IS.
--
...
-- Part 2: AD_Table 542644 (RV_ReceiptDisposition_DeliveryPlanning) -- self-owned Name/Description, TableName untouched.
UPDATE AD_Table SET
