...
INSERT INTO AD_Process (AccessLevel,AD_Client_ID,AD_Org_ID,AD_Process_ID,AllowProcessReRun,Classname,CopyFromProcess,Created,CreatedBy,EntityType,IsActive,IsApplySecuritySettings,IsBetaFunctionality,IsDirectPrint,IsFormatExcelFile,IsNotifyUserAfterExecution,IsOneInstanceOnly,IsReport,IsTranslateExcelHeaders,IsUpdateExportDate,IsUseBPartnerLanguage,JasperReport,LockWaitTimeout,Name,PostgrestResponseFormat,RefreshAllAfterExecution,ShowHelp,SpreadsheetFormat,SQLStatement,TechnicalNote,Type,Updated,UpdatedBy,Value) VALUES ('3',0,0,585207,'Y','de.metas.report.jasper.client.process.JasperReportStarter','N',TO_TIMESTAMP('2023-02-01 18:18:42','YYYY-MM-DD HH24:MI:SS'),100,'D','Y','N','N','Y','Y','N','N','Y','Y','N','Y','@PREFIX@/de/metas/docs/deliveryplanning/deliveryplanning.xls',0,'Delivery Planning Export Excel','json','N','N','xls','
SELECT div.documentno,
       COALESCE(deliveryL.bpartnername, deliveryBP.name) || E''\n'' || deliveryL.address AS ship_to_location,
       p.name                                                                          AS product_name,
       p.value                                                                         AS product_code,
       w.name                                                                          AS warehouse_name,
       fromC.name                                                                      AS from_country,
       TO_CHAR(div.deliverydate, ''YYYY-MM-DD'')                                         AS deliverydate,
       dp.batch,
       dp.releaseno,
       TO_CHAR(dp.plannedloadingdate, ''YYYY-MM-DD'')                                    AS plannedloadingdate,
       TO_CHAR(dp.actualloadingdate, ''YYYY-MM-DD'')                                     AS actualloadingdate,
       dp.plannedloadedquantity,
       dp.actualloadqty,
       TO_CHAR(dp.planneddeliverydate, ''YYYY-MM-DD'')                                   AS planneddeliverydate,
       TO_CHAR(dp.actualdeliverydate, ''YYYY-MM-DD'')                                    AS actualdeliverydate,
       dp.planneddischargequantity,
       dp.actualdischargequantity
FROM m_delivery_planning_delivery_instructions_v div
         INNER JOIN m_delivery_planning dp ON dp.m_delivery_planning_id = div.m_delivery_planning_id
         ...
         LEFT JOIN m_product p ON dp.m_product_id = p.m_product_id
         LEFT JOIN m_warehouse w ON dp.m_warehouse_id = w.m_warehouse_id
         LEFT JOIN c_country fromC ON dp.c_origincountry_id = fromC.c_country_id
;','Still to add a WHERE clause for the current selection','JasperReportsSQL',TO_TIMESTAMP('2023-02-01 18:18:42','YYYY-MM-DD HH24:MI:SS'),100,'Delivery Planning Export (Excel)')
;
--FIXME add proper where clause
