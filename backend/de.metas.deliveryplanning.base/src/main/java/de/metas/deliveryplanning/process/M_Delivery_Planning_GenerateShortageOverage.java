...
public class M_Delivery_Planning_GenerateShortageOverage extends JavaProcess implements IProcessPrecondition, IProcessDefaultParametersProvider
{
	...
	private I_M_Inventory generateInventoryDocument(@NonNull final DocTypeId doctypeId, @NonNull final DeliveryPlanningId deliveryPlanningId)
	{
		...
		final Set<InOutAndLineId> inOutLinesIds = inOutDAO.retrieveLinesByInOutId(InOutId.ofRepoId(inOutRecord.getM_InOut_ID()))
				.stream()
				.map(line -> InOutAndLineId.ofRepoId(line.getM_InOut_ID(), line.getM_InOutLine_ID()))
	...
	private void createInventoryLine(@NonNull final InventoryId inventoryId, @NonNull final InOutLineId inOutLineId)
	{
		...
		final TableRecordReferenceSet mInOutLineTableRecordReferenceSet = TableRecordReferenceSet.of(mInOutLineTableRecordReference);
		...
		final HUsForInventoryStrategy strategy = createShortageAndOverageStrategy(huRecord, p_Qty);

		final DraftInventoryLinesCreateRequest request = DraftInventoryLinesCreateRequest.builder()
				.inventory(inventoryRepository.getById(inventoryId))
				.lineAggregator(InventoryLineAggregatorFactory.getForAggregationMode(SINGLE_HU))
				.strategy(strategy)
				.build();

		DraftInventoryLinesCreateCommand.builder()
	...
	/**
	 * Builds inventory-line inputs for the single received HU, adjusting the counted quantity by the shortage/overage
	 * {@code qty} (positive = overage, negative = shortage). Reproduces the former
	 * {@code HuForInventoryLineFactory#ofHURecordWithQtyAdjustment} + {@code ShortageAndOverageStrategy}, which are no
	 * longer available in the handlingunits base API.
	 */
	private HUsForInventoryStrategy createShortageAndOverageStrategy(@NonNull final I_M_HU huRecord, @NonNull final BigDecimal qty)
	{
		final OrgId orgId = OrgId.ofRepoId(huRecord.getAD_Org_ID());

		final List<HuForInventoryLine> lines = handlingUnitsBL
				.getStorageFactory()
				.streamHUProductStorages(huRecord)
				.filter(huProductStorage -> !huProductStorage.isEmpty())
				.map(huProductStorage -> {
		...
		return HUsForInventoryStrategies.of(lines);
	}
