...
UPDATE AD_Tab_Trl SET Description=$desc$Components that are automatically added to the order when a trigger product (main product) is entered. Important: the schema is not triggered by these rows — it is triggered via the master-data field M_Product.C_CompensationGroup_Schema_ID (Product → “Compensation Group Schema”). Only main products should carry that pointer — never the components listed here, otherwise ordering the component alone will also fire the bundle.$desc$,IsTranslated='Y',Updated=NOW(),UpdatedBy=100 WHERE AD_Tab_ID=544005 AND AD_Language='en_US'
;
