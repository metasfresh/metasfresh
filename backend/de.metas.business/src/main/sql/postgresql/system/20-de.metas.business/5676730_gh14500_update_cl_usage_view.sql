...
FROM (WITH data AS (SELECT dsc.m_department_id,
           ...
           data_totals AS (SELECT m_department_id,
                           ...
                           FROM data
                           GROUP BY
                               GROUPING SETS ( (m_department_id, section_group_partner_id, c_bpartner_id, c_currency_id),
                                               (m_department_id, section_group_partner_id, c_currency_id),
                                               (m_department_id, c_currency_id)
                               )
                           ORDER BY m_department_id, section_group_partner_id, c_bpartner_id)
      ...
      SELECT 1000000 + ROW_NUMBER() OVER ()                         AS CreditLimit_Usage_V_ID,
             ...
             c_bpartner_id,
             (SELECT bp.m_sectioncode_id
              FROM c_bpartner bp
              WHERE bp.c_bpartner_id = dt.c_bpartner_id)            AS m_sectioncode_id,
      ...
      ORDER BY m_department_id, section_group_partner_id, c_bpartner_id) cl
;
