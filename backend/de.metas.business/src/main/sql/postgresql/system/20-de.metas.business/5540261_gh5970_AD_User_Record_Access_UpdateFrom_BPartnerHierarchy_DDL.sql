...
create or replace function AD_User_Record_Access_UpdateFrom_BPartnerHierarchy()
returns void
as
$BODY$ 
declare
...
begin
  ...
  with recursive bpartner as (
      ...
      where SalesRep_ID is not null
        and bpartner_parent_id is null
        --
      union all
      --
      ...
      from bpartner parent
               inner join c_bpartner child on child.bpartner_parent_id = parent.c_bpartner_id
  )
  insert
  ...
  --
  --
  delete from ad_user_record_access where 1 = 1;
  GET DIAGNOSTICS v_count = ROW_COUNT;
