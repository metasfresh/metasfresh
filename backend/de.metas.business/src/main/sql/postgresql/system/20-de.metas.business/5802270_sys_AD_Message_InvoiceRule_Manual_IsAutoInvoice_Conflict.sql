...
-- InvoiceRule Manual —Add AD_Message used by the C_Order interceptor that forbids InvoiceRule=Manual together with IsAutoInvoice=Y
...
-- InvoiceRule Manual —Set the ErrorCode for API consumers (per metasfresh-db skill rule on AD_Message error codes)
