...
class PlanLoaderAndSaver
{
	private static final Logger logger = LogManager.getLogger(PlanLoaderAndSaver.class);
	...
	public Plan getPlan(@NonNull final SimulationPlanId simulationId)
	{
		...
		for (final WOProject woProject : projects)
		{
			...
			for (WOProjectStep step : steps.toOrderedList())
			{
				...
				{
					final Duration duration = resourceOrig.getDuration();
					if (duration.toSeconds() <= 0)
					{
						logger.warn("Excluding step/resource from plan because duration is not positive: {}", resourceOrig);
						continue;
					}
