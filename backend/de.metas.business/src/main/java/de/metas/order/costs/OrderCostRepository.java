...
public class OrderCostRepository
{
	...
	public void save(final OrderCost orderCost)
	{
		...
		final HashMap<OrderLineId, I_C_Order_Cost_Detail> existingDetailRecords = queryBL.createQueryBuilder(I_C_Order_Cost_Detail.class)
				.addEqualsFilter(I_C_Order_Cost_Detail.COLUMNNAME_C_Order_Cost_ID, orderCost.getId())
				.stream()
	...
	private static void updateRecord(final I_C_Order_Cost record, final OrderCost from)
	{
		...
		record.setCostCalculationMethod(from.getCalculationMethod().getCode());
	...
	private static void updateRecord(final I_C_Order_Cost_Detail record, final OrderCostDetail from)
	{
		...
		record.setC_Currency_ID(from.getOrderLineNetAmt().getCurrencyId().getRepoId());
