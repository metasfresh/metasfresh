...
public class OrderPayScheduleLoaderAndSaver
{
	...
	private static @NotNull OrderPayScheduleId extractId(final @NotNull I_C_OrderPaySchedule record)
	{
	...
	private void save0(@NonNull final OrderPaySchedule orderPaySchedule)
	{
		...
		final HashMap<OrderPayScheduleId, I_C_OrderPaySchedule> records = getRecordsByOrderId(orderId)
				.stream()
				.collect(GuavaCollectors.toHashMapByKey(OrderPayScheduleLoaderAndSaver::extractId));
		final SeqNoProvider seqNoProvider = SeqNoProvider.start();

		for (final OrderPayScheduleLine line : orderPaySchedule.getLines())
		{
			final I_C_OrderPaySchedule record = line.getId() == null
					? InterfaceWrapperHelper.newInstance(I_C_OrderPaySchedule.class)
					: records.remove(line.getId());
			...
			updateRecord(record, line, seqNoProvider);
			InterfaceWrapperHelper.save(record);
			line.markSaved(extractId(record));
		}
	...
	private static void updateRecord(
			@NonNull final I_C_OrderPaySchedule record,
			@NonNull final OrderPayScheduleLine from,
			@NonNull final SeqNoProvider seqNoProvider)
	{
		...
		record.setM_InOut_ID(InOutId.toRepoId(from.getInoutId()));
		record.setC_Invoice_ID(InvoiceId.toRepoId(from.getInvoiceId()));
