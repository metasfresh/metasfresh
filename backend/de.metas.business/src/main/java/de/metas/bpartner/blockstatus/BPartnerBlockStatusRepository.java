...
class BPartnerBlockStatusRepository
{
	...
	@NonNull
	public BPartnerBlockStatus create(@NonNull final CreateBPartnerBlockStatusRequest createBPartnerBlockStatusRequest)
	{
		final I_C_BPartner_BlockStatus blockStatusRecord = InterfaceWrapperHelper.newInstance(I_C_BPartner_BlockStatus.class);
		blockStatusRecord.setBlockStatus(createBPartnerBlockStatusRequest.getBlockStatus().getCode());
		blockStatusRecord.setC_BPartner_ID(createBPartnerBlockStatusRequest.getBPartnerId().getRepoId());
		blockStatusRecord.setAD_Org_ID(createBPartnerBlockStatusRequest.getOrgId().getRepoId());
		blockStatusRecord.setReason(createBPartnerBlockStatusRequest.getReason());
	...
	public Optional<BPartnerBlockStatus> retrieveLatestByBPartnerId(@NonNull final BPartnerId bPartnerId)
	{
		return queryBL.createQueryBuilder(I_C_BPartner_BlockStatus.class)
				...
				.orderByDescending(I_C_BPartner_BlockStatus.COLUMNNAME_C_BPartner_BlockStatus_ID)
				.create()
				.firstOptional()
