...
class ProformaOrderAllocateCommand
{
	...
	private ProformaOrderAlloc execute0()
	{
		...
		// Recalculate pay schedule amounts based on Proforma invoice
		// The C_Order interceptor fires updatePayScheduleStatus() but only updates status/dates by default
		// We need to explicitly recalculate amounts when Proforma is allocated
		orderPayScheduleService.updatePayScheduleStatus(order, true);
