...
CREATE OR REPLACE VIEW edi_desadvpack_sscc_label
            (no_of_labels, SSCC, order_reference, date_shipped, GTIN, product_description, amount, amount_LU, weight, netweight, best_before, lot_no,
             bp_name, bp_address_GLN, bp_address_name1, bp_address_name2, bp_address_street, bp_address_zip_code, bp_address_city, bp_address_country,
             ho_name, ho_address_GLN, ho_address_name1, ho_address_name2, ho_address_street, ho_address_zip_code, ho_address_city,
             cnt_packs, cnt_in_packs, p_instance_id)
AS
SELECT
    1                                                                           AS no_of_labels,
    ...
    dl_pack.qtycusperlu                                                         AS amount_LU,
    p.weight						                                            AS weight,
    ...
    (dl_pack.edi_desadvline_pack_id - (select min(pk.edi_desadvline_pack_id)
                                          from edi_desadvline_pack pk
...
from t_selection t_sel
	...
	left join m_product p on p.m_product_id = dl.m_product_id;