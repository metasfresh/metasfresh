...
SELECT
    ...
    /* IF docSubType is CS, the we don't reference the original shipment*/
     , CASE WHEN dt.DocSubType='CS' THEN NULL ELSE COALESCE(s.MovementDate, desadv.MovementDate) END AS MovementDate
     , CASE WHEN dt.DocSubType='CS' THEN NULL ELSE COALESCE(s.DocumentNo, desadv.DocumentNo) END AS Shipment_DocumentNo
     , t.TotalVAT
     , t.TotalTaxBaseAmt
