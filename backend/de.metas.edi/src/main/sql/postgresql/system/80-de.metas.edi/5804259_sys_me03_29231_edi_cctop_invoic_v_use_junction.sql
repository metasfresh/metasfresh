...
-- Recreates edi_cctop_invoic_v (and its dependents) replacing the legacy single-FK join
--   INNER JOIN EDI_Desadv edi ON inout.EDI_Desadv_ID = edi.EDI_Desadv_ID
-- with a junction-based join through EDI_Desadv_M_InOut.
-- This must run before 5804260 which drops M_InOut.EDI_Desadv_ID.

-- Drop dependents first (they reference edi_cctop_invoic_v by name)
DROP VIEW IF EXISTS historical_invoices_json_v;
DROP VIEW IF EXISTS C_Invoice_Export_EDI_INVOIC_JSON_V;

-- Recreate edi_cctop_invoic_v without the dropped FK column reference
    ...
    /* IF docSubType is CS, the we don't reference the original shipment*/
     ...
     , (SELECT STRING_AGG(DISTINCT edi.DocumentNo, ',')
        FROM c_invoiceline icl
                 ...
                 INNER JOIN EDI_Desadv_M_InOut link ON link.M_InOut_ID = inout.M_InOut_ID AND link.IsActive = 'Y'
                 INNER JOIN EDI_Desadv edi ON edi.EDI_Desadv_ID = link.EDI_Desadv_ID
...
-- Recreate C_Invoice_Export_EDI_INVOIC_JSON_V (depends on edi_cctop_invoic_v)
...
-- Recreate historical_invoices_json_v (depends on edi_cctop_invoic_v)
