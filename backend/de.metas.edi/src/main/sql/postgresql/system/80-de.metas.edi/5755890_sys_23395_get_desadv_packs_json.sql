...
BEGIN
    SELECT
        jsonb_agg(
                jsonb_build_object(
                        ...
                        'M_HU_PackagingCode_Text', pc_lu.packagingcode, -- Assuming m_hu_packagingcode has name or text_col
    ...
    WHERE edp_lat.edi_desadv_id = p_edi_desadv_id AND edp_lat.isactive = 'Y'; -- Filter by parameter!

    RETURN COALESCE(v_packs_json, '[]'::jsonb);
END;
$$ LANGUAGE plpgsql STABLE; -- STABLE because it doesn't modify DB and returns same result for same args within a transaction