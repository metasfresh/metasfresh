...
public class CiiValidatorTest
{
	 ...
	 * at 19% with {@code LineNetAmt} 100.00 + 100.00 (GROSS) and per-line {@code TaxAmt} 15.97 each,
	 * where the VAT breakdown carries the values metasfresh actually books for a per-line tax —
	 * {@code TaxBaseAmt = Σ(LineNetAmt − TaxAmt)} = 168.06, {@code TaxAmt} = 31.94. BT-131 reuses the
	 * booked net (84.03 each) and sums to {@code TaxBaseAmt} exactly (see
	 * {@link CiiMapperTest#tax_included_multiLine_bt131_reuses_booked_net}), proving the reuse approach
	 * also satisfies the real KoSIT schematron (no BR-S-08 / BR-CO-10).
	 */
	private FixtureResult buildXRechnungTaxIncludedMultiLineFixture()
	{
		...
		// === VAT breakdown (BG-23): the values metasfresh books for a per-line tax —
		// TaxBaseAmt = Σ(LineNetAmt − TaxAmt) = 168.06, TaxAmt = 31.94 ===
		final I_C_InvoiceTax invoiceTax = newInstance(I_C_InvoiceTax.class);
		invoiceTax.setC_Invoice_ID(invoice.getC_Invoice_ID());
		invoiceTax.setC_Tax_ID(tax.getC_Tax_ID());
		invoiceTax.setIsTaxIncluded(true);
		invoiceTax.setTaxBaseAmt(new BigDecimal("168.06"));
		invoiceTax.setTaxAmt(new BigDecimal("31.94"));
		saveRecord(invoiceTax);
	...
	/**
	 * Builds a tax-INCLUDED XRechnung fixture with a single line of {@code QtyInvoiced} 3 @
	 * {@code PriceActual} 3.33 (GROSS), {@code LineNetAmt} 9.99, {@code TaxAmt} 1.60. The VAT breakdown
	 * carries {@code TaxBaseAmt = LineNetAmt − TaxAmt} = 8.39 and {@code TaxAmt} 1.60; TotalLines /
	 * GrandTotal = 9.99. The point is the per-unit/line-net divergence: BT-146 is the derived per-unit
	 * net {@code 3.33 / 1.19} rounded = 2.80, so BT-146 × qty (3) = 8.40 while BT-131 = 8.39. EN16931
	 * treats the unit price as informative and KoSIT enforces only Σ BT-131 = BT-116 / BT-106, so this
	 * must still validate with zero fatal/error rules.
	 */
	private FixtureResult buildXRechnungTaxIncludedQtyGreater1Fixture()
	{
		// === Seller org ===
		final I_AD_Org org = newInstance(I_AD_Org.class);
		saveRecord(org);
		...
		// === Invoice: IsTaxIncluded=Y, single qty-3 line, TotalLines/GrandTotal = 9.99 (gross) ===
		final I_C_Invoice invoice = newInstance(I_C_Invoice.class);
		invoice.setAD_Org_ID(org.getAD_Org_ID());
		...
		invoice.setC_BPartner_Location_ID(buyerBPLoc.getC_BPartner_Location_ID());
		invoice.setIsTaxIncluded(true);
		invoice.setTotalLines(new BigDecimal("9.99"));
		invoice.setGrandTotal(new BigDecimal("9.99"));
		invoice.setPaymentRule("T");
		saveRecord(invoice);
		...
		// === VAT breakdown (BG-23): TaxBaseAmt = LineNetAmt − TaxAmt = 9.99 − 1.60 = 8.39, TaxAmt 1.60 ===
		final I_C_InvoiceTax invoiceTax = newInstance(I_C_InvoiceTax.class);
		invoiceTax.setC_Invoice_ID(invoice.getC_Invoice_ID());
		invoiceTax.setC_Tax_ID(tax.getC_Tax_ID());
		invoiceTax.setIsTaxIncluded(true);
		invoiceTax.setTaxBaseAmt(new BigDecimal("8.39"));
		invoiceTax.setTaxAmt(new BigDecimal("1.60"));
		saveRecord(invoiceTax);

		// === UOM + product + single qty-3 line: PriceActual 3.33 gross, LineNetAmt 9.99, TaxAmt 1.60 ===
		final I_C_UOM uom = newInstance(I_C_UOM.class);
		uom.setName("Stück");
		uom.setX12DE355("PCE");
		saveRecord(uom);
		...
		final I_M_Product product = newInstance(I_M_Product.class);
		product.setName("Testprodukt Gamma");
		product.setValue("TP-703-A");
		saveRecord(product);
		...
		line.setPriceActual(new BigDecimal("3.33"));
		line.setLineNetAmt(new BigDecimal("9.99"));
		line.setTaxAmt(new BigDecimal("1.60"));
		saveRecord(line);
	...
	/**
	 * Real KoSIT-schematron proof that a per-unit/line-net rounding divergence is tolerated: a
	 * tax-included line qty 3 @ gross 3.33 where BT-146 (2.80) × qty (3) = 8.40 diverges from BT-131
	 * (8.39). EN16931 treats the unit price as informative and KoSIT enforces only the Σ BT-131 rules,
	 * so the invoice must validate with ZERO fatal/error rule ids — in particular NEITHER BR-S-08 NOR
	 * BR-CO-10.
	 */
	@Test
	void validate_xrechnung_taxIncluded_qtyGreater1_priceRoundingDivergence_noFatalErrors() throws Exception
	{
		final FixtureResult fixture = buildXRechnungTaxIncludedQtyGreater1Fixture();
		final CrossIndustryInvoiceType cii = new CiiMapper().map(fixture.invoice, fixture.recipientConfig);
		final String xml = marshalToXml(cii);
		...
		final List<String> fatalErrors = result.getFatalAndErrorRuleIds();
		assertThat(fatalErrors)
				.as("Expected ZERO fatal/error KoSIT rule violations on the qty>1 price-rounding-divergence "
						+ "tax-included XRechnung fixture. Firing rules (mapper gaps): " + fatalErrors
						+ ". All failed assertions: " + result.getFailedAssertions())
				.isEmpty();
		assertThat(fatalErrors)
				.as("BR-S-08 (line/VAT-breakdown net mismatch) must not fire. Actual rule ids: %s", fatalErrors)
	...
	/**
	 * Builds a complete, EN16931/XRechnung-valid, tax-INCLUDED, THREE-line invoice fixture whose booked
	 * per-line nets fall one cent SHORT of the VAT breakdown's {@code TaxBaseAmt}, so the mapper's
	 * last-line residual absorption is exercised end-to-end: three lines at 19% with {@code LineNetAmt}
	 * 100.00 (GROSS) and per-line {@code TaxAmt} 15.97 each → booked net 84.03 each, Σ = 252.09. The VAT
	 * breakdown carries the tax-precision round-of-sum {@code TaxBaseAmt} = 252.10 (a 1-cent residual,
	 * within the 3-line cap of 0.03) and {@code TaxAmt} = 47.90 (= round(252.10 × 19%)). The mapper
	 * absorbs the residual on the last line (84.03, 84.03, 84.04) so {@code Σ BT-131 = 252.10 = TaxBaseAmt}.
	 * Totals: {@code TotalLines}/{@code GrandTotal} = 300.00 gross, and BT-109 (252.10) + BT-110 (47.90) =
	 * 300.00 = BT-112, so the whole document stays internally consistent for KoSIT (BR-CO-13 etc.).
	 */
	private FixtureResult buildXRechnungTaxIncludedResidualAbsorbedFixture()
	{
		// === Seller org ===
		final I_AD_Org org = newInstance(I_AD_Org.class);
		saveRecord(org);
		...
		// === Invoice: IsTaxIncluded=Y, three lines, TotalLines/GrandTotal = 300.00 (gross) ===
		final I_C_Invoice invoice = newInstance(I_C_Invoice.class);
		invoice.setAD_Org_ID(org.getAD_Org_ID());
		...
		invoice.setC_BPartner_Location_ID(buyerBPLoc.getC_BPartner_Location_ID());
		invoice.setIsTaxIncluded(true);
		invoice.setTotalLines(new BigDecimal("300.00"));
		invoice.setGrandTotal(new BigDecimal("300.00"));
		invoice.setPaymentRule("T");
		saveRecord(invoice);
		...
		// === VAT breakdown (BG-23): round-of-sum TaxBaseAmt = 252.10 (booked nets sum to 252.09),
		// TaxAmt = 47.90 (= round(252.10 × 19%) = 300.00 gross − 252.10 net) ===
		final I_C_InvoiceTax invoiceTax = newInstance(I_C_InvoiceTax.class);
		invoiceTax.setC_Invoice_ID(invoice.getC_Invoice_ID());
		invoiceTax.setC_Tax_ID(tax.getC_Tax_ID());
		invoiceTax.setIsTaxIncluded(true);
		invoiceTax.setTaxBaseAmt(new BigDecimal("252.10"));
		invoiceTax.setTaxAmt(new BigDecimal("47.90"));
		saveRecord(invoiceTax);

		// === UOM + products + three lines: LineNetAmt/PriceActual GROSS (100.00 each), TaxAmt 15.97 ===
		final I_C_UOM uom = newInstance(I_C_UOM.class);
		uom.setName("Stück");
		uom.setX12DE355("PCE");
		saveRecord(uom);
		...
		final I_M_Product product3 = newInstance(I_M_Product.class);
		product3.setName("Testprodukt Gamma");
		product3.setValue("TP-704-C");
		saveRecord(product3);
		...
		final I_C_InvoiceLine line3 = newInstance(I_C_InvoiceLine.class);
		line3.setC_Invoice_ID(invoice.getC_Invoice_ID());
		line3.setLine(30);
		line3.setM_Product_ID(product3.getM_Product_ID());
		line3.setC_UOM_ID(uom.getC_UOM_ID());
		line3.setC_Tax_ID(tax.getC_Tax_ID());
		line3.setQtyInvoiced(new BigDecimal("1"));
		line3.setPriceActual(new BigDecimal("100.00"));
		line3.setLineNetAmt(new BigDecimal("100.00"));
		line3.setTaxAmt(new BigDecimal("15.97"));
		saveRecord(line3);
	...
	/**
	 * Real KoSIT-schematron proof of the last-line residual absorption: a three-line tax-included
	 * XRechnung whose booked per-line nets (Σ 252.09) fall one cent short of the round-of-sum
	 * {@code TaxBaseAmt} (252.10). The mapper absorbs the 1-cent residual on the last line so
	 * {@code Σ BT-131 = 252.10 = TaxBaseAmt}, and the marshalled XML must validate with ZERO fatal/error
	 * rule ids — specifically NEITHER BR-S-08 (BT-131 sum vs. BT-116) NOR BR-CO-10 (BT-106 vs. Σ BT-131).
	 */
	@Test
	void validate_xrechnung_taxIncluded_residualAbsorbed_noFatalErrors() throws Exception
	{
		final FixtureResult fixture = buildXRechnungTaxIncludedResidualAbsorbedFixture();
		final CrossIndustryInvoiceType cii = new CiiMapper().map(fixture.invoice, fixture.recipientConfig);
		final String xml = marshalToXml(cii);
		...
		final List<String> fatalErrors = result.getFatalAndErrorRuleIds();
		assertThat(fatalErrors)
				.as("Expected ZERO fatal/error KoSIT rule violations on the residual-absorbed tax-included "
						+ "XRechnung fixture. Firing rules (mapper gaps): " + fatalErrors
						+ ". All failed assertions: " + result.getFailedAssertions())
				.isEmpty();
		assertThat(fatalErrors)
