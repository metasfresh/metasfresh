...
public class JasperClassLoaderTests
{
	...
	{
		assertThat(JasperClassLoader.isJarInJarURL(URI.create("file:///opt/metasfresh/metasfresh-server.jar!/lib/spring-beans-4.2.5.RELEASE.jar").toURL())).isTrue();
		assertThat(JasperClassLoader.isJarInJarURL(URI.create("jar:file:/opt/metasfresh/metasfresh_server.jar!/lib/spring-beans-4.2.5.RELEASE.jar!/org/springframework/beans/factory/xml/spring-beans-2.0.xsd").toURL())).isTrue();
		assertThat(JasperClassLoader.isJarInJarURL(URI.create("file:///opt/metasfresh/reports/de/metas/docs/direct_costing/report.jasper").toURL())).isFalse();
	}
