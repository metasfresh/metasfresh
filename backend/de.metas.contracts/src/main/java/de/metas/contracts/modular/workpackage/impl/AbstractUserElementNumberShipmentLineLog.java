...
@Component
public abstract class AbstractUserElementNumberShipmentLineLog extends AbstractModularContractLogHandler
{
	...
	public AbstractUserElementNumberShipmentLineLog(
			...
			final @NonNull QuantityUOMConverter uomConverter)
	{
		...
		final ProductId productId = Check.assumeNotNull(logEntry.getProductId(), "ProductId shouldn't be null");
		final ProductPrice priceActual = Check.assumeNotNull(logEntry.getPriceActual(), "PriceActual shouldn't be null");
		...
		() ->
		{
			final Money zero = Money.zero(priceActual.getCurrencyId());
