		...
		IProcessDefaultParametersProvider
{
	...
	@Param(parameterName = PARAM_AD_ORG_SOURCE_ID, mandatory = true)
	protected OrgId p_orgSourceId;
	...
	{
		final ImmutableSet<BPartnerId> bPartnerIdsToMove = bpartnerBL.getBPartnerIdsForOrg(p_orgSourceId);

		for (final BPartnerId bPartnerId : bPartnerIdsToMove)
		{
			final OrgChangeRequest orgChangeRequest = OrgChangeRequest.builder()
					.bpartnerId(bPartnerId)
					.startDate(p_startDate)
					.groupCategoryId(p_groupCategoryId)
					.orgFromId(p_orgSourceId)
