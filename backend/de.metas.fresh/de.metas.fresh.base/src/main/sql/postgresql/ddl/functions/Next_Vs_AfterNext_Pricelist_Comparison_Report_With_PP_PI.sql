DROP FUNCTION IF EXISTS report.Next_Vs_AfterNext_Pricelist_Comparison_Report_With_PP_PI(p_C_BPartner_ID              numeric,
                                                                                        p_C_BP_Group_ID              numeric,
                                                                                        p_IsSoTrx                    text,
                                                                                        p_AD_Language                text,
                                                                                        p_show_product_price_pi_flag text)
;

CREATE OR REPLACE FUNCTION report.Next_Vs_AfterNext_Pricelist_Comparison_Report_With_PP_PI(p_C_BPartner_ID              numeric = NULL,
     ...
     PriceListVersionsByValidFrom AS
         (SELECT t.*
          FROM ((SELECT --
                        ...
                        plv.validfrom,
                        plv.name,
                        ROW_NUMBER() OVER (PARTITION BY plv.c_bpartner_id ORDER BY plv.validfrom ASC, plv.m_pricelist_version_id ASC) rank
...
SELECT *
FROM report.Next_Vs_AfterNext_Pricelist_Comparison_Report_With_PP_PI()
;

- Specific Bpartner

SELECT *
FROM report.Next_Vs_AfterNext_Pricelist_Comparison_Report_With_PP_PI(2156515)
;
 */
