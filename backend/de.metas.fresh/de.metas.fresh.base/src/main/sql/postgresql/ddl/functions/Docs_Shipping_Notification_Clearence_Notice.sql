...
SELECT sn.ad_org_id,
       ...
       t3.url     AS orgURL,
       TRIM(COALESCE(bp.companyname, wbp.name, '') || E'\n' ||
            COALESCE(bploc.address1 || E'\n', '') ||
            COALESCE(bploc.postal, '') || ' ' ||
            COALESCE(bploc.city, '')
       )          AS customerAddress,
       u.phone    AS customerPhone,
       u.fax      AS customerFax,
       u.email    AS customerEmail,
       TRIM(COALESCE(wbp.companyname, wbp.name, '') || E'\n' ||
            COALESCE(wbploc.address1 || E'\n', '') ||
            COALESCE(wbploc.postal, '') || ' ' ||
            COALESCE(wbploc.city, '')
       )          AS DeliveryFrom,
...
FROM M_Shipping_Notification sn
         ...
         INNER JOIN c_location bploc ON bploc.c_location_id = bpl.c_location_id
