-- gh#28336: Detail view for Intrastat window.
-- Shows individual invoice/shipment line combinations for intra-EU trade.
-- NOT aggregated (unlike Intrastat_Report_V which groups by commodity).
...
DROP VIEW IF EXISTS Intrastat_Report_Detail_V;

CREATE OR REPLACE VIEW Intrastat_Report_Detail_V AS
SELECT
    -- Synthetic PK: unique per invoice-line / shipment-line combination
    ...
    -- Quantity (per line, not aggregated)
    COALESCE(
        CASE
    ...
    -- Weight (per line, not aggregated)
    COALESCE(
