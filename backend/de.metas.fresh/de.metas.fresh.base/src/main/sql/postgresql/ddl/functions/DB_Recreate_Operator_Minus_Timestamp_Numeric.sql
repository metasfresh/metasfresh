...
CREATE OR REPLACE FUNCTION public.DB_Recreate_Operator_Minus_Timestamp_Numeric()
    RETURNS void
    LANGUAGE plpgsql
    VOLATILE
AS
...
BEGIN
    /*
     * 1. Validate that the subtractdays function exists with the correct signature
     */
    IF NOT EXISTS (SELECT 1
                   ...
                   WHERE n.nspname = 'public'
                     ...
                     AND p.pronargs = 2
                     AND p.proargtypes[0] = 'timestamp with time zone'::regtype::oid
                     AND p.proargtypes[1] = 'numeric'::regtype::oid) THEN
        RAISE EXCEPTION 'Function public.subtractdays(timestamp with time zone, numeric) does not exist with correct signature. Cannot recreate operator.';
    END IF;
    ...
    WHERE n.nspname = 'public'
      AND o.oprname = '-'
      AND o.oprleft = 'timestamp with time zone'::regtype
      AND o.oprright = 'numeric'::regtype;

    IF operator_oid IS NULL THEN
        RAISE EXCEPTION 'Operator public.- (timestamp with time zone, numeric) does not exist. Cannot recreate operator.';
    END IF;
    ...
    BEGIN
         ...
         */
        RAISE NOTICE 'Dropping operator public.- (timestamp with time zone, numeric)';
        EXECUTE 'DROP OPERATOR IF EXISTS public.- (timestamp with time zone, numeric)';

        RAISE NOTICE 'Creating operator public.- (timestamp with time zone, numeric) with owner %', operator_owner;
        EXECUTE FORMAT($op$
        ...
        IF operator_owner_oid IS NOT NULL AND current_user_oid IS NOT NULL
            ...
            EXECUTE FORMAT('ALTER OPERATOR public.- (timestamp with time zone, numeric) OWNER TO %I', operator_owner);
        END IF;
...
COMMENT ON FUNCTION public.DB_Recreate_Operator_Minus_Timestamp_Numeric() IS
    'Drops and recreates operator public.- (timestamp with time zone, numeric) by temporarily removing all dependent views and functions than restoring them afterwards.'
;
