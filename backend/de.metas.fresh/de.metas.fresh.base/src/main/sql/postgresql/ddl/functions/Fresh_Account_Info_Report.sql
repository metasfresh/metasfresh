...
SELECT
	...
	(CASE WHEN 
		...
		WHERE fa.ad_table_id = fa2.ad_table_id AND fa2.Record_ID =fa.Record_ID 
			...
			AND (case when fa.amtacctdr != 0 then fa2.amtacctcr != 0 when fa.amtacctcr != 0 then fa2.amtacctdr != 0 else false end ) AND fa2.isActive = 'Y') 
	...
	(
		...
		FROM 
			( 
				...
				WHERE ev.value >= ev_from.value and ev.value<= ev_to.value and char_length(ev.value) >= char_length(ev_from.value) and char_length(ev.value)<= char_length(ev_to.value) AND ev.isActive = 'Y' --getting elements between the selected values
