DROP FUNCTION IF EXISTS de_metas_endcustomer_fresh_reports.docs_flatrate_term_procurement_report_description(IN record_id   numeric,
                                                                                                             IN ad_language character varying)
;

CREATE OR REPLACE FUNCTION de_metas_endcustomer_fresh_reports.docs_flatrate_term_procurement_report_description(IN record_id   numeric,
                                                                                                                IN ad_language character varying)
    RETURNS TABLE
            (
...
SELECT
    -- header data
    COALESCE(ft.documentno, ft.c_flatrate_term_id::text) AS ContractNo,
    billBP.name,
    billbp.value,
    u.name,
    pmm.ProductName,
...
WHERE ft.C_Flatrate_Term_id = $1
  AND fc.type_conditions = 'Procuremnt'
  AND ft.isActive = 'Y'
