...
SELECT fa.DateAcct::Date,
       ...
       CASE
           WHEN p_showcurrencyexchange = 'Y' AND ConversionMultiplyRate IS NOT NULL THEN ConversionMultiplyRate * fa.AmtAcctDr
                                                                                    ELSE fa.AmtAcctDr
       END                                                                                                                                                   AS AmtAcctDr,
       CASE
           WHEN p_showcurrencyexchange = 'Y' AND ConversionMultiplyRate IS NOT NULL THEN ConversionMultiplyRate * fa.AmtAcctCr
                                                                                    ELSE fa.AmtAcctCr
       END                                                                                                                                                   AS AmtAcctCr,

       CASE
           WHEN p_showcurrencyexchange = 'Y' AND ConversionMultiplyRate IS NOT NULL THEN ConversionMultiplyRate * (SUM(fa.AmtAcctDr) OVER (PARTITION BY param_acct_value))
                                                                                    ELSE (SUM(fa.AmtAcctDr) OVER (PARTITION BY param_acct_value))
       END                                                                                                                                                   AS AmtAcctDrEnd,
       CASE
           WHEN p_showcurrencyexchange = 'Y' AND ConversionMultiplyRate IS NOT NULL THEN ConversionMultiplyRate * (SUM(fa.AmtAcctCr) OVER (PARTITION BY param_acct_value))
                                                                                    ELSE (SUM(fa.AmtAcctCr) OVER (PARTITION BY param_acct_value))
       END                                                                                                                                                   AS AmtAcctCrEnd,
       ...
       COALESCE(p_enddate::date, (SELECT enddate::date FROM C_Period WHERE C_Period_ID = p_period_end_id AND isActive = 'Y'))                                AS Param_End_Date,
       COALESCE(p_startdate::date, (SELECT startdate::date FROM C_Period WHERE C_Period_ID = p_period_start_id AND isActive = 'Y'))                          AS Param_Start_Date,

       pa.value                                                                                                                                              AS Param_Activity_Value,
       pa.Name                                                                                                                                               AS Param_Activit_Name,
       fa.docStatus,
       ConversionMultiplyRate,
       CASE WHEN p_showcurrencyexchange = 'Y' AND ConversionMultiplyRate IS NOT NULL THEN ConversionMultiplyRate * (CarryBalance + SUM(Balance) OVER ()) END AS EuroSaldo,
       containsEUR,
       fa.ad_org_id,
       fa.vat_code,
       fa.tax_rate_name
FROM (SELECT fa.Account_ID,
             ...
             (de_metas_acct.acctbalanceuntildate
                 (ev.C_ElementValue_ID,
                  acs.C_AcctSchema_ID,
                  COALESCE(p_startdate::date, (SELECT startdate::date FROM C_Period WHERE C_Period_ID = p_period_start_id)),
                  p_org_id)).Balance
                                                                                              AS CarryBalance,
             fa.DocStatus,
             ...
             currencyrate(
                     ...
                     COALESCE(p_enddate::date, (SELECT enddate::date FROM C_Period WHERE C_Period_ID = p_period_end_id AND isActive = 'Y')),
      ...
      FROM (SELECT ev.C_ElementValue_ID, ev.value, ev.name, ev.ad_client_id
            FROM C_ElementValue ev
                     JOIN C_ElementValue ev_from ON ev_from.C_ElementValue_ID = p_account_start_id AND ev_from.isActive = 'Y'
                     JOIN C_ElementValue ev_to ON ev_to.C_ElementValue_ID = p_account_end_id AND ev_to.isActive = 'Y'
          ...
          AND DateAcct >= COALESCE(p_startdate::date, (SELECT startdate::date FROM C_Period WHERE C_Period_ID = p_period_start_id AND isActive = 'Y'))
          AND DateAcct <= COALESCE(p_enddate::date, (SELECT enddate::date FROM C_Period WHERE C_Period_ID = p_period_end_id AND isActive = 'Y'))
          AND (CASE WHEN p_activity_id IS NOT NULL THEN COALESCE(C_Activity_ID = p_activity_id, FALSE) ELSE TRUE END) -- this used to be COALESCE( C_Activity_ID = p_activity_id, true) and it was showing the empty ones too when activity id was set
          AND fa.isActive = 'Y'
          ...
          --taking the currency of the client to convert it into euro
               ...
               LEFT OUTER JOIN C_Period period_LastYearEnd ON (period_LastYearEnd.C_Period_ID = report.Get_Predecessor_Period_Recursive(p_period_start_id, (SELECT PeriodNo::int FROM C_Period WHERE C_Period_ID = p_period_start_id AND isActive = 'Y'))) AND period_LastYearEnd.isActive = 'Y'
      WHERE fa.postingtype IN ('A', 'Y') -- task 09804 don't show/sum other than current (A) -- now we have other current Y (year end)
         ...
         LEFT OUTER JOIN C_Activity pa ON COALESCE(pa.C_Activity_ID = p_activity_id, FALSE) AND pa.isActive = 'Y'

WHERE CASE
          WHEN (p_displayvoiddocuments = 'Y') THEN
              TRUE
                                              ELSE
              (
  ...
  AND CASE
          WHEN (p_showonlyemptyactivity = 'N') THEN
              TRUE
                                               ELSE
              (
