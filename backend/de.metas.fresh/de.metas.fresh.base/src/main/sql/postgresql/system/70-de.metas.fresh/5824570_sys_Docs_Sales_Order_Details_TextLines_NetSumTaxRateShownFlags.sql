...
-- exactly as they are of an article row on the same document. IsNetSumShown / IsTaxRateShown are
-- the same idea one layer deeper: linenetamt and rate themselves ARE article-level and stay NULL,
-- but whether the Amount/Tax columns are shown AT ALL depends only on
-- report.IsHiddenReportElement(o.C_DocTypeTarget_ID, ...) -- a property of the ORDER's document
-- type, not of any article -- so that visibility flag is computed identically in both branches
-- instead of being inferred from a value column that a text row cannot supply. Only genuinely
-- ARTICLE-level columns (product, HU, qty, price, discount amount, tax rate/printing, compensation
-- group, weight, QtyPattern -- derived from an order line's UOM precision -- and iscampaignprice,
-- an order-line flag) are NULL. TextLine is nullable and an empty text line is legal (prints as a
-- blank line) -- it is passed straight through with no NULLIF/COALESCE that could coerce an empty
-- value away.
