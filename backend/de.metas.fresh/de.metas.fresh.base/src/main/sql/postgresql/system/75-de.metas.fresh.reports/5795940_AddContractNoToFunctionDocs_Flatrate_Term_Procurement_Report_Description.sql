DROP FUNCTION IF EXISTS de_metas_endcustomer_fresh_reports.Docs_Flatrate_Term_Procurement_Report_Description(IN p_record_id   numeric,
                                                                                                             IN p_ad_language character varying)
;

CREATE OR REPLACE FUNCTION de_metas_endcustomer_fresh_reports.Docs_Flatrate_Term_Procurement_Report_Description(IN p_record_id   numeric,
                                                                                                                IN p_ad_language character varying)
    RETURNS TABLE
            (
...
FROM C_Flatrate_Term ft
         ...
         LEFT OUTER JOIN C_UOM_trl uom_trl ON uom.C_UOM_ID = uom_trl.C_UOM_ID AND uom_trl.ad_language = p_ad_language

         JOIN c_period p_param_st ON ft.startdate >= p_param_st.startdate AND ft.startdate <= p_param_st.enddate
         LEFT OUTER JOIN C_Period_trl period_trl_st ON p_param_st.c_period_ID = period_trl_st.c_period_ID AND period_trl_st.ad_language = p_ad_language
         JOIN c_period p_param_end ON ft.enddate >= p_param_end.startdate AND ft.enddate <= p_param_end.enddate
         LEFT OUTER JOIN C_Period_trl period_trl_end ON p_param_end.c_period_ID = period_trl_end.c_period_ID AND period_trl_end.ad_language = p_ad_language

         JOIN C_Year y_st ON p_param_st.C_Year_ID = y_st.C_Year_ID
         JOIN C_Year y_end ON p_param_end.C_Year_ID = y_end.C_Year_ID

WHERE ft.C_Flatrate_Term_id = p_record_id
