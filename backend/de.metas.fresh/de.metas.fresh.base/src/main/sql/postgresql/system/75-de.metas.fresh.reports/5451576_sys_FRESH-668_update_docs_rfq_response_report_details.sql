...
INNER JOIN C_RfQResponse rr ON rl.C_RfQResponse_ID = rr.C_RfQResponse_ID AND rr.isActive = 'Y'
INNER JOIN PMM_Product pmm ON rl.PMM_Product_ID = pmm.PMM_Product_ID AND pmm.isActive = 'Y'
LEFT OUTER JOIN M_Product p ON pmm.M_Product_ID = p.M_Product_ID AND p.isActive = 'Y'
LEFT OUTER JOIN M_Product_Trl pt ON p.M_Product_ID = pt.M_Product_ID AND pt.AD_Language = $2 AND pt.isActive = 'Y'
INNER JOIN C_UOM uom ON rl.C_UOM_ID = uom.C_UOM_ID AND uom.isActive = 'Y'
LEFT OUTER JOIN C_UOM_Trl uomt ON uom.C_UOM_ID = uomt.C_UOM_ID AND uomt.AD_Language = $2 AND uomt.isActive = 'Y'
INNER JOIN C_Currency c ON rl.C_Currency_ID = c.C_Currency_ID AND c.isActive = 'Y'

WHERE rr.C_RfQResponse_ID = $1 AND rl.isActive = 'Y'
