DROP FUNCTION IF EXISTS de_metas_fresh_kpi.KPI_Purchase_Price_Range_Sub (IN startdate DATE, IN enddate DATE);
CREATE OR REPLACE FUNCTION de_metas_fresh_kpi.KPI_Purchase_Price_Range_Sub (IN startdate DATE, IN enddate DATE)
RETURNS TABLE
(
...
INNER JOIN M_Cost c ON p.M_Product_ID = c.M_Product_ID AND c.M_CostElement_ID = (SELECT M_CostElement_ID FROM M_CostElement WHERE costingmethod='A') 
