-- Recreates report.fresh_product_statistics_report(9 args) from the DDL source.
-- Companion to the fresh_statistics recreate — same drop-without-recreate cause
-- (backing tables were recreated with C_BPartner_ID; the wrapping functions were not).
