...
SELECT
	...
	'00' || tua_sscc18.Value || '413' || bp_loc.gln as barcode
...
LEFT JOIN (
		SELECT 	sub_tua.M_HU_ID, sub_tua.Value
		FROM	M_HU_Attribute sub_tua
			INNER JOIN M_Attribute sub_a ON sub_tua.M_Attribute_ID = sub_a.M_Attribute_ID AND sub_a.isActive = 'Y'
			
		WHERE 	sub_a.value = 'SSCC18' -- SSCC18
			 AND sub_tua.isActive = 'Y'
	) tua_sscc18 ON hu.M_HU_ID = tua_sscc18.M_HU_ID	
