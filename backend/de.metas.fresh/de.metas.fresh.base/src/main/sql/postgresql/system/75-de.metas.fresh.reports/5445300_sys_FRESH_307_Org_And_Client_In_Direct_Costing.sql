...
IN showRemaining character varying) -- only used in report
	RETURNS TABLE 
	(
		seq text,
		margin text,
...
SELECT	
	...
	*,
	...
	First_Agg( Margin ) OVER (PARTITION BY ad_org_id, ad_client_id ORDER BY Seq Desc ) != Margin AS isDisplayMarginSum,

	-- If there are no Sums in the 'Other'-Column, don't display it
	SUM( Balance_Other ) OVER (PARTITION BY ad_org_id, ad_client_id) != 0 AS isDisplayOther
FROM
	(
	SELECT
		*,
		-- Get initial profit to create the percentage of each line and/or sum
		NULLIF( First_agg ( L2_1000 ) OVER (PARTITION BY ad_org_id, ad_client_id ORDER BY Seq ), 0 ) AS Gross_1000,
		NULLIF( First_agg ( L2_2000 ) OVER (PARTITION BY ad_org_id, ad_client_id ORDER BY Seq ), 0 ) AS Gross_2000,
		NULLIF( First_agg ( L2_100 ) OVER (PARTITION BY ad_org_id, ad_client_id ORDER BY Seq ), 0 ) AS Gross_100,
		NULLIF( First_agg ( L2_150 ) OVER (PARTITION BY ad_org_id, ad_client_id ORDER BY Seq ), 0 ) AS Gross_150,
		NULLIF( First_agg ( L2_all ) OVER (PARTITION BY ad_org_id, ad_client_id ORDER BY Seq ), 0 ) AS Gross_All
	FROM
		(
		SELECT
			seq,
			...
			First_Agg( Incr_1000 ) OVER ( PARTITION BY Margin, ad_org_id, ad_client_id ORDER BY Seq DESC ) AS Margin_Incr_1000,
			First_Agg( Incr_2000 ) OVER ( PARTITION BY Margin, ad_org_id, ad_client_id ORDER BY Seq DESC ) AS Margin_Incr_2000,
			First_Agg( Incr_100 ) OVER ( PARTITION BY Margin, ad_org_id, ad_client_id ORDER BY Seq DESC ) AS Margin_Incr_100,
			First_Agg( Incr_150 ) OVER ( PARTITION BY Margin, ad_org_id, ad_client_id ORDER BY Seq DESC ) AS Margin_Incr_150,
			First_Agg( Incr_other ) OVER ( PARTITION BY Margin, ad_org_id, ad_client_id ORDER BY Seq DESC ) AS Margin_Incr_other,
			First_Agg( Incr ) OVER ( PARTITION BY Margin, ad_org_id, ad_client_id ORDER BY Seq DESC ) AS Margin_Incr_All,
			-- Level 1
			L1_Value, L1_Name, isDisplayL1Sum,
			Sum ( Balance_1000 ) OVER ( PARTITION BY Margin, L1_Value, ad_org_id, ad_client_id ) AS L1_1000,
			Sum ( Balance_2000 ) OVER ( PARTITION BY Margin, L1_Value, ad_org_id, ad_client_id ) AS L1_2000,
			Sum ( Balance_100 ) OVER ( PARTITION BY Margin, L1_Value, ad_org_id, ad_client_id ) AS L1_100,
			Sum ( Balance_150 ) OVER ( PARTITION BY Margin, L1_Value, ad_org_id, ad_client_id ) AS L1_150,
			Sum ( Balance_other ) OVER ( PARTITION BY Margin, L1_Value, ad_org_id, ad_client_id ) AS L1_other,
			Sum ( Balance ) OVER ( PARTITION BY Margin, L1_Value, ad_org_id, ad_client_id ) AS L1_All,
			L1_Multiplicator,
			-- Level 2
			L2_Value, L2_Name,
			Sum ( Balance_1000 ) OVER ( PARTITION BY Margin, L2_Value, ad_org_id, ad_client_id ) AS L2_1000,
			Sum ( Balance_2000 ) OVER ( PARTITION BY Margin, L2_Value, ad_org_id, ad_client_id ) AS L2_2000,
			Sum ( Balance_100 ) OVER ( PARTITION BY Margin, L2_Value, ad_org_id, ad_client_id ) AS L2_100,
			Sum ( Balance_150 ) OVER ( PARTITION BY Margin, L2_Value, ad_org_id, ad_client_id ) AS L2_150,
			Sum ( Balance_other ) OVER ( PARTITION BY Margin, L2_Value, ad_org_id, ad_client_id ) AS L2_other,
			Sum ( Balance ) OVER ( PARTITION BY Margin, L2_Value, ad_org_id, ad_client_id ) AS L2_All,
			L2_Multiplicator,
			-- Level 3
			L3_Value, L3_Name,
			...
			-- Margin
			First_Agg( Budget_Incr_1000 ) OVER ( PARTITION BY Margin, ad_org_id, ad_client_id ORDER BY Seq DESC ) AS Margin_Budget_Incr_1000,
			First_Agg( Budget_Incr_2000 ) OVER ( PARTITION BY Margin, ad_org_id, ad_client_id ORDER BY Seq DESC ) AS Margin_Budget_Incr_2000,
			First_Agg( Budget_Incr_100 ) OVER ( PARTITION BY Margin, ad_org_id, ad_client_id ORDER BY Seq DESC ) AS Margin_Budget_Incr_100,
			First_Agg( Budget_Incr_150 ) OVER ( PARTITION BY Margin, ad_org_id, ad_client_id ORDER BY Seq DESC ) AS Margin_Budget_Incr_150,
			First_Agg( Budget_Incr ) OVER ( PARTITION BY Margin, ad_org_id, ad_client_id ORDER BY Seq DESC ) AS Margin_Budget_Incr_All,
			-- Level 1
			Sum ( Budget_1000 ) OVER ( PARTITION BY Margin, L1_Value, ad_org_id, ad_client_id ) AS L1_Budget_1000,
			Sum ( Budget_2000 ) OVER ( PARTITION BY Margin, L1_Value, ad_org_id, ad_client_id ) AS L1_Budget_2000,
			Sum ( Budget_100 ) OVER ( PARTITION BY Margin, L1_Value, ad_org_id, ad_client_id ) AS L1_Budget_100,
			Sum ( Budget_150 ) OVER ( PARTITION BY Margin, L1_Value, ad_org_id, ad_client_id ) AS L1_Budget_150,
			Sum ( Budget ) OVER ( PARTITION BY Margin, L1_Value, ad_org_id, ad_client_id ) AS L1_Budget_All,
			-- Level 2
			Sum ( Budget_1000 ) OVER ( PARTITION BY Margin, L2_Value, ad_org_id, ad_client_id ) AS L2_Budget_1000,
			Sum ( Budget_2000 ) OVER ( PARTITION BY Margin, L2_Value, ad_org_id, ad_client_id ) AS L2_Budget_2000,
			Sum ( Budget_100 ) OVER ( PARTITION BY Margin, L2_Value, ad_org_id, ad_client_id ) AS L2_Budget_100,
			Sum ( Budget_150 ) OVER ( PARTITION BY Margin, L2_Value, ad_org_id, ad_client_id ) AS L2_Budget_150,
			Sum ( Budget ) OVER ( PARTITION BY Margin, L2_Value, ad_org_id, ad_client_id ) AS L2_Budget_All,
			-- Level 3
		...
		FROM
			...
			SELECT
				...
				Balance_1000, SUM( Balance_1000 ) OVER (PARTITION BY ad_org_id, ad_client_id ORDER BY Seq ) AS Incr_1000,
				Balance_2000, SUM( Balance_2000 ) OVER (PARTITION BY ad_org_id, ad_client_id ORDER BY Seq ) AS Incr_2000,
				Balance_100, SUM( Balance_100 ) OVER (PARTITION BY ad_org_id, ad_client_id ORDER BY Seq ) AS Incr_100,
				Balance_150, SUM( Balance_150 ) OVER (PARTITION BY ad_org_id, ad_client_id ORDER BY Seq ) AS Incr_150,
				Balance_other, SUM( Balance_other ) OVER (PARTITION BY ad_org_id, ad_client_id ORDER BY Seq ) AS Incr_other,
				Balance, SUM( Balance ) OVER (PARTITION BY ad_org_id, ad_client_id ORDER BY Seq ) AS Incr,
				Budget_1000, SUM( Budget_1000 ) OVER (PARTITION BY ad_org_id, ad_client_id ORDER BY Seq ) AS Budget_Incr_1000,
				Budget_2000, SUM( Budget_2000 ) OVER (PARTITION BY ad_org_id, ad_client_id ORDER BY Seq ) AS Budget_Incr_2000,
				Budget_100, SUM( Budget_100 ) OVER (PARTITION BY ad_org_id, ad_client_id ORDER BY Seq ) AS Budget_Incr_100,
				Budget_150, SUM( Budget_150 ) OVER (PARTITION BY ad_org_id, ad_client_id ORDER BY Seq ) AS Budget_Incr_150,
				Budget, SUM( Budget ) OVER (PARTITION BY ad_org_id, ad_client_id ORDER BY Seq ) AS Budget_Incr,
				L3_Multiplicator, L2_Multiplicator, L1_Multiplicator,
				ad_org_id,
				ad_client_id
			FROM
				de_metas_endcustomer_fresh_reports.Direct_Costing_Raw_Data($1)
			) x
		) y
	) z
	LEFT OUTER JOIN (
