DROP FUNCTION IF EXISTS de_metas_endcustomer_fresh_reports.Docs_Generics_Org_Report(IN p_Email     Character Varying(60),
                                                                                    IN p_IsSOTrx   Character Varying(1),
...
CREATE FUNCTION de_metas_endcustomer_fresh_reports.Docs_Generics_Org_Report(IN p_Email     Character Varying(60),
                                                                            IN p_IsSOTrx   Character Varying(1),
                                                                            IN p_AD_Org_ID Numeric)
    RETURNS TABLE
            (
 ...
 FROM ad_org ad_org
          ...
          INNER JOIN C_BPartner_Location org_bpl ON org_bpl.C_BPartner_Location_ID =
                                                    (SELECT C_BPartner_Location_ID
                                                     FROM C_BPartner_Location sub_bpl
          ...
          LEFT OUTER JOIN AD_User us ON us.AD_User_ID =
                                        (SELECT AD_User_ID
                                         ...
                                         LIMIT 1)
 WHERE ad_org.ad_org_id = p_AD_Org_ID)
$$
    LANGUAGE SQL STABLE
;
