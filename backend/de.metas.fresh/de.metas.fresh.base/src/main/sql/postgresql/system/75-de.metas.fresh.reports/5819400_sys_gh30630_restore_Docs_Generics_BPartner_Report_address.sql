-- 2026-08-16
-- gh30630 - restore de_metas_endcustomer_fresh_reports.docs_generics_bpartner_report to its canonical
-- ddl version (RETURNS TABLE incl. the address2 column). On this feature branch the applied function had
-- regressed to a form missing address2, which broke the shared report_bp_org_block_left address-block
-- subreport at fill time (JRException: Unknown column name address2 in result set) - visible e.g. on the
-- Purchase Order jasper (works on the base build, failed on the feature build). This recreates the
-- function verbatim from the source of truth so the applied version matches it:
--   backend/de.metas.fresh/de.metas.fresh.base/src/main/sql/postgresql/ddl/functions/Docs_Generic_BPartner_Report.sql
-- Numbered after the prior definers (5756680 / 5760210) so this is the effective definition.
