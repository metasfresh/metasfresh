...
-- AmountPattern (the currency) are equally document-level, and isDiscountPrinted comes from the
-- order's business partner -- none of these describe an article, so they are true of a text row
-- exactly as they are of an article row on the same document. Only genuinely ARTICLE-level columns
-- (product, HU, qty, price, discount amount, tax rate/printing, compensation group, weight,
-- QtyPattern -- derived from an order line's UOM precision -- and iscampaignprice, an order-line
-- flag) are NULL. TextLine is nullable and an empty text line is legal (prints as a blank line) --
...
SELECT tl.line                                                AS line,
       ...
       NULL::character(1)                                      AS isPrintTax,
...
-- puts a text line ahead of an article line landing on the exact same position.
