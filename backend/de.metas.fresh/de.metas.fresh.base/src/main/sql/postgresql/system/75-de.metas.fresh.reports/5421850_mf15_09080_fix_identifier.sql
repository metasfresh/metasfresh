
DROP FUNCTION IF EXISTS de_metas_endcustomer_fresh_reports.Docs_Generics_BPartner_Report ( IN Org_ID numeric, IN DocType text, IN BP_Loc_ID numeric, IN Record_ID numeric);
DROP TABLE IF EXISTS de_metas_endcustomer_fresh_reports.Docs_Generics_BPartner_Report;

CREATE TABLE de_metas_endcustomer_fresh_reports.Docs_Generics_BPartner_Report 
(
	Org_AddressLine text,
	AddressBlock text
);


CREATE FUNCTION de_metas_endcustomer_fresh_reports.Docs_Generics_BPartner_Report ( IN Org_ID numeric, IN DocType text, IN BP_Loc_ID numeric, IN Record_ID numeric )
RETURNS SETOF de_metas_endcustomer_fresh_reports.Docs_Generics_BPartner_Report AS
$$
SELECT
	(
		SELECT
			trim(
				COALESCE ( org_bp.name || ', ', '' ) ||
				COALESCE ( loc.address1 || ', ', '' ) ||
				COALESCE ( loc.postal || ' ', '' ) ||
				COALESCE ( loc.city, '' )
			)as org_addressline
		FROM
			c_bpartner org_bp
			JOIN c_bpartner_location org_bpl 	ON org_bp.c_bpartner_id	= org_bpl.c_bpartner_id
			JOIN c_location loc	 		ON org_bpl.c_location_id	= loc.c_location_id
		WHERE
			org_bp.ad_orgbp_id = $1
			and org_bpl.isbillto = 'Y'
		LIMIT 1
	)as org_addressline,

	CASE
		WHEN $2 = 'o' THEN o.BPartnerAddress
		WHEN $2 = 'o_delivery' THEN o.DeliveryToAddress
		WHEN $2 = 'io' THEN io.BPartnerAddress
		WHEN $2 = 'freshio' THEN freshio.BPartnerAddress
		WHEN $2 = 'i' THEN i.BPartnerAddress
		WHEN $2 = 'l' THEN tl.BPartnerAddress
		WHEN $3 IS NOT NULL THEN
			COALESCE(bp.name||E'\n', '') || COALESCE( bpl.address, '' )
		ELSE 'Incompatible Parameter!'
	END || E'\n' AS addressblock
FROM
	(SELECT '') x
	LEFT JOIN C_BPartner_Location bpl ON bpl.C_BPartner_Location_ID = $3
	LEFT JOIN C_BPartner bp	ON bp.C_BPartner_ID = bpl.C_BPartner_ID
	LEFT JOIN C_Greeting bpg ON bp.C_Greeting_id = bpg.C_Greeting_ID
	LEFT JOIN C_Location l ON bpl.C_Location_id = l.C_Location_ID
	LEFT JOIN C_Country lcou ON l.C_Country_id = lcou.C_Country_ID
	LEFT JOIN C_Region r ON l.C_Region_id = r.C_Region_ID

	LEFT JOIN C_Order o ON o.C_Order_id = $4
	LEFT JOIN C_Invoice i ON i.C_Invoice_id = $4
	LEFT JOIN T_Letter_Spool tl ON tl.AD_Pinstance_ID = $4
	LEFT JOIN M_InOut io ON io.M_InOut_ID = $4

	-- fresh specific: Retrieve an address for all receipt lines linked to an order line
	-- Note: This approach is used for purchase transactions only. Sales transactions are retrieved like always
	-- Note: Empties Receipts are also work the regular way
	LEFT JOIN C_Orderline ol ON ol.C_OrderLine_ID = $4
	-- Retrieve 1 (random) in out linked to the given order line
	-- We assume that the the BPartner address is not changed in between. (backed with pomo)
	LEFT JOIN (
		SELECT 	rs.Record_ID, Max(iol.M_InOut_ID) AS M_InOut_ID
		FROM 	M_ReceiptSchedule rs
			JOIN M_ReceiptSchedule_Alloc rsa ON rs.M_ReceiptSchedule_ID = rsa.M_ReceiptSchedule_ID
			JOIN M_InOutLine iol ON rsa.M_InOutLine_ID = iol.M_InOutLine_ID
		WHERE	AD_Table_ID=(SELECT AD_Table_ID FROM AD_Table WHERE TableName = 'C_OrderLine')
		GROUP BY	rs.Record_ID
	) io_id ON io_id.Record_ID = ol.C_OrderLine_ID
	LEFT JOIN M_InOut freshio 		ON io_id.M_InOut_ID = freshio.M_InOut_ID
$$
LANGUAGE sql STABLE
;