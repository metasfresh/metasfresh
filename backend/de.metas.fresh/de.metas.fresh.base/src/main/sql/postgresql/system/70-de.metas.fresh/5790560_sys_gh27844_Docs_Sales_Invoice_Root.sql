DROP FUNCTION IF EXISTS de_metas_endcustomer_fresh_reports.Docs_Sales_Invoice_Root (IN p_Record_ID numeric,
                                                                                    IN p_Language  Character Varying(6))
;

CREATE OR REPLACE FUNCTION de_metas_endcustomer_fresh_reports.Docs_Sales_Invoice_Root(IN p_Record_ID numeric,
                                                                                      IN p_Language  Character Varying(6))
    RETURNS TABLE
            (
                AD_Org_ID        numeric,
...
FROM C_Invoice i
         ...
         LEFT OUTER JOIN C_DocType_Trl dtt ON i.C_DocType_ID = dtt.C_DocType_ID AND dtt.AD_Language = p_Language
