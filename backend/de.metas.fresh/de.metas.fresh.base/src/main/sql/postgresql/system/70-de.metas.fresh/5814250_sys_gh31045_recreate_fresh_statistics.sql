-- Recreates report.fresh_statistics(9 args) from the DDL source, since the earlier
-- migration that added C_BPartner_ID to the backing tables (5789640) dropped the
-- three fresh_product_statistics_* functions and only recreated the tables — leaving
-- Artikelstatistik (Excel) and the fresh_statistics-based Jasper reports broken until
-- the DDL-recreate phase reruns on the affected instance.
...
COMMENT ON FUNCTION report.fresh_statistics(numeric, character varying, numeric, numeric, numeric, numeric, numeric, numeric, character varying) IS 'Making this function volatile is currently our only known way to avoid
