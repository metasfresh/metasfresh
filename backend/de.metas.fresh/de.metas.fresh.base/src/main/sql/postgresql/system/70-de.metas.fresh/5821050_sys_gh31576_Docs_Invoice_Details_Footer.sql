...
SELECT i.descriptionbottom,
       ...
       (CASE
            WHEN i.PaymentTermText != '@paymentterm@' THEN i.PaymentTermText
                                                          ELSE
                                                              REPLACE(
                                                                      REPLACE(
                                                                              REPLACE(
       ...
       i.incotermlocation,
       CASE
           WHEN report.IsHiddenReportElement(i.C_DocType_ID, 'DeliveryRule') = 'N' THEN
               COALESCE(o_dr_trl.name, o_dr.name)
       END                                                                                            AS deliveryrule,
       CASE
