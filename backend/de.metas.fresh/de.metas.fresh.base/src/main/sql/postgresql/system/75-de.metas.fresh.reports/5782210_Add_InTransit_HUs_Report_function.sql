DROP FUNCTION IF EXISTS report.InTransit_HUs_Report(IN p_M_Locator_ID numeric,
...
SELECT COALESCE(pt.Name, p.name) AS ProductName,
       ...
       CASE
           WHEN ddom.qtypicked > 0 THEN ddom.qtypicked
...
FROM M_HU hu
    ...
    -- Join DD Order data via DD_OrderLine_HU_Candidate (as per requirement)
         INNER JOIN DD_OrderLine_HU_Candidate ddolhc ON ddolhc.PickFrom_HU_ID = hu.M_HU_ID
    ...
    -- Product information
         ...
         INNER JOIN M_Product_Trl pt
                    ON ddol.M_Product_ID = pt.M_Product_ID AND pt.AD_Language = p_AD_Language AND pt.isActive = 'Y'
    ...
    -- QR Code information
         ...
         LEFT JOIN M_HU_QRCode qr ON qr_assign.m_hu_qrcode_id = qr.m_hu_qrcode_id
