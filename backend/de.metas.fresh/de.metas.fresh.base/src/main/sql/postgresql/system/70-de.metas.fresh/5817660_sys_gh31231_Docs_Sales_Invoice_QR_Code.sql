-- Source DDL: backend/de.metas.fresh/de.metas.fresh.base/src/main/sql/postgresql/ddl/functions/Docs_Sales_Invoice_QR_Code.sql
...
--      causing the outer COALESCE to short-circuit and never reach the IBAN branch.
        COALESCE(
            ...
            -- CR block (Creditor = org/company) — structured address (type S, mandatory from 2026-11-14)
        'S'                                                              || E'\n' || -- CR AddressType = Structured
        orgbp.name                                                       || E'\n' || -- CR Name
...
--      FM9999999.00 suppresses leading spaces but keeps the integer part.
        ...
        cur.iso_code || E'\n' ||
            ...
            -- UD block (Ultimate Debtor = bill recipient / customer) — structured address (type S)
        'S' || E'\n' || -- UD AddressType = Structured

-- UD Name: the partner's real name (company name, or first + last name for a person),
--          mirroring Docs_Sales_Dunning_QR_Code. The former bpartneraddress-first-line parse
--          emitted only the salutation (e.g. "Herr") when the address block led with one.
        (CASE
             WHEN bp.IsCompany = 'Y' THEN bp.CompanyName
             ELSE btrim(COALESCE(u.FirstName, '') || ' ' || COALESCE(u.LastName, ''))
         END)                                                            || E'\n' || -- UD Name
