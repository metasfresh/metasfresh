...
RETURNS TABLE
	...
	-- Breaks a tie between two rows landing on the same "line" AND the same istextline (in practice:
	-- two C_Doc_TextLine rows at one position -- two article rows can never tie here, C_OrderLine.Line
	-- is distinct per order). Carries each row's own id: the text branch's C_Doc_TextLine_ID (matching
	-- DocTextLineRepository.getByDocument's own "ORDER BY Line, C_Doc_TextLine_ID", so a tie prints in
	-- the same order the WebUI text-lines modal shows), the article branch's C_OrderLine_ID. The
	-- article value is never actually compared against a text row's: istextline ('N' vs 'Y') already
	-- differs for that pair, so ORDER BY never reaches this third key for an article/text comparison --
	-- a real id is used anyway (never NULL) so the column carries a meaningful value in both branches
	-- rather than relying on that reasoning to keep a NULL harmless.
...
text_runs AS (
	...
	FROM target_report tr
		...
		-- The next article line AT OR AFTER tl (>=, not >), anywhere in the order (ALL active order
		-- lines -- packaging material and any DocStatus included: the run is about the order's own
		-- physical line sequence, not about which lines this particular report happens to carry).
		-- Inclusive because the ORDER BY tiebreak below renders a text line BEFORE an article line
		-- tied at the same Line -- so a tied article ends this block rather than being skipped over,
		-- and the "block" step folds it back in as the block's own last member (its own bound is
		-- t.Line <= next_article_line, not <). A strict ">" here would treat the tied article as not
		-- separating the block at all, letting a later text line silently absorb it into the wrong
		-- block -- the same tie the run bounds below already treat as inclusive.
		CROSS JOIN LATERAL (
		...
		-- tl -- i.e. the furthest text line reachable from tl with no article line strictly between
		-- them. <= next_article_line (not <): a text line tied with the boundary article still belongs
		-- to this block, matching the tiebreak that renders it immediately before that article.
		CROSS JOIN LATERAL (
...
SELECT
	...
	'N' AS IsTextLine,
	article_rows.C_OrderLine_ID AS LineTieBreakId
FROM article_rows
...
SELECT
	...
	'Y' AS IsTextLine,
	tr_run.C_Doc_TextLine_ID AS LineTieBreakId
...
-- tiebreak, and is therefore part of that text line's run). LineTieBreakId is the third key, for the
-- one case the first two leave undecided: two text rows tied at the same line (see the RETURNS TABLE
-- comment on that column for why the article branch's own value never factors into that comparison).
