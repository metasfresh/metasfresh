...
FROM
	...
	INNER JOIN (
		SELECT
			...
			COALESCE(lhuas.weight, 0) AS qtyLU
		FROM
			C_OrderLine ol
			INNER JOIN M_ReceiptSchedule rs ON rs.AD_Table_ID = ( SELECT Get_Table_ID('C_OrderLine') ) AND rs.Record_ID = ol.C_OrderLine_ID AND rs.isActive = 'Y'
			INNER JOIN M_ReceiptSchedule_Alloc rsa ON rs.M_ReceiptSchedule_ID = rsa.M_ReceiptSchedule_ID AND rsa.isActive = 'Y'
			...
			LEFT OUTER JOIN 
			(
				SELECT lhuas.Record_ID, lhuas.isActive, lhuas.m_hu_id, lhuas.AD_Table_ID, lhuas.M_HU_Assignment_id, lhuas.M_LU_HU_ID, lp.weight
				FROM M_HU_Assignment lhuas 
				INNER JOIN M_HU lhu ON lhuas.M_LU_HU_ID = lhu.M_HU_ID
				INNER JOIN M_HU_Item lhui ON lhu.m_hu_id = lhui.m_hu_id AND lhui.isActive = 'Y'
				INNER JOIN M_HU_PI_Item lhupi ON lhui.M_HU_PI_Item_ID = lhupi.M_HU_PI_Item_ID and lhupi.itemtype='PM' AND lhupi.isActive = 'Y'
				INNER JOIN M_HU_PackingMaterial lpm ON lhupi.M_HU_PackingMaterial_ID = lpm.M_HU_PackingMaterial_ID AND lpm.isActive = 'Y'
				INNER JOIN M_Product lp ON lpm.M_Product_ID = lp.M_Product_ID AND lp.isActive = 'Y'
			) lhuas ON iol.M_InOutLine_ID = lhuas.Record_ID AND lhuas.isActive = 'Y' and lhuas.m_hu_ID =  thuas.m_hu_ID
								AND lhuas.AD_Table_ID = ((SELECT Get_Table_ID('M_InOutLine'))) and lhuas.M_HU_Assignment_id != thuas.M_HU_Assignment_id AND lhuas.M_LU_HU_ID is not null
