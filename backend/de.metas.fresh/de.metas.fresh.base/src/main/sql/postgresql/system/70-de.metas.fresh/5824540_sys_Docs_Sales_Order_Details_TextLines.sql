...
-- exclude here.)
-- Column shape: only the document-identity columns (order_description, cursymbol, iso_code -- all
-- derived from the order/currency, not from an article) are carried through; every column whose
-- purpose is describing/pricing/taxing an ARTICLE (product, HU, qty, price, discount, tax
-- rate/printing, compensation group, weight, the *Pattern formatting helpers) is NULL. TextLine is
-- nullable and an empty text line is legal (prints as a blank line) -- it is passed straight
-- through with no NULLIF/COALESCE that could coerce an empty value away.
SELECT tl.line                                                AS line,
       ...
       NULL::character(1)                                      AS isDiscountPrinted,
       ...
       NULL::text                                              AS PricePattern,
       NULL::text                                              AS AmountPattern,
