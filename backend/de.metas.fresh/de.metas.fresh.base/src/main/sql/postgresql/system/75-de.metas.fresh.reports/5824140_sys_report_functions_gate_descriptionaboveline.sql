-- Gate the free-text-above-line column behind the per-doctype report-element suppression, on the
-- order confirmation and the invoice.
--
-- AD_Ref_List 544365 makes 'descriptionaboveline' selectable in C_DocType_ReportElement, so a user
-- can switch the element off per document type. Until now only the delivery note honoured that
-- switch (Docs_Sales_InOut_Details), which meant a configured suppression silently did nothing on
-- the other documents. Both functions changed here already use report.IsHiddenReportElement for
-- their own elements -- 'p_description' / 'Net_Sum' / 'Tax_Rate' in the order function,
-- 'QtyInvoicedInPriceUOM' / 'UOMSymbol' / 'TotalAmount' / 'Weight' in the invoice one -- so this
-- follows each file's own established idiom, with the doctype column each already uses
-- (o.C_DocTypeTarget_ID for the order, i.C_DocType_ID for the invoice).
--
-- Deliberately NOT changed: the four _HU variants and Docs_Sales_Picking_Details use the mechanism
-- nowhere, and in three of the _HU functions the column is a grouping key, where wrapping it would
-- change the grouping rather than just the projection.
--
-- Bodies are byte-identical to the reference definitions in
...
SELECT io.DocType || ': ' || io.DocNo                         AS InOuts,
       ...
       CASE
           WHEN report.IsHiddenReportElement(i.C_DocType_ID, 'descriptionaboveline') = 'N' THEN
               ol.DescriptionAboveLine
       END                                                    AS descriptionaboveline
