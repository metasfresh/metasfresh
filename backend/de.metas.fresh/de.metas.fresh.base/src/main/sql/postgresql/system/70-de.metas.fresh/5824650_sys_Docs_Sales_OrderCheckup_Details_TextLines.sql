...
-- The record's OWN line set: the order lines this specific record actually carries -- same predicates
-- as the article branch below (active report line, active order, completed sales order, not packaging
-- material). Per task-16-controller-notes.md hazard #3, the carry-rule test must intersect against THIS
-- (the record's own lines), not against the order's full line set.
record_lines AS (
	SELECT ol.line
	FROM target_report tr
		INNER JOIN C_Order o ON o.C_Order_ID = tr.C_Order_ID AND o.isActive = 'Y'
		INNER JOIN C_Order_MFGWarehouse_ReportLine reportLine
			ON reportLine.C_Order_MFGWarehouse_Report_ID = tr.C_Order_MFGWarehouse_Report_ID AND reportLine.IsActive = 'Y'
...
--
-- Article lines (unchanged from the pre-text-line function; two NULL columns appended for the new
-- text/istextline output shape).
SELECT
	ol.line,
	att.Attributes,
...
-- head when none of its run is present here -- AC21). Column shape follows the same document-level vs.
-- article-level split worked out for the sibling sales-order report function (Docs_Sales_Order_Details,
-- commits 7500ca36784/95ec724008e): report/order/warehouse/plant/partner/date/document-type identity
-- columns are carried through unchanged (true of a text row exactly as of an article row on the same
-- record); every column that describes an ARTICLE (product, attributes, price, capacity, quantities,
-- container, UOM, the order-LINE id, and the report-LINE id + its barcode -- both reached only through
-- C_Order_MFGWarehouse_ReportLine, which a text line has no row in) is NULL.
SELECT
	CASE
		WHEN tr_run.TextLineScope = 'D' AND NOT EXISTS (
			...
			-- AC21: a whole-document-scoped line whose run is absent from this record still prints, at
			-- the head. The offset is a constant well below any real order-line Line value, so relative
			-- order among several such lines (if that ever happens) still follows their own Line.
