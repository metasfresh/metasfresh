...
class ExternalReferenceRestControllerServiceTest
{
	...
	void performLookup()
	{
		...
		assertThat(records)
				.extracting(COLUMNNAME_AD_Org_ID, COLUMNNAME_ExternalSystem_ID, COLUMNNAME_ExternalReference, COLUMNNAME_Type, COLUMNNAME_Record_ID)
				.containsExactly(
						tuple(orgId.getRepoId(), externalSystem.getId().getRepoId(), "id1", "bpartner", 25),
						tuple(orgId.getRepoId(), externalSystem.getId().getRepoId(), "id2", "bpartner", 26));
		...
		assertThat(items.getFirst().getLookupItem()).isEqualTo(lookupItem1);
		assertThat(items.getFirst().getMetasfreshId()).isNotNull();
		assertThat(items.getFirst().getMetasfreshId().getValue()).isEqualTo(25);
		assertThat(items.getFirst().getExternalReference()).isEqualTo("id1");
	}
}