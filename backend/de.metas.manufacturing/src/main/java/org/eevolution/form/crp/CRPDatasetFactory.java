...
public class CRPDatasetFactory extends CRPReasoner implements CRPModel
{
	...
	public static CRPModel get(final Instant start, final Instant end, final ResourceId resourceId)
	{
	...
	private void generate(final Instant start, final Instant end, final ResourceId resourceId)
	{
		...
		while (end.isAfter(dateTime))
		{
			...
			dateTime = dateTime.plus(1, ChronoUnit.DAYS); // TODO: teo_sarca: increment should be more general, not only days
		}
	...
	@Override
	public Duration calculateLoad(final Instant dateTime, final ResourceId resourceId)
	{
		final ResourceType resourceType = getResourceType(resourceId);
	 ...
	 */
	private static Instant[] getDayBorders(
			...
			final ResourceType resourceType)
	{
		...
		Instant endDayTime = resourceType.getDayEnd(dateTime);
		...
		// the resource type's time slot value
		Instant startDayTime = resourceType.getDayStart(dateTime);
		...
		return new Instant[] { startDayTime, endDayTime };
	}

	private Duration calculateDurationForDay(final Instant dateTime, final PPOrderRoutingActivitySchedule activity, final ResourceType resourceType)
	{
		final Instant[] borders = getDayBorders(dateTime, activity, resourceType);
	 ...
	 * @return
	 */
	private Map<DefaultMutableTreeNode, String> addTreeNodes(final Instant dateTime, final DefaultMutableTreeNode root, final I_S_Resource resource)
	{
	...
	private String getTreeNodeRepresentation(final Instant dateTime, final DefaultMutableTreeNode node, final I_S_Resource resource)
	{
		...
		else if (TimeUtil.isDateOrTimeObject(nodeData))
		{
			final Instant d = TimeUtil.asInstant(nodeData);
		...
		else if (nodeData instanceof PPOrderRoutingActivitySchedule)
		{
			...
			final Instant[] interval = getDayBorders(dateTime, activity, resourceType);
			name = interval[0].atZone(SystemTime.zoneId()).format(timeFormatter)
					+ " - "
					+ interval[1].atZone(SystemTime.zoneId()).format(timeFormatter)
			// + " " + activity.getName()
