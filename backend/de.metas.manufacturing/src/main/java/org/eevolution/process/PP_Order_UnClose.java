...
public class PP_Order_UnClose extends JavaProcess implements IProcessPrecondition
{
	...
	private void reverseCostCollectorsGeneratedOnClose(final PPOrderId ppOrderId)
	{
		...
		for (final I_PP_Cost_Collector cc : costCollectors)
		{
			...
			// Only UsageVariance collectors are generated by closeIt() itself, so only those are reversed here.
			// A CostDifferenceDistribution collector is created by the separate "Distribute" action on a
			// completed-but-not-closed order (independent of any Close event) and is intentionally left
			// untouched on unclose - same as MaterialReceipt/ComponentIssue collectors below (no-op).
