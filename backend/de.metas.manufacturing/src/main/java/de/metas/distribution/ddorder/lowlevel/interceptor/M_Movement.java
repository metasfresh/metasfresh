...
class M_Movement
{
	...
	M_Movement(final DDOrderLowLevelService ddOrderLowLevelService) {this.ddOrderLowLevelService = ddOrderLowLevelService;}
	...
	public void afterComplete(final I_M_Movement movement)
	{
		if (movement.getDD_Order_ID() > 0)
		{
			...
			ddOrderLowLevelService.save(order);
		}
		...
		for (final I_M_MovementLine movementLine : movementLines)
		{
			...
			final boolean isMovementReceipt = ddOrderLowLevelService.isMovementReceipt(movementLine);
			...
			ddOrderLowLevelService.save(ddOrderLineOrAlt);
		}
	}
