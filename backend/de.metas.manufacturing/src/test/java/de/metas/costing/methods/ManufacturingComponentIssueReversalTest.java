...
{
	private static final Instant DATE = Instant.parse("2026-08-27T00:00:00Z");

	/** the component's current cost, i.e. the price the issue is valued at */
	...
	private static final String ISSUED_COST = "100"; // = 10 EUR/Ea x 10 Ea
	...
	void componentIssue_accumulatesTheIssuedCostAsAPositiveInboundAmount(final ManufacturingHandlerUnderTest handlerUnderTest)
	{
		setupOrderFor(handlerUnderTest);

		handler.createOrUpdateCost(issueRequest());

		final PPOrderCost materialIssueCost = materialIssueCost();
		// the cost that went INTO the order is accumulated positive, while the qty keeps the movement direction
		assertThat(materialIssueCost.getAccumulatedAmount().toBigDecimal()).isEqualByComparingTo(ISSUED_COST);
		assertThat(materialIssueCost.getAccumulatedQty().toBigDecimal()).isEqualByComparingTo(ISSUED_QTY);
	}

	@ParameterizedTest
	@EnumSource(ManufacturingHandlerUnderTest.class)
	void componentIssue_thenReversal_netsAccumulatedAmountBackToZero(final ManufacturingHandlerUnderTest handlerUnderTest)
	{
		setupOrderFor(handlerUnderTest);

		final CostDetailCreateRequest issueRequest = issueRequest();
		handler.createOrUpdateCost(issueRequest);
		handler.createOrUpdateCost(reversalRequestOf(issueRequest));

		final PPOrderCost materialIssueCost = materialIssueCost();
		assertThat(materialIssueCost.getAccumulatedAmount().toBigDecimal()).isEqualByComparingTo(BigDecimal.ZERO);
		assertThat(materialIssueCost.getAccumulatedQty().toBigDecimal()).isEqualByComparingTo(BigDecimal.ZERO);
	}
	...
	private void setupOrderFor(@NonNull final ManufacturingHandlerUnderTest handlerUnderTest)
	{
		...
		issueCollectorId = createComponentIssueCollector();
		reversalCollectorId = createComponentIssueCollector();

		saveCurrentCost(componentProductId, COMPONENT_COST_PRICE, "100");
		createOrderCosts();
	}

	private PPCostCollectorId createComponentIssueCollector()
	{
		...
		cc.setMovementQty(ISSUED_QTY.negate());
	...
	private void createOrderCosts()
	{
		...
		final PPOrderCost mainProduct = PPOrderCost.builder()
				...
				.costSegmentAndElement(utils.extractCostSegmentAndElement(issueRequest.withProductId(mainProductId)))
				.price(costPrice("0"))
				.accumulatedAmount(CostAmount.zero(currencyId))
				.accumulatedQty(Quantity.zero(uomEach))
				.build();
	...
	private CostDetailCreateRequest issueRequest()
	{
		return CostDetailCreateRequest.builder()
				...
				.documentRef(CostingDocumentRef.ofCostCollectorId(issueCollectorId))
				.qty(Quantity.of(ISSUED_QTY, uomEach))
				.amt(CostAmount.zero(currencyId)) // N/A - the handler values the issue at the component's current cost
				.date(DATE)
				.build();
	}

	/**
	 * What {@code CostingService.toCostDetailCreateRequestFromReversalRequest} hands the handler when the
	 * collector is reversed: the stored cost detail of the initial document, negated.
	 */
	private CostDetailCreateRequest reversalRequestOf(@NonNull final CostDetailCreateRequest issueRequest)
	{
		final List<CostDetail> forwardCostDetails = utils.getExistingCostDetails(issueRequest);
		assertThat(forwardCostDetails).hasSize(1);
		final CostDetail forwardCostDetail = forwardCostDetails.get(0);

		return issueRequest.toBuilder()
				.documentRef(CostingDocumentRef.ofCostCollectorId(reversalCollectorId))
				.initialDocumentRef(issueRequest.getDocumentRef())
				.amtType(forwardCostDetail.getAmtType())
				.amt(forwardCostDetail.getAmt().negate())
				.qty(forwardCostDetail.getQty().negate())
				.build();
	}

	private PPOrderCost materialIssueCost()
	{
		final List<PPOrderCost> costs = Services.get(IPPOrderCostBL.class)
				.getByOrderId(orderId)
				.getByProductAndCostElements(componentProductId, ImmutableSet.of(costElement.getId()));
