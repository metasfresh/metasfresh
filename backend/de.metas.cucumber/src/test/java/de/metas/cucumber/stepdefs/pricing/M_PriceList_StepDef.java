...
public class M_PriceList_StepDef
{
	...
	/**
	 * Same as "metasfresh contains M_PriceLists" but does a GLOBAL lookup (ignoring pricing system) to find an
	 * existing pricelist matching the given country + IsSOPriceList. This is needed because the DB constraint
	 * {@code m_pricelist_uc_c_country} enforces UNIQUE(C_Country_ID, IsSOPriceList) globally — i.e. across all
	 * pricing systems. If an earlier test in the same DB created a pricelist for "no country" + IsSOPriceList=Y,
	 * a second test cannot create another one even under a different pricing system.
	 * <p>
	 * When an existing pricelist is found, the row's M_PricingSystem identifier is re-pointed to the existing
	 * pricelist's pricing system so that subsequent steps (e.g. M_PriceList_Versions, M_ProductPrices) hang off
	 * the right pricing system.
	 * <p>
	 * Example:
	 * <pre>
	 * And metasfresh contains or reuses global M_PriceList
	 *   | Identifier | M_PricingSystem_ID | C_Currency.ISO_Code | SOTrx | IsTaxIncluded | PricePrecision |
	 *   | pl_bsp     | ps_bsp             | EUR                 | true  | false         | 2              |
	 * </pre>
	 *
	 * Required columns: Identifier, M_PricingSystem_ID, C_Currency.ISO_Code (or C_Currency_ID.Identifier), SOTrx.<br>
	 * Optional columns: Name, IsTaxIncluded, PricePrecision, IsActive, C_Country_ID, C_Country.CountryCode, Description, AD_Org_ID.
	 */
	@And("metasfresh contains or reuses global M_PriceList")
	public void add_or_reuse_global_M_PriceList(@NonNull final DataTable dataTable)
	{
		DataTableRows.of(dataTable).forEach(this::findOrCreateGlobalM_PriceList);
	}

	private void findOrCreateGlobalM_PriceList(@NonNull final DataTableRow row)
	{
		...
		final int countryRepoId = country != null ? country.getC_Country_ID() : -1;

		// Global lookup: find any existing pricelist with the given country + IsSOPriceList — across all pricing systems.
		// Do NOT add IsActive filter — the DB unique constraint applies regardless of IsActive.
		I_M_PriceList priceList = queryBL.createQueryBuilder(I_M_PriceList.class)
				.addEqualsFilter(I_M_PriceList.COLUMNNAME_IsSOPriceList, soTrx.toBoolean())
				.addEqualsFilter(I_M_PriceList.COLUMNNAME_C_Country_ID, countryRepoId)
				.orderBy(I_M_PriceList.COLUMNNAME_M_PriceList_ID)
				.create()
				.first(I_M_PriceList.class);

		if (priceList != null)
		{
			// Re-point the row's pricing-system identifier to whatever pricing system the existing pricelist belongs to,
			// so that downstream steps (M_PriceList_Versions, M_ProductPrices) attach to the correct PricingSystem.
			final I_M_PricingSystem existingPs = InterfaceWrapperHelper.load(priceList.getM_PricingSystem_ID(), I_M_PricingSystem.class);
			row.getAsIdentifier(COLUMNNAME_M_PricingSystem_ID).putOrReplace(pricingSystemTable, existingPs);
		}
		else
		{
			...
			priceList.setC_Country_ID(countryRepoId);
