...
public class MD_Stock_PerWeek_StepDef
{
	...
	// ---------------------------------------------------------------------------
	// gh#30457 AC1 — MD_Stock_PerWeek_fn(product,warehouse) vs MD_Stock_PerWeek_V equivalence
	// ---------------------------------------------------------------------------

	/** All columns of both {@code MD_Stock_PerWeek_fn} and {@code MD_Stock_PerWeek_V}, in the same order. */
	private static final String STOCK_PER_WEEK_COLUMNS =
			"AD_Client_ID, AD_Org_ID, M_Product_ID, M_Warehouse_ID, WeekStartDate,"
					+ " QtyATPBegin, QtyExpectedShipments, QtyExpectedReceipts, QtyATP";

	/**
	 * Asserts that {@code MD_Stock_PerWeek_fn(product,warehouse)} returns exactly the same rows as
	 * {@code MD_Stock_PerWeek_V} filtered to that product/warehouse (set-equality, both {@code EXCEPT}
	 * directions empty) — gh#30457 AC1 "byte-identical" requirement.
	 *
	 * @cucumber.columns
	 *   <b>M_Product_ID</b> — (required, identifier-ref) product<br>
	 *   <b>M_Warehouse_ID</b> — (required, identifier-ref) warehouse<br>
	 */
	@Then("^MD_Stock_PerWeek_fn output equals MD_Stock_PerWeek_V for:$")
	public void assert_fn_output_equals_view(@NonNull final DataTable dataTable)
	{
		DataTableRows.of(dataTable).forEach(this::assertFnOutputEqualsViewRow);
	}

	private void assertFnOutputEqualsViewRow(@NonNull final DataTableRow row)
	{
		final I_M_Product productRecord = row.getAsIdentifier(I_MD_Stock_PerWeek_V.COLUMNNAME_M_Product_ID).lookupNotNullIn(productTable);
		final I_M_Warehouse warehouseRecord = row.getAsIdentifier(I_MD_Stock_PerWeek_V.COLUMNNAME_M_Warehouse_ID).lookupNotNullIn(warehouseTable);
		...
		final int fnMinusView = DB.getSQLValueEx(ITrx.TRXNAME_None,
				"SELECT COUNT(*) FROM ("
						+ "SELECT " + STOCK_PER_WEEK_COLUMNS + " FROM MD_Stock_PerWeek_fn(?, ?)"
						+ " EXCEPT "
						+ "SELECT " + STOCK_PER_WEEK_COLUMNS + " FROM MD_Stock_PerWeek_V WHERE M_Product_ID = ? AND M_Warehouse_ID = ?"
						+ ") fn_minus_view",
				productId, warehouseId, productId, warehouseId);

		final int viewMinusFn = DB.getSQLValueEx(ITrx.TRXNAME_None,
				"SELECT COUNT(*) FROM ("
						+ "SELECT " + STOCK_PER_WEEK_COLUMNS + " FROM MD_Stock_PerWeek_V WHERE M_Product_ID = ? AND M_Warehouse_ID = ?"
						+ " EXCEPT "
						+ "SELECT " + STOCK_PER_WEEK_COLUMNS + " FROM MD_Stock_PerWeek_fn(?, ?)"
		...
		final SoftAssertions softly = new SoftAssertions();
		softly.assertThat(fnMinusView)
				.as("Rows returned by MD_Stock_PerWeek_fn(%d,%d) but missing from MD_Stock_PerWeek_V — function diverges from the view",
						productId, warehouseId)
				.isZero();
		softly.assertThat(viewMinusFn)
				.as("Rows present in MD_Stock_PerWeek_V but missing from MD_Stock_PerWeek_fn(%d,%d) — function diverges from the view",
						productId, warehouseId)
				.isZero();
		softly.assertAll();
	}
