...
public class M_Product_ASI_Data_StepDef
{
	...
	@NonNull private final M_Product_ASI_Data_StepDefData asiDataTable;

	/**
	 * Creates {@code M_Product_ASI_Data} records.
	 * <p>
	 * Example:
	 * <pre>{@code
	 ...
	 *
	 * <ul>
	 * <li>{@code Identifier} — (optional) step def identifier for cross-step references</li>
	...
	private void createRecord(@NonNull final DataTableRow row)
	{
		...
		row.getAsOptionalIdentifier(I_M_Product_ASI_Data.COLUMNNAME_C_BPartner_ID)
				.ifPresent(id -> {
					final BPartnerId bPartnerId = bPartnerTable.getIdOptional(id)
							.orElseGet(() -> id.getAsId(BPartnerId.class));
