...
public class AD_Sequence_StepDef
{
	...
	@Given("^set AD_Sequence.CurrentNext=(.*) for sequence (.*)$")
	public void setAD_SequenceCurrentNext(@NonNull final String currentNextStr, @NonNull final String sequenceName)
	{
		final I_AD_Sequence sequenceRecord = sequenceDAO.retrieveSequenceByName(sequenceName);
		assertThat(sequenceRecord).isNotNull();
		
		sequenceRecord.setCurrentNext(Integer.parseInt(currentNextStr));
		
		InterfaceWrapperHelper.saveRecord(sequenceRecord);
	}
}
