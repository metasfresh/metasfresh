...
Feature: invoice payment allocation
  ...
  Background:
    ...
    And metasfresh contains C_BP_BankAccount
      ...
      | serviceProviderBankAccount | EUR           |
...
# ############################################################################################################################################
  ...
  Scenario: sales-creditmemo - service-fee purchase-invoice - inbound payment (REMADV case)
    ...
    And metasfresh contains C_RemittanceAdvice
      | Identifier | Source_BP_BankAccount_ID   | Destination_BP_BankAccount_ID | DocumentNo | DateDoc    | RemittanceAmt |
      | ra1        | serviceProviderBankAccount | orgBankAccount                | S0466_100  | 2025-05-07 | 1399.91 EUR   |
    And metasfresh contains C_RemittanceAdvice_Lines
      | Identifier | C_RemittanceAdvice_ID | LineIdentifier | C_Invoice_ID       | RemittanceAmt | ServiceFeeAmount | PaymentDiscountAmt |
      | ral1       | ra1                   | 10             | customerCreditMemo | 434.97        | 6.46 EUR         | -6.07 EUR          |
      | ral2       | ra1                   | 20             | vendorInvoice      | 434.97        | 6.46 EUR         | -6.07 EUR          |
      | ral3       | ra1                   | 30             | customerInvoice    | 529.97        | 32.10 EUR        | 32.93 EUR          |
    ...
    And validate created invoices
      ...
      | vendorServiceInvoice | serviceProvider | 6.46 EUR   | API         | true   | false           | 0       |
    ...
    And Fact_Acct records are matching
      ...
      | *                      |             |             |                 |          | customerCreditMemo   |
      # ------------------------------------------------------------------------------------------
      | V_Liability_Acct       |             | 6.46 EUR    | serviceProvider | -        | vendorServiceInvoice |
