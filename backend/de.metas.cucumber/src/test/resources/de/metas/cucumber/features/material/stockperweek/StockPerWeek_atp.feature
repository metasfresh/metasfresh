...
Feature: MD_Stock_PerWeek_V shows cumulative projected stock (QtyATP) and rolls overdue demand into the current week
  ...
  @allure.label.epic:E0155_Material_Disposition
  @allure.label.feature:F19100
  Scenario: QtyATP sums across attribute subgroups, taking the latest STOCK candidate per subgroup
    ...
    # week-end boundary, then SUMs across subgroups.
    #
    # Subgroup A (StorageAttributesKey=attrA): two STOCK candidates in week 0.
    #   - earlier (DayWithinWeek=1, ATP=30): DateProjected = Monday+1; validity interval ends
    ...
    # Subgroup B (StorageAttributesKey=attrB): one STOCK candidate in week 0 (ATP=20)
    #   → contributes 20.
    ...
    And metasfresh initially has this MD_Candidate data relative to current week
      | Identifier                  | MD_Candidate_Type | MD_Candidate_BusinessCase | M_Product_ID        | WeekOffset | DayWithinWeek | Qty | ATP | M_Warehouse_ID | StorageAttributesKey |
      | inv_S30457_attrA_early      | INVENTORY_UP      |                           | product_S30457_atp3 | 0          | 1             | 30  | 30  | wh_S25618_atp  | attrA                |
      | inv_S30457_attrA_late       | INVENTORY_UP      |                           | product_S30457_atp3 | 0          | 2             | 40  | 40  | wh_S25618_atp  | attrA                |
      | inv_S30457_attrB            | INVENTORY_UP      |                           | product_S30457_atp3 | 0          | 1             | 20  | 20  | wh_S25618_atp  | attrB                |

    # Week 0: view sums the latest STOCK per subgroup.
    #   attrA latest (DayWithinWeek=2) → ATP=40; attrB (DayWithinWeek=1, only entry) → ATP=20.
    #   QtyATP = 40 + 20 = 60.
