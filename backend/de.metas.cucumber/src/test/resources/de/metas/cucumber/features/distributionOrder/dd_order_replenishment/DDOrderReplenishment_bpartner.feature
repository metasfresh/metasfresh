...
Feature: DD_Order replenishment — a consolidated distribution order names no single customer
  As a mover replenishing a packing workplace,
  I want the consolidated distribution order not to be labelled with one of the customers it serves,
  so that neither the document nor its print-out claims to belong to a delivery it only partly covers.
  ...
  @from:cucumber
  Scenario: The order consolidating two customers' demand names neither customer as its owner
