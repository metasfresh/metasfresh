...
## F00751: E-Invoicing
  ...
  Background:
    ...
    # The sales invoice runs under the main org (1000000). The AD_MailConfig.AD_Org_ID is non-updateable
    # and is forced to the login/context org on save, so the invoice's org must equal that org for the
    # mail routing to match — hence the seller is the main org's own org-bpartner, made BR-DE-conformant.
    ...
    And metasfresh contains C_BPartners without locations:
      | Identifier    | Value | Name        | CompanyName | M_PricingSystem_ID | TaxID       |
      | seller_org_bp | dt    | Muster GmbH | Muster GmbH | ps_1               | DE123456789 |
