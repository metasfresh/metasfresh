...
## F1500: Costing
  ...
  Scenario: Co-products' percent sum exceeding 100% is rejected before any amount is carved
    # Randstuecke is given an impossible 120% share of the pool. The order completes and its cost
    # collectors get created normally (the percent-space guard fires at POST-CALC, which runs when each
    # collector is COSTED/POSTED, not at document completion) - but posting the co-product receipt is
    # rejected loudly, naming the offending product(s) and the sum, before any amount is carved from the
    # pool - so no negative finished-good amount is ever persisted.
    And update M_Product:
      | M_Product_ID.Identifier | CoProductCostDistributionPercent |
      | coProd                  | 120                              |
    ...
    And PP_Order_Cost are found:
      | PP_Order_ID.Identifier | M_Product_ID.Identifier | M_CostElement_ID | PP_Order_Cost_TrxType | PostCalculationAmt |
      | ppOrder                | mainProd                | AveragePO        | MR                    | 0                  |
