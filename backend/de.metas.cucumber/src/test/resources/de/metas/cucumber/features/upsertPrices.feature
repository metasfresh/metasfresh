Feature: Create or update using prices API
  ...
  Scenario: Create price list version from external ref
    ...
    When the user adds price list version data
      | Identifier  | OrgCode | M_PriceList.Identifier | Description                    | ValidFrom            | OPT.IsActive |
      | ext-Other-3 | 001     | 2                      | price_list_version_description | 2018-01-24T00:00:00Z | true         |
    ...
    And the metasfresh REST-API endpoint path 'api/prices/priceListVersions' receives a 'PUT' request with the payload from context and responds with '200' status code
    Then price list version is persisted correctly

  Scenario: Update price list version
    When the user adds price list version data
      | Identifier  | OrgCode | M_PriceList.Identifier | Description                | ValidFrom            | OPT.IsActive |
      | ext-Other-3 | 001     | 2                      | price_list_version_updated | 2017-06-13T00:00:00Z | true         |
    And we create a JsonRequestPriceListVersionUpsert, set syncAdvise to 'CREATE_OR_MERGE' and store request payload it in the test context
    When the metasfresh REST-API endpoint path 'api/prices/priceListVersions' receives a 'PUT' request with the payload from context and responds with '200' status code
    Then price list version is persisted correctly

  Scenario: Create product prices by price list version identifier
    And metasfresh contains M_Product with identifier '123'
    And the user adds product prices data
      ...
      | ext-Other-4 | 001     | 123                     | 10       | 20             | 30            | 10    | true         | NORMAL                   |
    ...
    When the metasfresh REST-API endpoint path 'api/prices/priceListVersions/ext-Other-3/productPrices' receives a 'PUT' request with the payload from context and responds with '200' status code
