...
## F5000: Handling Unit
  ...
  Scenario: Cleared HUs can be manually picked
    ...
    # Regression guard: a reversed schedule must reappear in M_Packageable_V (the fix must not hide it)
    And validate M_Packageable_V for shipmentSchedule identified by s_s_2
      | IsPresent |
      | Y         |


  Scenario: Fully-picked order on a draft shipment is not packageable
    Given metasfresh contains C_BPartners:
      | Identifier   | Name                                   | OPT.IsVendor | OPT.IsCustomer | M_PricingSystem_ID.Identifier |
      | bpartner_bug | BPartnerTest_DraftShipmentNotPackeable | N            | Y              | pricingSys_1                  |
    And metasfresh contains C_BPartner_Locations:
      | Identifier   | GLN           | C_BPartner_ID.Identifier |
      | location_bug | 0285656789099 | bpartner_bug             |

    And metasfresh contains C_Orders:
      | Identifier | IsSOTrx | C_BPartner_ID.Identifier | DateOrdered | DeliveryRule |
      | order_bug  | true    | bpartner_bug             | 2022-03-28  | F            |
    And metasfresh contains C_OrderLines:
      | Identifier | C_Order_ID.Identifier | M_Product_ID.Identifier | QtyEntered |
      | ol_bug     | order_bug             | huProduct               | 10         |

    And the order identified by order_bug is completed

    And after not more than 60s, M_ShipmentSchedules are found:
      | Identifier | C_OrderLine_ID.Identifier | IsToRecompute |
      | s_s_bug    | ol_bug                    | N             |

    # Positive direction: while QtyToDeliver > 0 the schedule IS packageable
    And validate M_Packageable_V for shipmentSchedule identified by s_s_bug
      | IsPresent |
      | Y         |

    And validate M_HUs are available to pick for shipmentSchedule identified by s_s_bug
      | M_HU_ID.Identifier |
      | createdCU          |

    When create M_PickingCandidate for M_HU
      | M_HU_ID.Identifier | M_ShipmentSchedule_ID.Identifier | QtyPicked | Status | PickStatus | ApprovalStatus |
      | createdCU          | s_s_bug                          | 10        | IP     | P          | ?              |

    And process picking
      | M_HU_ID.Identifier | M_ShipmentSchedule_ID.Identifier |
      | createdCU          | s_s_bug                          |

    # Generate a DRAFT shipment (IsCompleteShipments = false) — do NOT complete it
    And 'generate shipments' process is invoked individually for each M_ShipmentSchedule
      | M_ShipmentSchedule_ID.Identifier | QuantityType | IsCompleteShipments | IsShipToday |
      | s_s_bug                          | P            | false               | false       |
