...
Feature: The receipt-logistics window lists what is arriving, planned or not
  ...
  Scenario: Several unplanned rows received together are grouped exactly as the receipt-schedule batch groups them
    ...
    And after not more than 60s, the C_Order identified by orderMultiA_RL has exactly the following rows in RV_ReceiptLogistics:
      | RV_ReceiptLogistics_ID | M_Delivery_Planning_ID | M_ReceiptSchedule_ID | OPT.IsPlanned |
      | rowMultiA1_RL          | null                   | scheduleMultiA1_RL   | false         |
      | rowMultiA2_RL          | null                   | scheduleMultiA2_RL   | false         |
    And after not more than 60s, the C_Order identified by orderMultiB_RL has exactly the following rows in RV_ReceiptLogistics:
      | RV_ReceiptLogistics_ID | M_Delivery_Planning_ID | M_ReceiptSchedule_ID | OPT.IsPlanned |
  ...
  Scenario: Several planned rows received together produce one receipt per planning, each carrying its own
    ...
    And after not more than 60s, the C_Order identified by orderPlanTwo_RL has exactly the following rows in RV_ReceiptLogistics:
      | RV_ReceiptLogistics_ID | M_Delivery_Planning_ID | M_ReceiptSchedule_ID | OPT.IsPlanned |
  ...
  Scenario: A mixed selection routes per row, so one gesture yields a linked receipt and a plain one
    ...
    And after not more than 60s, the C_Order identified by orderMixed_RL has exactly the following rows in RV_ReceiptLogistics:
      | RV_ReceiptLogistics_ID | M_Delivery_Planning_ID | M_ReceiptSchedule_ID  | OPT.IsPlanned |
  ...
  Scenario: Two plannings of ONE order line are received together without receiving that line twice
    ...
    And after not more than 60s, the C_Order identified by orderSplit_RL has exactly the following rows in RV_ReceiptLogistics:
      | RV_ReceiptLogistics_ID | M_Delivery_Planning_ID | M_ReceiptSchedule_ID | OPT.IsPlanned |
