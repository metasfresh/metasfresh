...
public class WEBUI_CreateRequest extends JavaProcess
{
	...
	private void createRequestFromUser(final I_AD_User user)
	{
		final ImmutableList.Builder<JSONDocumentChangedEvent> events = ImmutableList.builder();
		...
		getResult().setRecordToOpen(TableRecordReference.of(I_R_Request.Table_Name, documentId.toInt()), documentPath.getWindowId().toInt(), OpenTarget.SingleDocument);
	}
}
