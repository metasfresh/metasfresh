...
/**
 * Listener for notifying the user when a dunning PDF concatenating was done
 * <ul>
 * when the workpackage was processed, the listener is triggered
 * </ul>
 */
public class AutomaticallyDunningPrintinAsyncBatchListener implements IAsyncBatchListener
{
	private static final AdMessageKey MSG_Event_DunningPDFGenerated = AdMessageKey.of("AutomaticallyDunningPdfPrintinAsyncBatchListener_Pdf_Done");
	...
	{
		if(asyncBatchBL.isAsyncBatchForAutomaticallyDunningPDFPrinting(asyncBatch))
		{
	...
	private void sendUserNotifications(@NonNull final I_C_Async_Batch asyncBatch)
	{
		...
		notificationBL.send(UserNotificationRequest.builder()
									...
									.contentADMessage(MSG_Event_DunningPDFGenerated)
