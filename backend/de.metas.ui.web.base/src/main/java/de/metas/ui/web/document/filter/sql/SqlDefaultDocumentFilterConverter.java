...
/* package */final class SqlDefaultDocumentFilterConverter implements SqlDocumentFilterConverter
{
			...
			@NonNull final SqlDocumentFilterConverterContext context)
	{
		...
		for (final DocumentFilterParam filterParam : filter.getParameters())
		{
			...
			else if (filterParam.isJoinAnd())
			{
				sql = sql.and(sqlFilterParam);
			}
			else
			{
				sql = sql.or(sqlFilterParam);
			}
		}

		return sql != null ? SqlFilter.of(sql) : SqlFilter.ACCEPT_ALL;
	}
	 ...
	 */
	@Nullable
	private SqlAndParams buildSqlWhereClause(final String filterId, final DocumentFilterParam filterParam, final SqlOptions sqlOpts)
	{
		//
		// SQL filter
		if (filterParam.isSqlFilter())
		{
			...
			return SqlAndParams.of(sqlWhereClause, sqlWhereClauseParams);
		}
	...
	static SqlAndParams buildSqlWhereClause_Like(final String sqlColumnExpr, final boolean negate, final boolean ignoreCase, @Nullable final Object sqlValue)
	{
		...
		final String sql = DBConstants.FUNCNAME_unaccent_string + "(" + sqlColumnExpr + ", 1)"
				+ sqlOperator
				+ DBConstants.FUNCNAME_unaccent_string + "(?, 1)";
		return SqlAndParams.of(sql, sqlValueStr);
	}
			...
			final SqlOptions sqlOpts)
	{
		...
		for (final LookupValue lookupValue : lookupValues)
		{
			...
			sql.append("EXISTS (SELECT 1 FROM ").append(labelsTableName).append(" labels ")
					...
					.append(" AND labels.").append(labelsValueColumnName).append("=").appendParam(labelValue)
					.append(")");
		}
