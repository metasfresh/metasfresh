 ...
 * <p>
 * {@code RV_ReceiptLogistics} is a UNION of two branches - a <b>planned</b> row (an active {@code Incoming}
...
public abstract class ReceiptLogisticsViewBasedProcess extends ViewBasedProcessTemplate
{
	...
	protected final ImmutableList<ReceiptScheduleAndDeliveryPlanningId> getReceiptScheduleAndPlanningIds()
	{
		return getView().streamByIds(getSelectedRowIds())
				.map(ReceiptLogisticsViewBasedProcess::extractReceiptScheduleAndPlanningId)
	...
	static ReceiptScheduleAndDeliveryPlanningId extractReceiptScheduleAndPlanningId(@NonNull final IViewRow row)
	{
		return ReceiptScheduleAndDeliveryPlanningId.of(
				row.getFieldValueAsRepoId(I_RV_ReceiptLogistics.COLUMNNAME_M_ReceiptSchedule_ID, ReceiptScheduleId::ofRepoId),
				row.getFieldValueAsNullableRepoId(I_RV_ReceiptLogistics.COLUMNNAME_M_Delivery_Planning_ID, DeliveryPlanningId::ofRepoId));
	}
