...
public class WEBUI_DocTextLines_MoveUp extends ViewBasedProcessTemplate implements IProcessPrecondition
{
	...
	{
		return checkMoveUpPreconditions(getView(), getSelectedRowIds());
	}
	 ...
	 */
	public static ProcessPreconditionsResolution checkMoveUpPreconditions(
	...
	protected String doIt()
	{
		...
		moveUp(getView(), rowId);

		return MSG_OK;
	}

	/** Public so it is directly unit-testable without the {@code JavaProcess} parameter/view-loading machinery. */
	public void moveUp(@NonNull final DocTextLinesView view, @NonNull final DocumentId rowId)
	{
		view.moveRow(rowId, true);
	}
}
