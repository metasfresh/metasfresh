...
class GetInvoiceCandidatesAmtSelectionSummaryCommand
{
	...
	public InvoiceCandidatesAmtSelectionSummary execute()
	{
		...
		{
			// The extraWhereClause is generated externally by the metasfresh view framework and contains
			// bare column references (e.g. C_Order_ID, isSOTrx, AD_Org_ID).
			// After adding LEFT JOIN C_OrderLine ol, columns that exist in both tables become ambiguous.
			// We qualify them with the C_Invoice_Candidate table name to resolve the ambiguity.
			final String qualifiedExtraWhereClause = qualifyAmbiguousColumns(extraWhereClause);
			sql.append(" AND (").append(qualifiedExtraWhereClause).append(")");
		}
		...
		{
			final String qualifiedDefaultFilter = qualifyAmbiguousColumns(sqlDefaultFilter);
	...
	/**
	 * Qualifies bare column references that are ambiguous after the LEFT JOIN C_OrderLine.
	 * Both C_Invoice_Candidate and C_OrderLine share these column names, so PostgreSQL
	 * raises "column reference is ambiguous" without an explicit table prefix.
	 *
	 * The where-clauses we qualify here come from two external sources:
	 *   - extraWhereClause: generated by the metasfresh view framework (MQuery + T_WEBUI_ViewSelection)
	 *   - sqlDefaultFilter: generated by IInvoiceCandDAO.getSQLDefaultFilter()
	 *
	 * We use negative lookbehind (?<![.\w]) to avoid double-qualifying already-prefixed references.
	 */
	private static String qualifyAmbiguousColumns(@NonNull final String whereClause)
	{
		final String tableName = I_C_Invoice_Candidate.Table_Name;
		// Columns known to exist in both C_Invoice_Candidate and C_OrderLine:
		return whereClause
				.replaceAll("(?<![.\\w])C_Order_ID(?![\\w])", tableName + ".C_Order_ID")
				.replaceAll("(?<![.\\w])isSOTrx(?![\\w])", tableName + ".isSOTrx")
				.replaceAll("(?<![.\\w])AD_Org_ID(?![\\w])", tableName + ".AD_Org_ID")
				.replaceAll("(?<![.\\w])Processed(?![\\w])", tableName + ".Processed")
				.replaceAll("(?<![.\\w])IsActive(?![\\w])", tableName + ".IsActive");
	}
}
