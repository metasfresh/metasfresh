...
public class M_InOutLine_Handler extends AbstractInvoiceCandidateHandler
{
			...
			@Nullable final BigDecimal forcedQtyToAllocate)
	{
		...
		if (icRecord.isPackagingMaterial())
		{
			...
			saveRecord(iciol);
			Loggables.withLogger(logger, Level.INFO).addLog("Setting qtyDelivered={} on icIol_ID={}", icRecord.getQtyOrdered().multiply(qtyMultiplier), iciol.getC_InvoiceCandidate_InOutLine_ID());
		}
		else
		{
