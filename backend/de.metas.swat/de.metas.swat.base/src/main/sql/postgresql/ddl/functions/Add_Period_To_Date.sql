...
DROP FUNCTION IF EXISTS report.Add_Period_To_Date( p_C_Calendar_ID numeric, p_Date date, p_numberOfPeriods int);
CREATE OR REPLACE FUNCTION report.Add_Period_To_Date( p_C_Calendar_ID numeric, p_Date date, p_numberOfPeriods int) RETURNS date AS
/**
...
BEGIN
	...
	INTO 	v_Result 
	FROM	C_Period
	WHERE	C_Period_ID = report.Get_Successor_Period_Recursive( v_C_Period_ID, p_numberOfPeriods );
...
ALTER FUNCTION report.Add_Period_To_Date( p_C_Calendar_ID numeric, p_Date date, p_numberOfPeriods int) OWNER TO adempiere;