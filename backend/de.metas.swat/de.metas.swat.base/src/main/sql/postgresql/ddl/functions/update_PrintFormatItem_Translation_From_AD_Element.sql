...
DROP FUNCTION IF EXISTS update_PrintFormatItem_Translation_From_AD_Element(
    p_AD_Element_ID numeric,
    p_AD_Language character varying
);

CREATE OR REPLACE FUNCTION update_PrintFormatItem_Translation_From_AD_Element
(
...
BEGIN
    ...
    IF (p_AD_Language IS NULL OR isBaseAD_Language(p_AD_Language) = 'Y') THEN
        UPDATE metasfresh.public.ad_printformatitem p
        SET Name        = e_trl.Name,
            printname = e_trl.printname
