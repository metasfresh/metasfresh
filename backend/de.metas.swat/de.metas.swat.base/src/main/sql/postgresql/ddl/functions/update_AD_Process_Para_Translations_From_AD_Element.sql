...
BEGIN
    ...
    UPDATE metasfresh.public.ad_process_para_trl p_trl
    SET
    ...
    WHERE (p_AD_Element_ID IS NULL OR e_trl.AD_Element_ID = p_AD_Element_ID)
      ...
      AND EXISTS(SELECT 1 FROM metasfresh.public.ad_process_para p WHERE p.ad_element_id = e_trl.ad_element_id AND p.ad_process_para_id = p_trl.ad_process_para_id);
    ...
    IF (p_AD_Language IS NULL OR isBaseAD_Language(p_AD_Language) = 'Y') THEN
        UPDATE metasfresh.public.ad_process_para p
        SET Name        = e_trl.Name,
            Description = e_trl.Description,
            Help = e_trl.help
