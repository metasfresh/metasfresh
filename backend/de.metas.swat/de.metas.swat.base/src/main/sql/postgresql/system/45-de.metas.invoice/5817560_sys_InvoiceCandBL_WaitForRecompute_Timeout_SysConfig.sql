-- SysConfig backing InvoiceCandBL.waitForInvoiceCandidatesUpdated: the max number of seconds the wait
-- polls for a selection's invoice candidates to be recomputed before aborting. Default 3600 (one hour);
-- lower it to make a wedged recompute fail fast instead of blocking for the full hour.
