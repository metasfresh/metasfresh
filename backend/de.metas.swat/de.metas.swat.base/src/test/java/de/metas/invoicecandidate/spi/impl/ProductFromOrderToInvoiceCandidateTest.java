...
public class ProductFromOrderToInvoiceCandidateTest
{
	...
	private ProductId changedProductId;
	...
	public void beforeEach()
	{
		...
		final I_M_Product changedProduct = BusinessTestHelper.createProduct("changedProduct", stockUom);
		changedProductId = ProductId.ofRepoId(changedProduct.getM_Product_ID());
	...
	public void testProductChanged()
	{
		...
		assertThat(candidate.getInvoiceRule()).isNotNull();
		saveRecord(candidate);
		// ReActivate
		docActionBL.processEx(order, IDocument.ACTION_ReActivate, IDocument.STATUS_InProgress);
		orderLine.setM_Product_ID(changedProductId.getRepoId());

		olHandler.setOrderedData(candidate);
		assertThat(candidate.getM_Product_ID()).as("Invalid changed product").isEqualByComparingTo(orderLine.getM_Product_ID());
	}
}
