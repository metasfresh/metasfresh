...
public class ShipmentScheduleSplitServiceTest
{
	...
	{
		final I_M_ShipmentSchedule_Split completedSplit = createShipmentScheduleSplit(X_M_InOut.DOCSTATUS_Completed, new BigDecimal(8), false);
		final I_M_ShipmentSchedule_Split voidedSplit = createShipmentScheduleSplit(X_M_InOut.DOCSTATUS_Voided, new BigDecimal(2), false);

		final List<ShipmentScheduleSplit> result = shipmentScheduleSplitService.getByShipmentScheduleId(ShipmentScheduleId.ofRepoId(shipmentSchedule.getM_ShipmentSchedule_ID()));
	...
	@Test
	void testGetRecordToProcessByShipmentScheduleId()
	{
		final I_M_ShipmentSchedule_Split processedSplit = createShipmentScheduleSplit(X_M_InOut.DOCSTATUS_Completed, new BigDecimal(8), true);
		final I_M_ShipmentSchedule_Split notProcessedSplit = createShipmentScheduleSplit(X_M_InOut.DOCSTATUS_Completed, new BigDecimal(2), false);

		final List<ShipmentScheduleSplit> result = shipmentScheduleSplitService.getRecordToProcessByShipmentScheduleId(ShipmentScheduleId.ofRepoId(shipmentSchedule.getM_ShipmentSchedule_ID()));
	...
	private I_M_InOut createInOut(final String status, final BigDecimal movementQty)
	{
	...
	private I_M_ShipmentSchedule_Split createShipmentScheduleSplit(final String status, final BigDecimal movementQty, final boolean processed)
	{
		...
		final I_M_InOut inout = createInOut(status, movementQty);
		split.setM_InOut(inout);
