...
public class TestDoubleReceiptInvoiceMoreThanReceived extends AbstractDoubleReceiptQtyOverride
{
	...
	protected void step_validate_after_aggregation(List<I_C_Invoice_Candidate> invoiceCandidates, List<I_M_InOutLine> inOutLines, List<IInvoiceHeader> invoices)
	{
		...
		assertThat(icIolToUpdate111.getQtyInvoiced().getStockQty().toBigDecimal())
				.isEqualTo(FIFTY);

		final InvoiceCandidateInOutLineToUpdate icIolToUpdate121 = retrieveIcIolToUpdateIfExists(il1, iol121);
		assertThat(icIolToUpdate121.getQtyInvoiced().getStockQty().toBigDecimal())
				.isEqualTo(FIFTY.add(TWENTY));
	}
