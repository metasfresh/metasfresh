DROP FUNCTION IF EXISTS report.tax_accounting_report_details_sum(IN p_dateFrom   date,
                                                                 IN p_dateTo     date,
...
CREATE OR REPLACE FUNCTION report.tax_accounting_report_details_sum(IN p_dateFrom   date,
                                                                    IN p_dateTo     date,
                                                                    IN p_vatcode    varchar,
                                                                    IN p_account_id numeric,
                                                                    IN p_c_tax_id   numeric,
...
SELECT y.vatcode,
       ...
       y.ad_org_id
FROM (
         SELECT DISTINCT ON (x.vatcode, x.kontono, x.kontoname, x.taxname, x.taxrate, x.c_currency_id, x.dateacct) x.vatcode,
                                                                                                                   x.kontono,
                                                                                                                   x.kontoname,
                                                                                                                   x.taxname,
                                                                                                                   x.taxrate,
         ...
         JOIN c_acctschema aas ON y.ad_org_id = aas.ad_orgonly_id
GROUP BY y.vatcode,
         y.kontono,
         y.kontoname,
         y.taxname,
