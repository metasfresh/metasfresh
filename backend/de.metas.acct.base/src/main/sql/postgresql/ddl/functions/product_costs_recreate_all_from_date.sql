...
BEGIN
    ...
    --
    --
    -- RESUME, PART 2: THE RECORDED BATCHES MUST COVER THIS CALL'S OWN PRODUCTS
    --                - CHECKED HERE BECAUSE THIS IS THE LAST MUTATION-FREE MOMENT
    --
    --
    -- The second half of the run-identity check the resume branch above started, and it has to be finished
    -- HERE. The parameters can all match and the product POPULATION still have changed since the crashed run
    -- selected it: this procedure has no product parameter, it re-derives its product set from the documents
    -- in range on every call. So a document posted into that range in the meantime - the ordinary thing that
    -- happens while an operator investigates a failed run - brings a new product in, and because the batches
    -- are cut by M_Product_ID, a newcomer with a lower id shifts every batch boundary after it. A batch_no
    -- recorded DONE then names a DIFFERENT set of products, which would be skipped: never rewound, never
    -- staged, and the run reports success. Hence refuse.
    --
    -- Why the check cannot live inside the batch loop, where it is cheapest: the loop is reached only AFTER
    -- the MAI purge (c) below has deleted the target element's in-range cost details and COMMITted them.
    -- That purge runs on EVERY call whose requested cost element differs from the MAI target - resume or
    -- fresh - so a refusal raised from the loop would be a refusal that has already destroyed the very rows
    -- a later, correct resume needs, and no rollback can bring them back. A refusal is only honest while
    -- nothing has been mutated, so every DONE batch is compared here, up front, in one statement.
    --
    -- IS DISTINCT FROM, not <>: a DONE batch_no that the current call does not produce at all (the
    -- population shrank, or the recorded run used a different products-per-commit - which the parameter
    -- check above already refused) has no current product set, and NULL must count as "different", not as
    -- "unknown".
    --
    IF (p_Resume = 'Y') THEN
        SELECT pr.batch_no,
               pr.products AS recorded_products,
               cur.products AS current_products
        INTO v_populationBatch
        FROM "de_metas_acct".product_costs_recreate_progress pr
                 LEFT JOIN (SELECT p.batch_no,
                                   ARRAY_AGG(p.m_product_id ORDER BY p.m_product_id) AS products
                            FROM tmp_pcra_products p
                            GROUP BY p.batch_no) cur ON cur.batch_no = pr.batch_no
        WHERE pr.status = 'DONE'
          AND pr.products IS DISTINCT FROM cur.products
        ORDER BY pr.batch_no
        LIMIT 1;
        IF (FOUND) THEN
            RAISE EXCEPTION 'Cannot resume: batch % was recorded for product(s) % but now covers product(s) % - the product population changed since that run. Start it over with p_Resume:=''N''.',
                v_populationBatch.batch_no, v_populationBatch.recorded_products, v_populationBatch.current_products;
        END IF;
    END IF;
    ...
    FOR v_batch IN (
        ...
        LOOP
            --
            -- Skipping a DONE batch is safe to decide on batch_no alone, because "RESUME, PART 2" above has
            -- already proven - before anything was mutated - that every DONE batch_no covers exactly the
            -- products it was recorded for. There is deliberately no second product comparison here:
            -- tmp_pcra_products is a temporary table computed once per call and never written again, and the
            -- advisory lock (a) keeps any other run of this procedure out, so nothing between that check and
            -- here can change either side of the comparison. A copy of it in this loop could therefore never
            -- fire - and this is precisely where it must NOT be, since the purge (c) has committed by now.
            --
            IF (p_Resume = 'Y'
                AND EXISTS(SELECT 1
                           FROM "de_metas_acct".product_costs_recreate_progress pr
