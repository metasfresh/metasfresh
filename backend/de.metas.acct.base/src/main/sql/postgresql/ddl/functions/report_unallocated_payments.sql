--  B_UnallocatedCash_Acct - Nicht zugeordnete Debitoren Zahlungen
-- Fact_Acct.AD_Table_ID: C_AllocationHdr, C_Payment, GL_Journal
-- => common dimension: C_Payment_ID

DROP FUNCTION IF EXISTS de_metas_acct.report_unallocated_payments
(
    numeric,
    numeric,
    date
...
CREATE OR REPLACE FUNCTION de_metas_acct.report_unallocated_payments(
...
BEGIN
    ...
    DROP TABLE IF EXISTS tmp_fact_acct_prefiltered_unallocated_payments;
    CREATE TEMPORARY TABLE tmp_fact_acct_prefiltered_unallocated_payments
    (
    ...
    INSERT INTO tmp_fact_acct_prefiltered_unallocated_payments (fact_acct_id, tablename, c_payment_id)
    SELECT fa.fact_acct_id,
           t.tablename,
           (CASE
                WHEN t.tablename = 'C_Payment'       THEN fa.record_id
                WHEN t.tablename = 'C_AllocationHdr' THEN (SELECT al.c_payment_id FROM c_allocationline al WHERE al.c_allocationline_id = fa.line_id)
            END) AS c_payment_id
    FROM fact_acct fa
    ...
    CREATE UNIQUE INDEX ON tmp_fact_acct_prefiltered_unallocated_payments (fact_acct_id);

    --
    --

    DROP TABLE IF EXISTS tmp_fact_acct_unallocated_payments;
    CREATE TEMPORARY TABLE tmp_fact_acct_unallocated_payments AS
    SELECT (CASE
    ...
    FROM (SELECT sel.c_payment_id,
                 ...
                 fa.*
          FROM tmp_fact_acct_prefiltered_unallocated_payments sel
    ...
    CREATE INDEX ON tmp_fact_acct_unallocated_payments (c_acctschema_id, aggregationKey, accountno);

    --
    -- Aggregate
    --

    DROP TABLE IF EXISTS tmp_fact_acct_unbalanced_agg_unallocated_payments;
    CREATE TEMPORARY TABLE tmp_fact_acct_unbalanced_agg_unallocated_payments AS
    SELECT t.c_acctschema_id, t.aggregationKey, t.accountno
    FROM tmp_fact_acct_unallocated_payments t
    GROUP BY t.c_acctschema_id, t.aggregationKey, t.accountno
    HAVING COALESCE(SUM(t.amtacctdr), 0) != COALESCE(SUM(t.amtacctcr), 0);

    CREATE UNIQUE INDEX ON tmp_fact_acct_unbalanced_agg_unallocated_payments (c_acctschema_id, aggregationKey, accountno);
    ...
    RETURN QUERY SELECT --
                 ...
                 WHERE TRUE
                   AND EXISTS(SELECT 1
                              FROM tmp_fact_acct_unbalanced_agg_unallocated_payments agg
