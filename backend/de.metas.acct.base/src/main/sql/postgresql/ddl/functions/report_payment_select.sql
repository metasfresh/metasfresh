-- B_PaymentSelect_Acct - Ausgewählte Zahlungen
-- Fact_Acct.AD_Table_ID: C_AllocationHdr, C_Payment, GL_Journal
-- => common dimension: C_Payment_ID

DROP FUNCTION IF EXISTS de_metas_acct.report_payment_select
(
    numeric,
    numeric,
    date
...
CREATE OR REPLACE FUNCTION de_metas_acct.report_payment_select(
...
BEGIN
    ...
    DROP TABLE IF EXISTS tmp_fact_acct_prefiltered_payment_select;
    CREATE TEMPORARY TABLE tmp_fact_acct_prefiltered_payment_select
    (
    ...
    INSERT INTO tmp_fact_acct_prefiltered_payment_select (fact_acct_id, tablename, c_payment_id)
    ...
    CREATE UNIQUE INDEX ON tmp_fact_acct_prefiltered_payment_select (fact_acct_id);

    --
    --

    DROP TABLE IF EXISTS tmp_fact_acct_payment_select;
    CREATE TEMPORARY TABLE tmp_fact_acct_payment_select AS
    SELECT (CASE
    ...
    FROM (SELECT sel.c_payment_id,
                 ...
                 fa.*
          FROM tmp_fact_acct_prefiltered_payment_select sel
    ...
    CREATE INDEX ON tmp_fact_acct_payment_select (c_acctschema_id, aggregationKey, accountno);

    --
    -- Aggregate
    --

    DROP TABLE IF EXISTS tmp_fact_acct_unbalanced_agg_payment_select;
    CREATE TEMPORARY TABLE tmp_fact_acct_unbalanced_agg_payment_select AS
    SELECT t.c_acctschema_id, t.aggregationKey, t.accountno
    FROM tmp_fact_acct_payment_select t
    GROUP BY t.c_acctschema_id, t.aggregationKey, t.accountno
    HAVING COALESCE(SUM(t.amtacctdr), 0) != COALESCE(SUM(t.amtacctcr), 0);

    CREATE UNIQUE INDEX ON tmp_fact_acct_unbalanced_agg_payment_select (c_acctschema_id, aggregationKey, accountno);
    ...
    RETURN QUERY SELECT --
                 ...
                 WHERE TRUE
                   AND EXISTS(SELECT 1
                              FROM tmp_fact_acct_unbalanced_agg_payment_select agg
