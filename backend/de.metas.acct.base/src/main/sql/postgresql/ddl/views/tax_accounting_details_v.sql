...
SELECT ev.value          AS kontono,
       ...
       (CASE
            WHEN fa.line_id IS NULL AND fa.C_Tax_id IS NOT NULL
                THEN
                (select cfa.amtacctdr - cfa.amtacctcr
                 FROM fact_acct cfa
                 where cfa.record_id = fa.record_id and cfa.ad_table_id=fa.ad_table_id and cfa.line_id is not null and cfa.c_tax_id=fa.c_tax_id)
            ELSE 0
           END)          AS total,
       acct_cur.iso_code as schema_currency,
...
FROM fact_acct fa
         ...
         LEFT OUTER JOIN c_acctschema cas on fa.c_acctschema_id = cas.c_acctschema_id
         LEFT OUTER JOIN C_Currency acct_cur ON cas.c_Currency_ID = acct_cur.C_Currency_ID
    ...
    END)                                  AS taxbaseamt,
                         ...
                         ON (fa.record_id = i.c_invoice_id AND fa.ad_table_id = get_Table_Id('C_Invoice') AND
    ...
    END)                                                                AS taxbaseamt,
                          ...
                          FROM gl_journal gl
                                   JOIN GL_JournalLine gll
                                        ON gl.gl_journal_id = gll.gl_journal_id) gl
                         ON (fa.record_id = gl.gl_journal_id AND fa.ad_table_id = get_Table_Id('GL_Journal') AND
                             gl.gl_journalline_id = fa.line_id AND gl.tax_id = fa.c_tax_id)
    --
    -- if allocationHdr
         LEFT OUTER JOIN c_allocationline al
                         ON (al.c_allocationline_id = fa.line_id AND al.c_allocationhdr_id = fa.record_id AND
                             fa.ad_table_id = get_Table_Id('C_AllocationHdr'))
;
