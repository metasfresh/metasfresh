...
DECLARE
    rowcount integer;
BEGIN
    RAISE NOTICE 'accounting_docs_to_repost_reorder: p_DateAcct_Trunc=%', p_DateAcct_Trunc;
    ...
    WITH docs AS (SELECT t.*,
                  ...
                  FROM (SELECT DISTINCT --
                                        TRUNC(doc.dateacct, p_DateAcct_Trunc) AS dateacct,
                                        doc.tablename_prio,
                                        doc.tablename,
                                        doc.record_id,
                                        doc.reversal_id
