...
BEGIN
    ...
    IF (v_inventoryInfo.docstatus != 'DR' AND p_DryRun != 'Y') THEN
    ...
    SELECT loc.m_warehouse_id,
           fa.m_product_id,
           COALESCE(fa.c_uom_id, p.c_uom_id) AS c_uom_id,
    ...
    WHERE ev.value = p_ProductAssetAccount
      ...
      AND NOT (fa.ad_table_id = get_table_id('M_Inventory') AND fa.record_id = p_M_Inventory_ID)                                                            -- Exclude this inventory
      AND NOT (fa.dateacct::date >= v_inventoryInfo.DateAcctTo::date AND fa.ad_table_id = get_table_id('M_Inventory') AND fa.record_id >= p_M_Inventory_ID) -- Exclude all inventories after the inventory we are computing
      AND (p_ImportOnlyGivenProducts = 'N'
        OR
           EXISTS(SELECT 1 FROM m_inventoryline il WHERE il.m_inventory_id = p_M_Inventory_ID AND fa.m_product_id = il.m_product_id and fa.c_uom_id=il.c_uom_id))

    GROUP BY loc.m_warehouse_id, fa.m_product_id, COALESCE(fa.c_uom_id, p.c_uom_id);
    GET DIAGNOSTICS v_rowcount = ROW_COUNT;
    CREATE UNIQUE INDEX ON tmp_fact_acct (m_product_id, c_uom_id);
    ...
    IF (p_RecreateLines = 'Y') THEN
        ...
        GET DIAGNOSTICS v_rowcount = ROW_COUNT;
        CREATE UNIQUE INDEX ON tmp_prev_inventoryline (m_product_id, c_uom_id); -- atm we support only one product/uom per line
        ...
        INSERT INTO m_inventoryline (m_inventoryline_id, ad_client_id, ad_org_id, createdby, updatedby, m_inventory_id, m_locator_id, m_product_id, line, description, c_charge_id, qtyinternaluse, reversalline_id, m_inoutline_id, c_uom_id, m_hu_pi_item_product_id, qtytu, qualitynote, m_hu_id, assignedto, externalid,
                                     isexplicitcostprice,
                                     costprice,
                                     qtybook,
                                     qtycount,
        ...
        SELECT NEXTVAL('m_inventoryline_seq'),
               ...
               NULL                                                                                   AS assignedto,
               NULL                                                                                   AS externalid,
               prev_invl.isexplicitcostprice                                                          AS isexplicitcostprice,
        ...
        WHERE
            -- for products and UOMs we don't already have
            NOT EXISTS(SELECT 1 FROM m_inventoryline invl WHERE invl.m_inventory_id = p_M_Inventory_ID AND invl.m_product_id = prev_invl.m_product_id AND invl.c_uom_id = prev_invl.c_uom_id)
        ...
        CREATE INDEX ON tmp_hu (c_uom_id);
        RAISE NOTICE 'Selected % M_HU records', v_rowcount;

        -- IMPORTANT: we assume the Products/UOMs from M_InventoryLine are distinct!!!
        RAISE NOTICE 'Making sure we don''t have multiple lines with same product...';
        FOR v_record IN (SELECT invl.m_product_id, p.Value, p.name, invl.c_uom_id, COUNT(1) AS duplicates
                         FROM m_inventoryline invl
                                  INNER JOIN m_product p ON p.m_product_id = invl.m_product_id
                         WHERE invl.m_inventory_id = p_M_Inventory_ID
                         GROUP BY invl.m_product_id, p.Value, p.name, invl.c_uom_id
                         HAVING COUNT(1) > 1)
            LOOP
        ...
        SELECT invl.ad_client_id,
               ...
               invl.m_inventory_id
        FROM m_inventoryline invl
                 LEFT JOIN tmp_hu hu ON hu.m_product_id = invl.m_product_id AND hu.c_uom_id = invl.c_uom_id
        WHERE invl.m_inventory_id = p_M_Inventory_ID
          AND invl.isactive = 'Y'
    ...
    ELSE
        ...
        SET qtybook=COALESCE((SELECT COALESCE(t.qty, 0) FROM tmp_fact_acct t WHERE t.m_product_id = il.m_product_id AND t.c_uom_id = il.c_uom_id), 0),
            iscounted='Y'
