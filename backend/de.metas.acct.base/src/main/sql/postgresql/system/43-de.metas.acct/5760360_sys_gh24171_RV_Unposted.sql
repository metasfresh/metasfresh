...
CREATE VIEW rv_unposted(ad_client_id, ad_org_id, created, createdby, updated, updatedby, isactive, documentno, datedoc, dateacct, ad_table_id, record_id, issotrx, posted, postingerror_issue_id, processing, processed, docstatus) AS
...
FROM gl_journal
WHERE gl_journal.posted <> 'Y'::bpchar
  AND gl_journal.docstatus <> 'VO'::bpchar
  AND gl_journal.processed = 'Y'::bpchar
UNION ALL
...
WHERE j.posted <> 'Y'::bpchar
  AND j.docstatus::text <> 'VO'::text
  AND j.processed = 'Y'::bpchar
UNION ALL
...
SELECT c_invoice.ad_client_id,
       ...
       c_invoice.docstatus
FROM c_invoice
WHERE c_invoice.posted <> 'Y'::bpchar
  AND c_invoice.docstatus <> 'VO'::bpchar
  AND c_invoice.processed = 'Y'::bpchar
UNION ALL
SELECT m_inout.ad_client_id,
       ...
       m_inout.docstatus
FROM m_inout
WHERE m_inout.posted <> 'Y'::bpchar
  AND m_inout.docstatus <> 'VO'::bpchar
  AND m_inout.processed = 'Y'::bpchar
UNION ALL
...
FROM m_inventory
WHERE m_inventory.posted <> 'Y'::bpchar
  AND m_inventory.docstatus <> 'VO'::bpchar
  AND m_inventory.processed = 'Y'::bpchar
UNION ALL
...
FROM m_movement
WHERE m_movement.posted <> 'Y'::bpchar
  AND m_movement.docstatus <> 'VO'::bpchar
  AND m_movement.processed = 'Y'::bpchar
UNION ALL
SELECT c_cash.ad_client_id,
       ...
       c_cash.docstatus
FROM c_cash
WHERE c_cash.posted <> 'Y'::bpchar
  AND c_cash.docstatus <> 'VO'::bpchar
  AND c_cash.processed = 'Y'::bpchar
UNION ALL
SELECT c_payment.ad_client_id,
       ...
       c_payment.docstatus
FROM c_payment
WHERE c_payment.posted <> 'Y'::bpchar
  AND c_payment.docstatus <> 'VO'::bpchar
  AND c_payment.processed = 'Y'::bpchar
UNION ALL
...
WHERE c_allocationhdr.posted <> 'Y'::bpchar
  AND c_allocationhdr.docstatus <> 'VO'::bpchar
  AND c_allocationhdr.processed = 'Y'::bpchar
UNION ALL
...
WHERE c_bankstatement.posted <> 'Y'::bpchar
  AND c_bankstatement.docstatus <> 'VO'::bpchar
  AND c_bankstatement.processed = 'Y'::bpchar
UNION ALL
...
WHERE m_matchinv.posted <> 'Y'::bpchar
  AND m_matchinv.processed = 'Y'::bpchar
UNION ALL
SELECT c_order.ad_client_id,
       ...
       c_order.docstatus
FROM c_order
WHERE c_order.posted <> 'Y'::bpchar
  AND (c_order.docstatus::text <> ALL (ARRAY ['VO'::text, 'WP'::text]))
  AND c_order.processed = 'Y'::bpchar
UNION ALL
...
FROM m_requisition
WHERE m_requisition.posted <> 'Y'::bpchar
  AND m_requisition.docstatus <> 'VO'::bpchar
  AND m_requisition.processed = 'Y'::bpchar
UNION ALL
SELECT pp_order.ad_client_id,
       ...
       pp_order.docstatus
FROM pp_order
WHERE pp_order.posted <> 'Y'::bpchar
  AND pp_order.docstatus <> 'VO'::bpchar
  AND pp_order.processed = 'Y'::bpchar
UNION ALL
...
WHERE pp_cost_collector.posted <> 'Y'::bpchar
  AND pp_cost_collector.docstatus <> 'VO'::bpchar
  AND pp_cost_collector.processed = 'Y'::bpchar
UNION ALL
...
WHERE cr.posted <> 'Y'::bpchar
  AND (cr.docstatus::text = ANY (ARRAY ['CO'::character varying::text, 'CL'::character varying::text, 'RE'::character varying::text]))
  AND cr.processed = 'Y'::bpchar
;
