...
CREATE OR REPLACE FUNCTION de_metas_acct.InventoryValue_Correct_CostPrice(
    IN p_DateAcct            timestamp WITH TIME ZONE,
    IN p_M_Product_ID        numeric(10, 0) = 0,
    IN p_C_AcctSchema_ID     numeric(10, 0) = NULL,
    IN p_GL_Journal_DateAcct timestamp WITH TIME ZONE = NULL,
    IN p_GL_Journal_ID       NUMERIC(10, 0) = 0
)
    RETURNS void
    LANGUAGE plpgsql
AS
...
DECLARE
    ...
    v_M_CostElement_ID        numeric;
    v_M_CostElement_Name      varchar;
BEGIN
    --
    -- GL_Journal_ID
    ...
    END IF;
    -- keep only negative cost prices:
    DELETE FROM tmp_inventory_valuation WHERE CostPrice >= 0;
    --
    ...
    --
    -- M_CostElement_ID
    SELECT ce.m_costelement_id, ce.name
    INTO v_M_CostElement_ID, v_M_CostElement_Name
    FROM c_acctschema cas
             INNER JOIN m_costelement ce ON ce.costingmethod = cas.costingmethod
    WHERE ce.costelementtype = 'M' -- Material
    ;
    RAISE NOTICE 'Using M_CostElement_ID: % (%)', v_M_CostElement_ID, v_M_CostElement_Name;

    --
    -- CostPrice_new,InventoryValue_new, update ErrorAmt 
    ...
    SET costprice_new=getCurrentCost(
            p_m_product_id=>t.m_product_id,
            p_c_uom_id =>t.c_uom_id,
            p_date =>p_DateAcct::date,
            p_acctschema_id =>t.C_AcctSchema_ID,
            p_m_costelement_id =>v_M_CostElement_ID::numeric,
            p_ad_client_id =>t.AD_Client_ID,
            p_ad_org_id =>t.AD_Org_ID
                      );
    ...
    ALTER TABLE tmp_inventory_valuation
        ALTER COLUMN InventoryValue_new SET NOT NULL;
    --
    DELETE FROM tmp_inventory_valuation t WHERE ErrorAmt = 0;
    ...
    DELETE FROM gl_journalline jl WHERE jl.gl_journal_id = v_GL_Journal_ID AND jl.processed = 'N';
    DELETE FROM gl_journal j WHERE j.gl_journal_id = v_GL_Journal_ID AND j.processed = 'N';
    ...
    SELECT v_GL_Journal_ID                                             AS gl_journal_id,
           ...
           'DR'                                                        AS docstatus,
           'CO'                                                        AS docaction,
           'Correct negative cost prices until ' || (p_DateAcct::date) AS description,
           'A'                                                         AS postingtype,
    ...
    SELECT NEXTVAL('gl_journalline_seq')                                                             AS gl_journalline_id,
           ...
           'N'                                                                                       AS isgenerated,
           'CostPrice: ' || t.CostPrice || '->' || t.CostPrice_new
               || ', Qty: ' || t.Qty || ' ' || t.UOMSymbol
               || ', Amt: ' || t.AmtAcctBalance || '+' || t.ErrorAmt || '->' || t.InventoryValue_new AS description,
