...
BEGIN
    ...
    END IF;
    RAISE DEBUG '=> C_ElementValue_ID=%', v_C_ElementValue_ID;
    ...
    WHERE TRUE
      ...
      -- NOTE: because we still have that bug when userelementstring gets liternal null instead of NULL,
      -- we have to be a bit "lenient" here
      AND NULLIF(userelementstring1, 'null') IS NULL
      AND NULLIF(userelementstring2, 'null') IS NULL
      AND NULLIF(userelementstring3, 'null') IS NULL
      AND NULLIF(userelementstring4, 'null') IS NULL
      AND NULLIF(userelementstring5, 'null') IS NULL
      AND NULLIF(userelementstring6, 'null') IS NULL
      AND NULLIF(userelementstring7, 'null') IS NULL
    ...
    IF (v_C_ValidCombination_ID IS NULL) THEN
        ...
        RAISE DEBUG '=> C_ValidCombination_ID=% (created)', v_C_ValidCombination_ID;
    ELSE
        RAISE DEBUG '=> C_ValidCombination_ID=% (existing)', v_C_ValidCombination_ID;
    END IF;
