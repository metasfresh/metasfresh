...
DROP FUNCTION IF EXISTS de_metas_acct.report_InventoryValue(
    ...
    IN p_M_Warehouse_ID         numeric(10, 0),
    IN p_AD_Language            character varying(6),
    IN p_Exclude_GL_Journal_IDs numeric[]
)
;
...
BEGIN
    ...
    SELECT ROW_NUMBER() OVER ()                                                                              AS id,
           ...
           NULL::numeric                                                                                     AS ErrorAmt, -- Expected(InventoryValue) minus AmtAcctBalance
    ...
    --
    -- Compute Inventory Value (aka Expected Amount)
    UPDATE tmp_inventory_valuation t SET InventoryValue=ROUND(t.CostPrice * t.Qty, t.StdPrecision);

    --
    -- Compute Error Amount (i.e., Expected Amount minus Accounted Amount)
    UPDATE tmp_inventory_valuation t SET ErrorAmt=t.InventoryValue - t.AmtAcctBalance;
    ...
    RETURN QUERY (
        ...
        SELECT t.AccountValue   AS Combination,
               ...
               t.UOMSymbol,
               t.CostPrice,
               t.InventoryValue AS ExpectedAmt,
               t.AmtAcctBalance,
               t.ErrorAmt,
