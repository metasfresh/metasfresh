SELECT db_drop_functions('de_metas_acct.m_inventoryline_update_qtybook_from_fact_acct')
;
...
CREATE OR REPLACE FUNCTION de_metas_acct.m_inventoryline_update_qtybook_from_fact_acct(
    ...
    p_ExcludeInventoriesAfter char(1) = 'Y',
...
DECLARE
    ...
    v_record        record;
BEGIN
    RAISE NOTICE 'Updating inventory line QtyBook from fact acct: p_M_Inventory_ID=%, p_ProductAssetAccount=%, p_RecreateLines=%, p_DryRun=%, p_ImportOnlyGivenProducts=%, p_ExcludeInventoriesAfter=%, p_DateAcctFrom=%, p_DateAcctTo=%',
        p_M_Inventory_ID,
        p_ProductAssetAccount,
        p_RecreateLines,
        p_DryRun,
        p_ImportOnlyGivenProducts,
        p_ExcludeInventoriesAfter,
        p_DateAcctFrom,
        p_DateAcctTo;
    ...
    WHERE ev.value = p_ProductAssetAccount
      ...
      -- Exclude all inventories after the inventory we are computing:
      AND (
        p_ExcludeInventoriesAfter = 'N'
            OR NOT (fa.dateacct::date >= v_inventoryInfo.DateAcctTo::date AND fa.ad_table_id = get_table_id('M_Inventory') AND fa.record_id >= p_M_Inventory_ID)
        )
