...
$CHECK$
    BEGIN
        IF NOT EXISTS(SELECT 1 FROM ad_column WHERE ad_table_id = get_table_id('M_Product_Category_Acct') AND columnname ILIKE 'P_ExternallyOwnedStock_Acct')
        THEN
            RAISE NOTICE 'M_Product_Acct.P_ExternallyOwnedStock_Acct does not exist, creating createM_Product_Category_Acct without it';
