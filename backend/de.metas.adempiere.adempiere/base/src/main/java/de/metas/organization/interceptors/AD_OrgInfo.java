...
@Callout(I_AD_OrgInfo.class)
@Component
public class AD_OrgInfo
{
	private static final AdMessageKey MSG_DROPSHIP_WAREHOUSE_NOT_FLAGGED = AdMessageKey.of("DropShipWarehouse_NotFlagged");

	@NonNull private final IWarehouseDAO warehouseDAO = Services.get(IWarehouseDAO.class);

	public AD_OrgInfo()
	{
		// Register this class as a programmatic callout so @CalloutMethod methods fire
		// immediately when the corresponding column changes in the WebUI.
	 ...
	 * Fires both:
	 * <ul>
	 * <li>{@code @CalloutMethod} — immediately when the user picks a warehouse in the WebUI
	 *     lookup (instant UI feedback before save).</li>
	 * <li>{@code @ModelChange} on BEFORE_NEW / BEFORE_CHANGE — server-side enforcement at
	 *     save time, covering programmatic updates that bypass the UI.</li>
	 * </ul>
	 * Loads the chosen warehouse via {@link IWarehouseDAO} and throws a user-validation error
	 * when the warehouse is not flagged as a dropship warehouse. Clearing the field
	 * (setting it to 0/NULL) is always allowed.
	 * <p>
	 * Paired with:
	 * <ul>
	 ...
	 * </ul>
	 */
	@CalloutMethod(columnNames = { I_AD_OrgInfo.COLUMNNAME_DropShip_Warehouse_ID })
	@ModelChange(
	...
	public void validateDropShipWarehouseIsFlagged(@NonNull final I_AD_OrgInfo orgInfoRecord)
	{
		...
		{
			throw new AdempiereException(MSG_DROPSHIP_WAREHOUSE_NOT_FLAGGED)
