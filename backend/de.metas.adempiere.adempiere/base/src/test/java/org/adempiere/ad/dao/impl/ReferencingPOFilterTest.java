...
public class ReferencingPOFilterTest
{
	...
	/**
	 * Tests for {@link ReferencingPOFilter#accept(IPOReferenceAware)} that a given model is only accepted if it references the model that was given as constructor argument.
	 */
	@Test
	public void accept()
	{
		final ReferencingPOFilter<IPOReferenceAware> filter = new ReferencingPOFilter<IPOReferenceAware>(ol1);
		Assertions.assertTrue(filter.accept(referencing1), "note1 references ol1");
		Assertions.assertFalse(filter.accept(referencing2), "note2 does not reference ol1");
	}
}
