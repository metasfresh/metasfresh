...
--
-- IsTranslated on the three new elements - 'N' on de_DE / de_CH, 'Y' on en_US. That is not a
-- to-do left behind; it is what the flag means. de_DE is the BASE language
-- (AD_Language.IsBaseLanguage='Y'), so AD_Element.Name/Description/Help already ARE the German
-- text: the de_DE _Trl row is a verbatim copy of the authored original, nothing was translated
-- into German, and 'Y' would claim a translation step that never happened. That is also the
-- AD-wide norm and not a local convention - 13217 of 15040 de_DE AD_Element_Trl rows on this
-- stack carry 'N' (87.9%), against 10670 of 15040 en_US rows carrying 'Y' (70.9%). The seed
-- INSERT itself hardcodes 'N' for every language for the same reason, and X_AD_Element_Trl's
-- generated constructor defaults IsTranslated to false. en_US is the only row here that receives
-- a real, separately-authored English wording, so en_US is the only one flipped to 'Y'.
-- de_CH stays 'N' with the de_DE text verbatim: no Swiss-specific wording is needed for these
-- three (no 'ss'/'ß' divergence, no Swiss term swap) - verified against the live rows.
-- fr_CH gets no override at all and keeps the seeded German. Untranslated is the AD-wide norm
-- there (only 145 of 15040 fr_CH element rows are 'Y', 1.0%), and IsTranslated='N' says exactly
-- that. Note the branch is not uniform about WHICH untranslated text a French user then sees:
-- 5820600 and 5821180 point fr_CH at the English wording instead. Neither is wrong; a follow-up
-- decision on one house rule for fr_CH would settle it.
--
