...
BEGIN
    ...
    UPDATE Mobile_Application_TRL ma_trl
    SET name        = ma_trl_effective.name,
        description = ma_trl_effective.description
    ...
    WHERE (p_Mobile_Application_ID IS NULL OR ma_trl_effective.mobile_application_id = p_Mobile_Application_ID)
      AND (p_AD_Language IS NULL OR ma_trl_effective.AD_Language = p_AD_Language)
      AND ma_trl.mobile_application_id = ma_trl_effective.mobile_application_id
      AND (ma_trl.name <> ma_trl_effective.name OR ma_trl.description <> ma_trl_effective.description);

    --
    GET DIAGNOSTICS update_count = ROW_COUNT;
    RAISE NOTICE 'UPDATE % Mobile_Application_TRL ROWS USING Mobile_Application_ID=%, AD_Language=%. The update is needed in case of Mobile_Application_Trl customizations', update_count, p_Mobile_Application_ID, p_AD_Language;
...
COMMENT ON FUNCTION public.update_Mobile_Application_TRLs(numeric, character varying) IS
 ...
 WHEN the Mobile_Application_TRL has customizations, the custom values should be copied into the Mobile_Application_TRL itself.'
;
