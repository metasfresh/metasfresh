...
--   * The Help names the picking list it actually covers. Two different documents are both
--     called "Pickliste": the standard one, printed by the "Pickliste drucken" action, and the
--     Traffic-Manager one, printed by "Pickliste drucken (Traffic Manager)". This feature prints
--     the block only on the standard one -- the Traffic-Manager list is a separate report with no
--     line-detail subreport and is not touched here -- so the Help says which of the two is meant
--     instead of promising both. Both labels are the AD action names, quoted as they appear.
