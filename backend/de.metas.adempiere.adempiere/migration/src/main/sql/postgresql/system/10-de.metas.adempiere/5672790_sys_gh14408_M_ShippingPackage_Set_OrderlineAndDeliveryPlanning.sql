-- PORT-FORWARD (NEW_DAWN WINS): the M_ShippingPackage.C_OrderLine_ID column is already
-- baked in the new_dawn_uat base under AD_Column_ID=590587 (master used 585599). The whole
-- C_OrderLine_ID create half (AD_Column + AD_Column_Trl + update_Column_Translation_From_AD_Element(561)
-- + db_alter_table ADD COLUMN + FK constraint) is stripped here to avoid the
-- "duplicate key on ad_column_name (540031, C_OrderLine_ID)" collision. The branch-only
-- 5809200_sys_Transport_Window_Changes.sql already binds the field to the base column 590587.
-- Only the M_Delivery_Planning_ID column (585600, genuinely absent in base) is kept below.
