...
BEGIN
    ...
    --
    -- Process Access: When the Role is Read Only (p_IsReadWrite = 'N') then only Jasper Report and Excel Export are added
    ...
    WHERE p.ad_process_id = ANY (p_AD_Process_IDs)
      ...
      AND (p_IsReadWrite = 'Y' OR (p_IsReadWrite = 'N' AND p.isreport = 'Y') OR (p_IsReadWrite = 'N' AND p.type = 'Excel'));
    --
    GET DIAGNOSTICS v_rowcount = ROW_COUNT;
