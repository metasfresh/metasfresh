-- DD_Order picking replenishment — add DD_Order.M_Picking_Job_Schedule_ID FK + partial index.
-- Links a DD_Order to the M_Picking_Job_Schedule workstation assignment that triggered it.
--
...
VALUES (592792 /*From ID Server*/, 0, 0, 'Y',
        TO_TIMESTAMP('2026-06-09 14:20:00','YYYY-MM-DD HH24:MI:SS'), 100,
        TO_TIMESTAMP('2026-06-09 14:20:00','YYYY-MM-DD HH24:MI:SS'), 100,
        0,
        ...
        'Kommissionierplan',
        'Kommissionierplan-Zuweisung (Arbeitsplatz), für die dieser DD_Order erzeugt wurde.',
        'Verknüpft den DD_Order mit der M_Picking_Job_Schedule-Zeile (Arbeitsplatz-Zuweisung), die ihn ausgelöst hat.',
        10, 'N', 'Y', 'N',
        NULL, 'D', 'N', 'N', 'N',
        'N', 'N', 'N', 'Y',
        'NP');
...
WHERE l.IsActive = 'Y' AND l.IsSystemLanguage = 'Y' AND t.AD_Column_ID = 592792
  AND NOT EXISTS (SELECT 1 FROM AD_Column_Trl tt WHERE tt.AD_Language = l.AD_Language AND tt.AD_Column_ID = t.AD_Column_ID);

-- =============================================================================
-- 2. Physical DDL — new nullable FK column
-- =============================================================================
ALTER TABLE DD_Order ADD COLUMN IF NOT EXISTS M_Picking_Job_Schedule_ID NUMERIC(10);
ALTER TABLE DD_Order
  ADD CONSTRAINT mpickingjobschedule_ddorder
  FOREIGN KEY (M_Picking_Job_Schedule_ID) REFERENCES M_Picking_Job_Schedule(M_Picking_Job_Schedule_ID)
  DEFERRABLE INITIALLY DEFERRED;

-- Partial index (most DD_Order rows will NOT have a M_Picking_Job_Schedule_ID; only picking-warehouse ones do)
CREATE INDEX IF NOT EXISTS dd_order_m_pickingjobschedule_idx
  ON DD_Order (M_Picking_Job_Schedule_ID)
