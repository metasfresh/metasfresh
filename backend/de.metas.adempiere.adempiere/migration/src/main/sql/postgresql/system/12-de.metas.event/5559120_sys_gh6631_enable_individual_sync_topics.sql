...
UPDATE AD_SysConfig SET Description='If a thread posts an event to the local event bus and this is Y, then by default the respective event listeners are enqueued and invoked within a dedicated worker-thread. 
...
-- I forgot to set the DICTIONARY_ID_COMMENTS System Configurator
INSERT INTO AD_SysConfig (AD_Client_ID,AD_Org_ID,AD_SysConfig_ID,ConfigurationLevel,Created,CreatedBy,Description,EntityType,IsActive,Name,Updated,UpdatedBy,Value) VALUES (0,0,541323,'S',TO_TIMESTAMP('2020-05-12 20:56:44','YYYY-MM-DD HH24:MI:SS'),100,'Tells if cache invalidation events in particular shall be processed synchronously. I think this should be Y because there are a lot of them and their processing is supposed to be real quick. 
So processing them synchronously in thread that posted the event should per-se introduce no performance bottle-neck.
If they are not processed quickly a huge (async) queue can build up quickly, and weird things happen (mostly UIs are not updated).
So in other words: having them synchronous shall be fine, and if not we need to know about it anyways because it directly affects the users'' experience.','de.metas.event','Y','de.metas.event.asyncEventBus.topic_de.topic_de.metas.cache.CacheInvalidationRemoteHandler',TO_TIMESTAMP('2020-05-12 20:56:44','YYYY-MM-DD HH24:MI:SS'),100,'Y')
;
