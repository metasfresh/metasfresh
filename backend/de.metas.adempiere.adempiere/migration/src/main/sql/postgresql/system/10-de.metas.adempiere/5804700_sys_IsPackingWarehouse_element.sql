-- Add IsPackingWarehouse to M_Warehouse
...
SET Name        = 'Picking Warehouse',
    PrintName   = 'Picking Warehouse',
    ...
    Help        = 'Marks this warehouse as a picking warehouse. For such warehouses a dedicated, idempotent reconcile flow is enabled that creates exactly one DD_Order per M_ShipmentSchedule. For the flow to function, MRP_Exclude=Y must also be set on the warehouse and DD_NetworkDistribution_ID must be populated.',
    IsTranslated = 'Y',
...
UPDATE AD_Element_Trl
SET Name        = 'Kommissionierungslager',
    PrintName   = 'Kommissionierungslager',
    ...
    Help        = 'Markiert dieses Lager als Kommissionierungslager. Für solche Lager wird ein dedizierter, idempotenter Reconcile-Flow aktiviert, der pro M_ShipmentSchedule genau einen DD_Order erzeugt. Damit der Flow korrekt funktioniert, muss am Lager zusätzlich MRP_Exclude=Ja gesetzt sein und ein DD_NetworkDistribution_ID hinterlegt sein.',
    IsTranslated = 'N',
