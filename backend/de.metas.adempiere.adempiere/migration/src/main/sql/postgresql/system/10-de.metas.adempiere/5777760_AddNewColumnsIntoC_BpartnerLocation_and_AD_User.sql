...
-- Element: IsDefaultVisitorAdress
...
/* DDL */ SELECT public.db_alter_table('C_BPartner_Location','ALTER TABLE public.C_BPartner_Location ADD COLUMN IsDefaultVisitorAdress CHAR(1) DEFAULT ''N'' CHECK (IsDefaultVisitorAdress IN (''Y'',''N'')) NOT NULL')
;

-- 2025-11-20T10:34:53.803Z
INSERT INTO AD_Index_Table (AD_Client_ID,AD_Index_Table_ID,AD_Org_ID,AD_Table_ID,BeforeChangeCode,BeforeChangeCodeType,BeforeChangeWarning,Created,CreatedBy,EntityType,ErrorMsg,IsActive,IsUnique,Name,Processing,Updated,UpdatedBy,WhereClause) VALUES (0,540839,0,293,'IsDefaultVisitorAdress=''N''','SQLS','Möchten sie wirklich die standard Rechnungsanschrift ändern?',TO_TIMESTAMP('2025-11-20 10:34:53.686000','YYYY-MM-DD HH24:MI:SS.US')::timestamp without time zone AT TIME ZONE 'UTC',100,'D','Only one default visitor address may be active at a time.','Y','Y','IsDefaultVisitorAdress','N',TO_TIMESTAMP('2025-11-20 10:34:53.686000','YYYY-MM-DD HH24:MI:SS.US')::timestamp without time zone AT TIME ZONE 'UTC',100,'IsDefaultVisitorAdress=''Y'' and isActive=''Y''')
;
...
CREATE UNIQUE INDEX IsDefaultVisitorAdress ON C_BPartner_Location (C_BPartner_ID,IsDefaultVisitorAdress) WHERE IsDefaultVisitorAdress='Y' and isActive='Y'
;

-- 2025-11-20T10:36:32.427Z
CREATE OR REPLACE FUNCTION IsDefaultVisitorAdress_tgfn()
 RETURNS TRIGGER AS $IsDefaultVisitorAdress_tg$
 BEGIN
 IF TG_OP='INSERT' THEN
UPDATE C_BPartner_Location SET IsDefaultVisitorAdress='N', Updated=NEW.Updated, UpdatedBy=NEW.UpdatedBy WHERE 1=1  AND C_BPartner_ID=NEW.C_BPartner_ID AND IsDefaultVisitorAdress=NEW.IsDefaultVisitorAdress AND C_BPartner_Location_ID<>NEW.C_BPartner_Location_ID AND IsDefaultVisitorAdress='Y' and isActive='Y';
 ELSE
IF OLD.C_BPartner_ID<>NEW.C_BPartner_ID OR OLD.IsDefaultVisitorAdress<>NEW.IsDefaultVisitorAdress THEN
UPDATE C_BPartner_Location SET IsDefaultVisitorAdress='N', Updated=NEW.Updated, UpdatedBy=NEW.UpdatedBy WHERE 1=1  AND C_BPartner_ID=NEW.C_BPartner_ID AND IsDefaultVisitorAdress=NEW.IsDefaultVisitorAdress AND C_BPartner_Location_ID<>NEW.C_BPartner_Location_ID AND IsDefaultVisitorAdress='Y' and isActive='Y';
 END IF;
 END IF;
 RETURN NEW;
 END;
 $IsDefaultVisitorAdress_tg$ LANGUAGE plpgsql;
;

-- 2025-11-20T10:36:32.430Z
DROP TRIGGER IF EXISTS IsDefaultVisitorAdress_tg ON C_BPartner_Location
;

-- 2025-11-20T10:36:32.431Z
CREATE TRIGGER IsDefaultVisitorAdress_tg BEFORE INSERT OR UPDATE  ON C_BPartner_Location FOR EACH ROW EXECUTE PROCEDURE IsDefaultVisitorAdress_tgfn()
;
