...
public class CreateInvoiceCandidatesService
{
	...
	private NewManualInvoiceCandidate createCandidate(@NonNull final JsonCreateInvoiceCandidatesRequestItem item)
	{
		...
		syncPaymentTermIdToCandidate(item, orgId, candidate);
	...
	private void syncPaymentTermIdToCandidate(final @NonNull JsonCreateInvoiceCandidatesRequestItem item,
											  final OrgId orgId,
											  final @NonNull NewManualInvoiceCandidateBuilder candidate)
	{
		...
		final PaymentTermId paymentTermId = paymentTermRepository.retrievePaymentTermId(PaymentTermQuery.builder()
						...
						.orgId(orgId)
						.build())
				.orElseThrow(() -> MissingResourceException.builder().resourceName("paymentTerm").resourceIdentifier(paymentTerm).parentResource(item).build());
