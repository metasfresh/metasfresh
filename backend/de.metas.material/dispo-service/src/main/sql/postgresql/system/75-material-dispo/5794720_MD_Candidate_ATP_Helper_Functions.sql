...
DROP FUNCTION IF EXISTS de_metas_material.MD_Candidate_Get_Stock_Impact(varchar, numeric);
DROP FUNCTION IF EXISTS de_metas_material.MD_Candidate_Get_Stock_Impact(varchar, varchar, numeric, numeric);
...
DROP FUNCTION IF EXISTS de_metas_material.MD_Candidate_Get_Effective_Qty(varchar, varchar, numeric, numeric);
...
DROP FUNCTION IF EXISTS de_metas_material.MD_Candidate_Find_Stock_ID(numeric, numeric, varchar);
...
BEGIN
    ...
    ELSE
        ...
        SELECT MD_Candidate_ID INTO v_stock_candidate_id
        FROM MD_Candidate
...
DROP FUNCTION IF EXISTS de_metas_material.MD_Candidate_Update_QtyDetails(numeric, numeric, numeric, numeric, numeric);
...
BEGIN
    ...
    ELSE
        ...
        IF p_previous_stock_candidate_id IS NOT NULL THEN
            INSERT INTO MD_Candidate_QtyDetails (
                MD_Candidate_QtyDetails_ID,
                AD_Client_ID,
                AD_Org_ID,
                Created,
                CreatedBy,
            ...
            SELECT
                nextval('MD_Candidate_QtyDetails_seq'),
                c.AD_Client_ID,
                c.AD_Org_ID,
                NOW(),
                99,
