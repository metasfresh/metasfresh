...
@Immutable
public final class DefaultESModelIndexer implements ESModelIndexer
{
	// services
	private static final transient Logger logger = LogManager.getLogger(DefaultESModelIndexer.class);
	...
	private final ImmutableList<DefaultESModelIndexer> includedModelIndexers;
	@Getter(AccessLevel.PRIVATE)
	...
	private DefaultESModelIndexer(
			...
			@NonNull @Singular final ImmutableList<DefaultESModelIndexer> includedModelIndexers,
	...
	private void createUpdateIndexTypeMapping()
	{
		...
		try
		{
			...
			logger.debug("Updated index mapping: {} \n {}", indexName, mapping);
		}
	...
	private void appendMapping_Properties(final XContentBuilder builder) throws IOException
	{
		...
		for (final DefaultESModelIndexer includedModelIndexer : includedModelIndexers)
		{
	...
	private List<Map<String, Object>> denormalizeIncludedForParent(final Object parentModel, final DefaultESModelIndexer includedModelIndexer)
	{
		...
		return queryBL.createQueryBuilder(includedModelIndexer.getModelTableName())
				...
				.create()
				.stream()
				.map(POBasedESModelToIndex::ofObject)
				.map(includedModel -> denormalizeIncludedModel(includedModel, includedModelIndexer))
				.collect(ImmutableList.toImmutableList());
	}

	private static Map<String, Object> denormalizeIncludedModel(final ESModelToIndex includedModel, final DefaultESModelIndexer includedModelIndexer)
	{
