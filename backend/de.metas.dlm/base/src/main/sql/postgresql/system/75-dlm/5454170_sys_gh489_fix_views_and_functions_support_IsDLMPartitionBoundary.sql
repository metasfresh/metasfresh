...
'Uses the view dlm.triggers to iterate "incoming" FK constraints for the given p_table_name table and creates a trigger&triggerfunction to avoid "dangling" references in case a record''s DLM_Level is increased. See the view dlm.triggers for more details.
