...
SELECT CustomerArticleNumber,
       ...
       QtyPattern,
       transp
FROM (SELECT COALESCE(bpp.ProductNo, p.value)                                                                             AS CustomerArticleNumber,
      ...
      FROM
           ...
           WHERE io.DocStatus = 'CO'
             ...
             AND io.isActive = 'Y') io
              ...
              LEFT OUTER JOIN (SELECT AVG(ic.PriceEntered_Override) AS PriceEntered_Override,
                               ...
                               GROUP BY ic.Price_UOM_ID, iol.M_InOutLine_ID, ol.line, o.documentno) ic ON iol.M_InOutLine_ID = ic.M_InOutLine_ID
          ...
          (SELECT STRING_AGG(DISTINCT name, E'\n' ORDER BY name) AS Name, M_InOutLine_ID
           FROM (SELECT DISTINCT
                 ...
                 WHERE piv.M_HU_PI_Version_ID != 101) x
           GROUP BY M_InOutLine_ID) pi ON iol.M_InOutLine_ID = pi.M_InOutLine_ID
              ...
              AND conv.isActive = 'Y'
              -- Attributes
              LEFT OUTER JOIN (SELECT STRING_AGG(ai_value, ', ' ORDER BY LENGTH(ai_value), ai_value) AS Attributes, M_AttributeSetInstance_ID
                               FROM Report.fresh_Attributes
                               WHERE at_value IN ('1000015', '1000001') -- Marke (ADR), Herkunft
                               GROUP BY M_AttributeSetInstance_ID) att ON iol.M_AttributeSetInstance_ID = att.M_AttributeSetInstance_ID
...
GROUP BY CustomerArticleNumber, name, attributes, HUName, UOMSymbol, QtyPattern, transp
ORDER BY CustomerArticleNumber, transp, name
$$
    LANGUAGE sql STABLE
;
