...
BEGIN
    ...
    -- -----------------------------------------------------------------------
    DROP TABLE IF EXISTS tmp_hu_destroy_parents;
    ...
    --         Skips HUs already Destroyed or Shipped.
    -- -----------------------------------------------------------------------
    DROP TABLE IF EXISTS tmp_hu_destroy_children;
    ...
    WHERE ht.m_hu_id NOT IN (SELECT m_hu_id FROM tmp_hu_destroy_parents)
      AND hu.hustatus NOT IN ('D', 'E');  -- skip already Destroyed and Shipped
