...
public class HUShipmentProcess_DropshipTest extends AbstractHUShipmentProcessIntegrationTest
{
	private ShipmentScheduleQtyPickedExpectations afterPick_ShipmentScheduleQtyPickedExpectations = null;
	private I_C_BPartner dropshipVendorBpartner;

	private I_C_BPartner_Location dropshipLocation;
	private I_C_BPartner billBpartner;
	private I_C_BPartner_Location billLocation;

	protected void initialize()
	{
		super.initialize();
		dropshipVendorBpartner = createBPartner("Vendor");
		dropshipLocation = createBPartnerLocation(dropshipVendorBpartner);

		billBpartner = createBPartner("Bill partner");
		billLocation = createBPartnerLocation(billBpartner);
	}
	...
	protected void step10_createShipmentSchedules()
	{
		...
		final I_M_ShipmentSchedule shipmentSchedule = createShipmentSchedule(/* newOrder */true, product, productUOM, qtyOrdered, 10/* orderLineNo */);
		shipmentSchedule.setIsDropShip(true);
		shipmentSchedule.setBill_BPartner_ID(billBpartner.getC_BPartner_ID());
		shipmentSchedule.setBill_Location_ID(billLocation.getC_BPartner_Location_ID());
		shipmentSchedule.setC_BPartner_Vendor_ID(dropshipVendorBpartner.getC_BPartner_ID());
		InterfaceWrapperHelper.save(shipmentSchedule);

		shipmentSchedules = Collections.singletonList(
				shipmentSchedule // shipment schedule 0
		);
	}
	...
	protected void step50_GenerateShipment_validateGeneratedShipments()
	{
		...
		assertThat(shipment.isDropShip()).isTrue();
		assertThat(shipment.getDropShip_BPartner_ID()).isEqualTo(bpartner.getC_BPartner_ID());
		assertThat(shipment.getDropShip_Location_ID()).isEqualTo(bpartnerLocation.getC_BPartner_Location_ID());
		assertThat(shipment.getC_BPartner_ID()).isEqualTo(billBpartner.getC_BPartner_ID());
		assertThat(shipment.getC_BPartner_Location_ID()).isEqualTo(billLocation.getC_BPartner_Location_ID());

		//
		// Retrieve generated shipment lines
		// We expect to have 1 shipment line
		final List<I_M_InOutLine> shipmentLines = Services.get(IInOutDAO.class).retrieveLines(shipment);
		assertThat(shipmentLines).as("Invalid generated shipment lines count").hasSize(1);
