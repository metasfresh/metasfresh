...
public class HUTransformServiceTests
{
	...
	class cuToNewTUs
	{
		@ParameterizedTest(name = "isOwnPackingMaterials={argumentsWithNames}")
		...
		public void testRealCU_To_NewTUs_1Tomato_TU_Capacity_2(final boolean isOwnPackingMaterials)
		{
			...
			final List<I_M_HU> newTUs = huTransformService
					.cuToNewTUs(
							cuToSplit,
							Quantity.of("1", data.helper.uomKg),
							data.piTU_Item_Product_Bag_8KgTomatoes,
							isOwnPackingMaterials);

			MatcherAssert.assertThat(newTUs.size(), is(1));

			final Node cuToSplitXML = HUXmlConverter.toXml(cuToSplit);
			MatcherAssert.assertThat(cuToSplitXML, hasXPath("HU-VirtualPI/@HUStatus", is("A")));
			MatcherAssert.assertThat(cuToSplitXML, hasXPath("count(HU-VirtualPI/Storage[@M_Product_Value='Tomato' and @Qty='39.000' and @C_UOM_Name='Kg'])", is("1")));

			final Node newTUXML = HUXmlConverter.toXml(newTUs.get(0));

			MatcherAssert.assertThat(newTUXML, hasXPath("HU-TU_Bag/@HUStatus", is("A")));
			MatcherAssert.assertThat(newTUXML, hasXPath("HU-TU_Bag/@HUPlanningReceiptOwnerPM", is(Boolean.toString(isOwnPackingMaterials))));
			MatcherAssert.assertThat(newTUXML, hasXPath("count(HU-TU_Bag/Storage[@M_Product_Value='Tomato' and @Qty='1.000' and @C_UOM_Name='Kg'])", is("1")));
		}
		...
		public void testRealCU_To_NewTUs_1Tomato_TU_Capacity_40(final boolean isOwnPackingMaterials)
		{
			...
			final Node cuToSplitXML = HUXmlConverter.toXml(cuToSplit);
			MatcherAssert.assertThat(cuToSplitXML, hasXPath("count(HU-VirtualPI[@HUStatus='D'])", is("1")));
			MatcherAssert.assertThat(cuToSplitXML, hasXPath("count(HU-VirtualPI/Storage[@M_Product_Value='Tomato' and @Qty='0.000' and @C_UOM_Name='Kg'])", is("1")));

			final Node newTUXML = HUXmlConverter.toXml(newTUs.get(0));

			MatcherAssert.assertThat(newTUXML, hasXPath("count(HU-TU_IFCO[@HUStatus='A'])", is("1")));
			MatcherAssert.assertThat(newTUXML, hasXPath("string(HU-TU_IFCO/@HUPlanningReceiptOwnerPM)", is(Boolean.toString(isOwnPackingMaterials))));
			MatcherAssert.assertThat(newTUXML, hasXPath("count(HU-TU_IFCO/Storage[@M_Product_Value='Tomato' and @Qty='2.000' and @C_UOM_Name='Kg'])", is("1")));
		}
		...
		public void testRealCU_To_NewTUs_40Tomatoes_TU_Capacity_8(final boolean isOwnPackingMaterials)
		{
			...
			MatcherAssert.assertThat(newTUs.size(), is(5));
			...
			for (final I_M_HU newTU : newTUs)
			{
				...
				MatcherAssert.assertThat(newTUXML, hasXPath("count(HU-TU_Bag[@HUStatus='A'])", is("1")));
				MatcherAssert.assertThat(newTUXML, hasXPath("string(HU-TU_Bag/@HUPlanningReceiptOwnerPM)", is(Boolean.toString(isOwnPackingMaterials))));
				MatcherAssert.assertThat(newTUXML, hasXPath("count(HU-TU_Bag/Storage[@M_Product_Value='Tomato' and @Qty='8.000' and @C_UOM_Name='Kg'])", is("1")));
			}
		}
	}
	...
	class cuToExistingTU
	{
		...
		public void testRealCU_To_ExistingRealTU()
		{
			...
			MatcherAssert.assertThat(existingTUs.size(), is(1));
			final I_M_HU existingTU = existingTUs.get(0);
			MatcherAssert.assertThat(handlingUnitsBL.isAggregateHU(existingTU), is(false));

			final Node existingTUBeforeXML = HUXmlConverter.toXml(existingTU);
			MatcherAssert.assertThat(existingTUBeforeXML, not(hasXPath("HU-TU_IFCO/M_HU_Item_Parent_ID"))); // verify that there is still no parent HU
			MatcherAssert.assertThat(existingTUBeforeXML, hasXPath("count(HU-TU_IFCO[@HUStatus='A'])", is("1")));
			MatcherAssert.assertThat(existingTUBeforeXML, hasXPath("count(HU-TU_IFCO/Storage[@M_Product_Value='Tomato' and @Qty='20.000' and @C_UOM_Name='Kg'])", is("1")));
			...
			MatcherAssert.assertThat(cuToSplitXML, hasXPath("count(HU-VirtualPI/Storage[@M_Product_Value='Tomato' and @Qty='20.000' and @C_UOM_Name='Kg'])", is("1")));
			...
			MatcherAssert.assertThat(existingTUXML, hasXPath("count(HU-TU_IFCO/Storage[@M_Product_Value='Tomato' and @Qty='40.000' and @C_UOM_Name='Kg'])", is("1")));
		}
		...
		public void testRealCU_To_ExistingRealTU_overfill()
		{
			...
			final I_M_HU existingTU;
			{
				...
				MatcherAssert.assertThat(existingTUBeforeXML, not(hasXPath("HU-TU_IFCO/M_HU_Item_Parent_ID"))); // verify that there is still no parent HU
				MatcherAssert.assertThat(existingTUBeforeXML, hasXPath("count(HU-TU_IFCO[@HUStatus='A'])", is("1")));
				MatcherAssert.assertThat(existingTUBeforeXML, hasXPath("count(HU-TU_IFCO/Storage[@M_Product_Value='Tomato' and @Qty='30.000' and @C_UOM_Name='Kg'])", is("1")));
			}
			// prepare the CU to split
			final I_M_HU cuToSplit = testsBase.getData().mkRealStandAloneCuWithCuQty("20");
			...
			MatcherAssert.assertThat(existingTUXML, not(hasXPath("HU-TU_IFCO/M_HU_Item_Parent_ID"))); // verify that there is still no parent HU
			MatcherAssert.assertThat(existingTUXML, hasXPath("count(HU-TU_IFCO[@HUStatus='A'])", is("1")));
			MatcherAssert.assertThat(existingTUXML, hasXPath("string(HU-TU_IFCO/Storage[@M_Product_Value='Tomato' and @C_UOM_Name='Kg']/@Qty)", is("50.000")));
		...
		public void testRealCU_To_ExistingAggregateTU()
		{
			...
			final Node existingTUBeforeXML = HUXmlConverter.toXml(existingTU);
			MatcherAssert.assertThat(existingTUBeforeXML, hasXPath("string(HU-VirtualPI/@HUStatus)", is("A")));
			MatcherAssert.assertThat(existingTUBeforeXML, hasXPath("string(HU-VirtualPI/Storage[@M_Product_Value='Tomato' and @C_UOM_Name='Kg']/@Qty)", is("80.000")));
			...
			final Node cuToSplitXML = HUXmlConverter.toXml(cuToSplit);
			MatcherAssert.assertThat(cuToSplitXML, hasXPath("string(HU-VirtualPI/@HUStatus)", is("D")));
			MatcherAssert.assertThat(cuToSplitXML, hasXPath("string(HU-VirtualPI/Storage[@M_Product_Value='Tomato' and @C_UOM_Name='Kg']/@Qty)", is("0.000")));

			// the existing TU to which we wanted to add stuff is unchanged, but it now has a "real-TU" sibling
			final Node existingTUXML = HUXmlConverter.toXml(existingTU);
			MatcherAssert.assertThat(existingTUXML, hasXPath("string(HU-VirtualPI/@HUStatus)", is("A")));
			MatcherAssert.assertThat(existingTUXML, hasXPath("string(HU-VirtualPI/Storage[@M_Product_Value='Tomato' and @C_UOM_Name='Kg']/@Qty)", is("80.000")));
			...
			MatcherAssert.assertThat(fullTargetHUXML, hasXPath("string(HU-LU_Palet/Item[@ItemType='HA']/HU-VirtualPI/@M_HU_ID)", is(Integer.toString(existingTU.getM_HU_ID())))); // fullTargetHU contains existingTU
			MatcherAssert.assertThat(fullTargetHUXML, hasXPath("string(HU-LU_Palet/Item[@ItemType='HU']/HU-TU_IFCO/Storage[@M_Product_Value='Tomato' and @C_UOM_Name='Kg']/@Qty)", is("20.000"))); // fullTargetHU also contains a real IFCO with 20
		...
		public void test_CUToExistingTU_create_mixed_TU_partialCU()
		{
			...
			MatcherAssert.assertThat(cu1.getHUStatus(), is(X_M_HU.HUSTATUS_Active));

			final LUTUProducerDestinationTestSupport data = testsBase.getData();

			final I_M_HU existingTU = testsBase.retrieveParent(cu1);
			MatcherAssert.assertThat(existingTU.getHUStatus(), is(X_M_HU.HUSTATUS_Active));
			...
			MatcherAssert.assertThat(secondCUXML, hasXPath("HU-VirtualPI[@M_HU_ID=" + cu2.getM_HU_ID() + "]/@HUStatus", is("A")));
			MatcherAssert.assertThat(secondCUXML, hasXPath("HU-VirtualPI[@M_HU_ID=" + cu2.getM_HU_ID() + "]/Storage[@M_Product_Value='Salad' and @C_UOM_Name='Kg']/@Qty", is("1.400")));

			final Node existingLUXML = HUXmlConverter.toXml(existingTU);
			MatcherAssert.assertThat(existingLUXML, hasXPath("string(HU-TU_IFCO/Storage[@M_Product_Value='Tomato' and @C_UOM_Name='Kg']/@Qty)", is("2.000")));
			MatcherAssert.assertThat(existingLUXML, hasXPath("string(HU-TU_IFCO/Storage[@M_Product_Value='Salad' and @C_UOM_Name='Kg']/@Qty)", is("1.600")));
			MatcherAssert.assertThat(existingLUXML, hasXPath("string(HU-TU_IFCO/Item[@ItemType='MI']/Storage[@M_Product_Value='Tomato' and @C_UOM_Name='Kg']/@Qty)", is("2.000")));
			MatcherAssert.assertThat(existingLUXML, hasXPath("string(HU-TU_IFCO/Item[@ItemType='MI']/Storage[@M_Product_Value='Salad' and @C_UOM_Name='Kg']/@Qty)", is("1.600")));
			MatcherAssert.assertThat(existingLUXML, hasXPath("string(HU-TU_IFCO/Item[@ItemType='MI']/HU-VirtualPI[@M_HU_ID=" + cu1.getM_HU_ID() + "]/Storage[@M_Product_Value='Tomato' and @C_UOM_Name='Kg']/@Qty)", is("2.000")));
			MatcherAssert.assertThat(existingLUXML, hasXPath("string(HU-TU_IFCO/Item[@ItemType='MI']/HU-VirtualPI/Storage[@M_Product_Value='Salad' and @C_UOM_Name='Kg']/@Qty)", is("1.600")));
		}
		...
		public void test_CUToExistingTU_create_mixed_TU_completeCU()
		{
			...
			MatcherAssert.assertThat(tuWithMixedCUsXML, hasXPath("string(HU-TU_IFCO/Storage[@M_Product_Value='Tomato' and @C_UOM_Name='Kg']/@Qty)", is("5.000")));
			MatcherAssert.assertThat(tuWithMixedCUsXML, hasXPath("string(HU-TU_IFCO/Storage[@M_Product_Value='Salad' and @C_UOM_Name='Kg']/@Qty)", is("4.000")));

			MatcherAssert.assertThat(tuWithMixedCUsXML, hasXPath("count(HU-TU_IFCO/Item[@ItemType='MI']/HU-VirtualPI[@M_HU_ID=" + cu1.getM_HU_ID() + "])", is("1")));
			MatcherAssert.assertThat(tuWithMixedCUsXML, hasXPath("string(HU-TU_IFCO/Item[@ItemType='MI']/HU-VirtualPI[@M_HU_ID=" + cu1.getM_HU_ID() + "]/Storage[@M_Product_Value='Tomato' and @C_UOM_Name='Kg']/@Qty)", is("5.000")));

			MatcherAssert.assertThat(tuWithMixedCUsXML, hasXPath("count(HU-TU_IFCO/Item[@ItemType='MI']/HU-VirtualPI[@M_HU_ID=" + cu2.getM_HU_ID() + "])", is("1")));
			MatcherAssert.assertThat(tuWithMixedCUsXML, hasXPath("string(HU-TU_IFCO/Item[@ItemType='MI']/HU-VirtualPI[@M_HU_ID=" + cu2.getM_HU_ID() + "]/Storage[@M_Product_Value='Salad' and @C_UOM_Name='Kg']/@Qty)", is("4.000")));
		}
	}
	...
	class tuToNewTUs
	{
		...
		public void testAggregateTU_To_NewTUs_MaxValueParent()
		{
			...
			MatcherAssert.assertThat(testsBase.retrieveParentItem(tuToSplit), notNullValue()); // guard: tuToSplit shall have a parent

			// invoke the method under test
			final List<I_M_HU> newTUs = HUTransformService.newInstance(testsBase.getData().helper.getHUContext())
					.tuToNewTUs(tuToSplit, QtyTU.ofInt(4)); // tuQty=4; we only have 2 TUs in the source
			MatcherAssert.assertThat(newTUs.size(), is(2));

			MatcherAssert.assertThat(testsBase.retrieveParentItem(newTUs.get(0)), nullValue());
			MatcherAssert.assertThat(testsBase.retrieveParentItem(newTUs.get(1)), nullValue());
		}
		...
		public void testAggregateTU_To_NewTUs()
		{
			...
			final List<I_M_HU> newTUs = HUTransformService.newInstance(testsBase.getData().helper.getHUContext())
					.tuToNewTUs(tuToSplit,
							QtyTU.ofInt(1)); // tuQty=1; we have 2 TUs in the source, so we will will only expect 1x40 to be actually loaded
			MatcherAssert.assertThat(newTUs.size(), is(1));

			final Node newTUXML = HUXmlConverter.toXml(newTUs.get(0));
			MatcherAssert.assertThat(newTUXML, not(hasXPath("HU-TU_IFCO/M_HU_Item_Parent_ID"))); // verify that there is no parent HU
			MatcherAssert.assertThat(newTUXML, hasXPath("string(HU-TU_IFCO/Storage[@M_Product_Value='Tomato' and @C_UOM_Name='Kg']/@Qty)", is("40.000")));
		}
		...
		public void test_TakeOutTUsFromCustomLU()
		{
			...
			MatcherAssert.assertThat(data.piLU_Item_IFCO.getQty(), not(comparesEqualTo(BigDecimal.valueOf(13))));
			...
			for (final I_M_HU newTU : newTUs)
			{
				final Node newTUXML = HUXmlConverter.toXml(newTU);
				MatcherAssert.assertThat(newTUXML, not(hasXPath("HU-TU_IFCO/M_HU_Item_Parent_ID"))); // verify that there is no parent HU
				MatcherAssert.assertThat(newTUXML, hasXPath("string(HU-TU_IFCO/Storage[@M_Product_Value='Tomato' and @C_UOM_Name='Kg']/@Qty)", is("13.000")));
			}

		}

		/**
		 * Verifies the nothing is changed if {@link HUTransformService#tuToNewTUs(I_M_HU, QtyTU)}  is run with the source TU's full qty or more.
		 */
		@Test
		public void testRealTU_To_NewTUs_MaxValue()
		{
			...
			MatcherAssert.assertThat(tusToSplit.size(), is(1));
			final I_M_HU tuToSplit = tusToSplit.get(0);
			MatcherAssert.assertThat(handlingUnitsBL.isAggregateHU(tuToSplit), is(false)); // guard; make sure it's "real"

			// invoke the method under test
			final List<I_M_HU> newTUs = huTransformService
					.tuToNewTUs(tuToSplit,
							QtyTU.ofInt(4)); // tuQty=4; we only have 1 TU in the source which only holds 20kg
		...
		public void testAggregateSingleLUFullyLoaded_non_int()
		{
			...
			MatcherAssert.assertThat(lutuProducer.getCreatedHUs().size(), is(1));
			...
			MatcherAssert.assertThat(aggregateTUs.size(), is(1));
			final I_M_HU aggregateTU = aggregateTUs.get(0);
			MatcherAssert.assertThat(handlingUnitsBL.isAggregateHU(aggregateTU), is(true));

			final List<I_M_HU> newTUs = huTransformService.tuToNewTUs(aggregateTU, QtyTU.ONE);
			MatcherAssert.assertThat(newTUs.size(), is(1));
			final I_M_HU newTU = newTUs.get(0);
			// data.helper.commitAndDumpHU(newTU);

			final Node newTuXML = HUXmlConverter.toXml(newTU);
			MatcherAssert.assertThat(newTuXML, hasXPath("string(HU-TU_IFCO/Storage[@M_Product_Value='Tomato' and @C_UOM_Name='Kg']/@Qty)", is("5.470")));
			...
			MatcherAssert.assertThat(createdLuXML, hasXPath("count(HU-LU_Palet/Item[@ItemType='HU'])", is("0")));

			// the aggregate HU shall contain the full remaining quantity and represent 19 IFCOs; 5.47 x 19 = 103,93
			MatcherAssert.assertThat(createdLuXML, hasXPath("string(HU-LU_Palet/Storage[@M_Product_Value='Tomato' and @C_UOM_Name='Kg']/@Qty)", is("103.930")));

			MatcherAssert.assertThat(createdLuXML, hasXPath("string(HU-LU_Palet/Item[@ItemType='HA']/@Qty)", is("19")));
			MatcherAssert.assertThat(createdLuXML, hasXPath("string(HU-LU_Palet/Item[@ItemType='HA']/HU-VirtualPI/Storage[@M_Product_Value='Tomato' and @C_UOM_Name='Kg']/@Qty)", is("103.930")));
		}
	}
	...
	class tuToNewLUs
	{
		@ParameterizedTest(name = "isOwnPackingMaterials={argumentsWithNames}")
		@ValueSource(booleans = { true, false })
		public void testRealTU_To_NewLUs(final boolean isOwnPackingMaterials)
		{
			final LUTUProducerDestinationTestSupport data = testsBase.getData();
			...
			final I_M_HU lu; // the parent LU of the TU to split;
			{
				...
				{
					MatcherAssert.assertThat(lus.size(), is(1));
					lu = lus.get(0);
					final List<I_M_HU> includedHUs = testsBase.retrieveIncludedHUs(lu);
					MatcherAssert.assertThat(includedHUs.size(), is(1));
					MatcherAssert.assertThat(includedHUs.get(0).getM_HU_ID(), is(tuToSplit.getM_HU_ID()));

					MatcherAssert.assertThat(tuToSplit.getM_HU_Item_Parent().getM_HU_ID(), is(lu.getM_HU_ID()));
				}
			}
			...
			MatcherAssert.assertThat(newTUs.size(), is(1)); // we transfer 20kg, one IFCO holds 40kg, so we expect 1 IFCO
			MatcherAssert.assertThat(newTUs.get(0).getM_HU_ID(), is(tuToSplit.getM_HU_ID()));
			MatcherAssert.assertThat(newTUs.get(0).getM_HU_Item_Parent(), nullValue());

			MatcherAssert.assertThat(lu.getHUStatus(), is("D"));
			MatcherAssert.assertThat(testsBase.retrieveIncludedHUs(lu).isEmpty(), is(true));
		}
		...
		{
			final I_M_HU tuToSplit = testsBase.getData().mkAggregateHUWithTotalQtyCU("80");
			MatcherAssert.assertThat(handlingUnitsBL.isAggregateHU(tuToSplit), is(true)); // guard; make sure it's aggregate
			MatcherAssert.assertThat(tuToSplit.getHUStatus(), is(X_M_HU.HUSTATUS_Active));
			...
			final List<I_M_HU> newLUs = HUTransformService
					...
					.tuToNewLUs(tuToSplit,
							QtyTU.ofInt(4), // tuQty=4; we only have 2 TUs in the source which hold 40kg each, so we will will expect 2x40 to be actually loaded
							data.piLU_Item_IFCO,
							isOwnPackingMaterials);

			MatcherAssert.assertThat(newLUs.size(), is(1)); // we transfered 80kg, the target TUs are still IFCOs one IFCO still holds 40kg, one LU holds 5 IFCOS, so we expect one LU to suffice

			final Node luXML = HUXmlConverter.toXml(newLUs.get(0));
			MatcherAssert.assertThat(luXML, not(hasXPath("HU-LU_Palet/M_HU_Item_Parent_ID"))); // verify that the LU has no parent HU
			MatcherAssert.assertThat(luXML, hasXPath("HU-LU_Palet/@HUStatus", is("A"))); // gh #1975
			MatcherAssert.assertThat(luXML, hasXPath("HU-LU_Palet/@HUPlanningReceiptOwnerPM", is(Boolean.toString(isOwnPackingMaterials))));
			MatcherAssert.assertThat(luXML, hasXPath("HU-LU_Palet/Storage[@M_Product_Value='Tomato' and @C_UOM_Name='Kg']/@Qty", is("80.000")));

			// the pallet's included aggregate HU is 'tuToSplit'
			MatcherAssert.assertThat(luXML, hasXPath("HU-LU_Palet/Item[@ItemType='HA']/HU-VirtualPI/@HUStatus", is("A"))); // gh #1975
			MatcherAssert.assertThat(luXML, hasXPath("HU-LU_Palet/Item[@ItemType='HA']/HU-VirtualPI/@M_HU_ID", is(Integer.toString(tuToSplit.getM_HU_ID()))));
			MatcherAssert.assertThat(luXML, hasXPath("HU-LU_Palet/Item[@ItemType='HA']/@M_HU_PI_Item_ID", is(Integer.toString(data.piLU_Item_IFCO.getM_HU_PI_Item_ID()))));

			MatcherAssert.assertThat(luXML, hasXPath("count(HU-LU_Palet/Item[@ItemType='HA' and @Qty='2'])", is("1")));
			MatcherAssert.assertThat(luXML, hasXPath("HU-LU_Palet/Item[@ItemType='HA' and @Qty='2']/Storage[@M_Product_Value='Tomato' and @C_UOM_Name='Kg']/@Qty", is("80.000")));
		}
		...
		public void testAggregateTU_To_MultipleNewLUs(final boolean isOwnPackingMaterials)
		{
			...
			final List<I_M_HU> newLUs = huTransformService
					.tuToNewLUs(tuToSplit,
							QtyTU.ofInt(6), // tuQty=6;
							data.piLU_Item_IFCO,
							isOwnPackingMaterials);

			MatcherAssert.assertThat(newLUs.size(), is(2)); // we have 6 TUs in the source; one pallet can old 5 IFCOS, to we expect two pallets.
			{
				final Node lu1XML = HUXmlConverter.toXml(newLUs.get(0));

				MatcherAssert.assertThat(lu1XML, hasXPath("HU-LU_Palet/@HUStatus", is("A"))); // gh #1975
				MatcherAssert.assertThat(lu1XML, not(hasXPath("HU-LU_Palet/M_HU_Item_Parent_ID"))); // verify that the LU has no parent HU
				MatcherAssert.assertThat(lu1XML, hasXPath("HU-LU_Palet/@HUPlanningReceiptOwnerPM", is(Boolean.toString(isOwnPackingMaterials))));

				MatcherAssert.assertThat(lu1XML, hasXPath("HU-LU_Palet/Item[@ItemType='HA']/@M_HU_PI_Item_ID", is(Integer.toString(data.piLU_Item_IFCO.getM_HU_PI_Item_ID()))));
				MatcherAssert.assertThat(lu1XML, hasXPath("HU-LU_Palet/Item[@ItemType='HA']/HU-VirtualPI/@HUStatus", is("A"))); // gh #1975
				MatcherAssert.assertThat(lu1XML, hasXPath("HU-LU_Palet/Storage[@M_Product_Value='Tomato' and @C_UOM_Name='Kg']/@Qty", is("200.000")));
				MatcherAssert.assertThat(lu1XML, hasXPath("count(HU-LU_Palet/Item[@ItemType='HA' and @Qty='5'])", is("1")));
				MatcherAssert.assertThat(lu1XML, hasXPath("HU-LU_Palet/Item[@ItemType='HA' and @Qty='5']/Storage[@M_Product_Value='Tomato' and @C_UOM_Name='Kg']/@Qty", is("200.000")));
			}
			{
				final Node lu2XML = HUXmlConverter.toXml(newLUs.get(1));

				MatcherAssert.assertThat(lu2XML, hasXPath("HU-LU_Palet/@HUStatus", is("A"))); // gh #1975
				MatcherAssert.assertThat(lu2XML, not(hasXPath("HU-LU_Palet/M_HU_Item_Parent_ID"))); // verify that the LU has no parent HU
				MatcherAssert.assertThat(lu2XML, hasXPath("HU-LU_Palet/@HUPlanningReceiptOwnerPM", is(Boolean.toString(isOwnPackingMaterials))));

				MatcherAssert.assertThat(lu2XML, hasXPath("HU-LU_Palet/Storage[@M_Product_Value='Tomato' and @C_UOM_Name='Kg']/@Qty", is("40.000")));

				MatcherAssert.assertThat(lu2XML, hasXPath("HU-LU_Palet/Item[@ItemType='HA']/HU-VirtualPI/@HUStatus", is("A"))); // gh #1975
			}
		}
		...
		public void testRealStandaloneTU_To_NewLU(final boolean isOwnPackingMaterials)
		{
			...
			final List<I_M_HU> newLUs = huTransformService
					.tuToNewLUs(tuToSplit,
							QtyTU.ofInt(4), // tuQty=4; we only have 1 TU in the source which only holds 20kg, so we will expect the TU to be moved
							data.piLU_Item_IFCO,
							isOwnPackingMaterials);

			MatcherAssert.assertThat(newLUs.size(), is(1)); // we transfered 20kg, the target TUs are still IFCOs one IFCO still holds 40kg, one LU holds 5 IFCOS, so we expect one LU with one IFCO to suffice
			...
			MatcherAssert.assertThat(newLUXML, not(hasXPath("HU-LU_Palet/M_HU_Item_Parent_ID"))); // verify that the LU has no parent HU
			MatcherAssert.assertThat(newLUXML, hasXPath("string(HU-LU_Palet/@HUPlanningReceiptOwnerPM)", is(Boolean.toString(isOwnPackingMaterials))));

			MatcherAssert.assertThat(newLUXML, hasXPath("string(HU-LU_Palet/Item[@ItemType='HU']/@M_HU_PI_Item_ID)", is(Integer.toString(data.piLU_Item_IFCO.getM_HU_PI_Item_ID()))));
			MatcherAssert.assertThat(newLUXML, hasXPath("string(HU-LU_Palet/Item[@ItemType='HU']/HU-TU_IFCO/@M_HU_ID)", is(Integer.toString(tuToSplit.getM_HU_ID()))));

			MatcherAssert.assertThat(newLUXML, hasXPath("string(HU-LU_Palet/Storage[@M_Product_Value='Tomato' and @C_UOM_Name='Kg']/@Qty)", is("20.000")));
			MatcherAssert.assertThat(newLUXML, hasXPath("string(HU-LU_Palet/Item[@ItemType='HU']/HU-TU_IFCO/Storage[@M_Product_Value='Tomato' and @C_UOM_Name='Kg']/@Qty)", is("20.000")));
		}
		...
		public void testRealTUwithLU_To_NewLU(final boolean isOwnPackingMaterials)
		{
			...
			final List<I_M_HU> oldLUs = HUTransformService
					.newInstance(data.helper.getHUContext())
					.tuToNewLUs(tuToSplit, QtyTU.ONE, data.piLU_Item_IFCO, isOwnPackingMaterials);
			MatcherAssert.assertThat(oldLUs.size(), is(1)); // guard
			MatcherAssert.assertThat(tuToSplit.getM_HU_Item_Parent().getM_HU_ID(), is(oldLUs.get(0).getM_HU_ID()));
			MatcherAssert.assertThat(oldLUs.get(0).getHUStatus(), is(X_M_HU.HUSTATUS_Active));
			...
			MatcherAssert.assertThat(oldLUs.get(0).getHUStatus(), is(X_M_HU.HUSTATUS_Destroyed));
	...
	class tuToExistingLU
	{
		...
		public void testAggregateTU_to_existingLU_withRealTU()
		{
			...
			final LUTUProducerDestinationTestSupport data = testsBase.getData();
			{
				...
				MatcherAssert.assertThat(handlingUnitsBL.isAggregateHU(exitingTu), is(false)); // guard; make sure it's "real"

				final List<I_M_HU> existingLUs = HUTransformService.newInstance(localHuContextCopy)
						.tuToNewLUs(exitingTu,
								QtyTU.ofInt(4), // tuQty=4; we only have 1 TU in the source which only holds 20kg, so we will will expect 1x20 to be actually loaded
								data.piLU_Item_IFCO,
								false);
				MatcherAssert.assertThat(existingLUs.size(), is(1));
				...
				MatcherAssert.assertThat(existingLUXML, hasXPath("string(HU-LU_Palet/Item[@ItemType='HU']/HU-TU_IFCO/Storage[@M_Product_Value='Tomato' and @C_UOM_Name='Kg']/@Qty)", is("20.000")));
			}
			...
			huTransformService
					.tuToExistingLU(tuToSplit,
							QtyTU.ofInt(4), // tuQty=4; we only have 2 TU in the source which hold 40kg each, so we will will expect 2x40 to be actually loaded
							existingLU);

			// we had 20 and loaded 80, so we now expect 100
			final Node existingLUXML = HUXmlConverter.toXml(existingLU);
			MatcherAssert.assertThat(existingLUXML, hasXPath("string(HU-LU_Palet/Storage[@M_Product_Value='Tomato' and @C_UOM_Name='Kg']/@Qty)", is("100.000")));
		...
		public void testAggregateTU_To_existingLU_withAggregateTU()
		{
			...
			final IMutableHUContext huContext = data.helper.getHUContext();
			{
				...
				MatcherAssert.assertThat(handlingUnitsBL.isAggregateHU(exitingTu), is(true)); // guard; make sure it's "aggregate"
				...
				final List<I_M_HU> existingLUs = HUTransformService.newInstance(localCopy)
						.tuToNewLUs(exitingTu,
								QtyTU.ofInt(4), // tuQty=4; we only have 2 TUs in the source which only holds 80kg, so we will will expect 2x40 to be actually loaded onto one LU
								data.piLU_Item_IFCO,
								false);
				MatcherAssert.assertThat(existingLUs.size(), is(1));
				// data.helper.commitAndDumpHU(existingLUs.get(0));
				...
				// guard: the contained TU is "real"
				final Node existingLUXML = HUXmlConverter.toXml(existingLU);
				MatcherAssert.assertThat(existingLUXML, hasXPath("string(HU-LU_Palet/Storage[@M_Product_Value='Tomato' and @C_UOM_Name='Kg']/@Qty)", is("80.000"))); // the LU has 80kg
				MatcherAssert.assertThat(existingLUXML, hasXPath("string(HU-LU_Palet/Item[@ItemType='HA']/Storage[@M_Product_Value='Tomato' and @C_UOM_Name='Kg']/@Qty)", is("80.000"))); // those 80kg are contained in one aggreagate HU

				// that aggregate HU represents two IFCOS
				MatcherAssert.assertThat(existingLUXML, hasXPath("string(HU-LU_Palet/Item[@ItemType='HA']/@Qty)", is("2")));
				MatcherAssert.assertThat(existingLUXML, hasXPath("string(HU-LU_Palet/Item[@ItemType='HA']/@M_HU_PI_Item_ID)", is(Integer.toString(data.piLU_Item_IFCO.getM_HU_PI_Item_ID()))));
			}
			...
			final Node existingLUXML = HUXmlConverter.toXml(existingLU);
			MatcherAssert.assertThat(existingLUXML, hasXPath("string(HU-LU_Palet/Storage[@M_Product_Value='Tomato' and @C_UOM_Name='Kg']/@Qty)", is("160.000")));
			// the original aggreagate HU is still intact
			MatcherAssert.assertThat(existingLUXML, hasXPath("string(HU-LU_Palet/Item[@ItemType='HA']/@Qty)", is("2")));
			MatcherAssert.assertThat(existingLUXML, hasXPath("string(HU-LU_Palet/Item[@ItemType='HA']/@M_HU_PI_Item_ID)", is(Integer.toString(data.piLU_Item_IFCO.getM_HU_PI_Item_ID()))));

			// the aggregate 80kg TU which we moved in was de-aggregated into two 40kg TUs
			MatcherAssert.assertThat(existingLUXML, hasXPath("count(HU-LU_Palet/Item[@ItemType='HU']/HU-TU_IFCO/Storage[@M_Product_Value='Tomato' and @C_UOM_Name='Kg' and @Qty='40.000'])", is("2")));
			// data.helper.commitAndDumpHU(existingLU);
		}
	}
	...
	class husToNewCUs
	{
		...
		public void husToNewCUs_mixed_source_HU()
		{
			...
			final HUsToNewCUsRequest husToNewCUsRequest = HUsToNewCUsRequest.builder()
					...
					.qtyCU(Quantity.of("1", data.helper.uomKg))
			...
			assertThat(tuWithMixedCUs)
					.hasStorage(data.helper.pSaladProductId, Quantity.of("3", data.helper.uomKg))
			...
			assertThat(newSaladCU).hasStorage(data.helper.pSaladProductId, Quantity.of("1", data.helper.uomKg));
		}
		...
		public void husToNewCUs_with_aggregate_source_multiple_TUs()
		{
			...
			final HUsToNewCUsRequest husToNewCUsRequest = HUsToNewCUsRequest.builder()
					.sourceHU(topLevelParent)
					.productId(data.helper.pTomatoProductId)
					.qtyCU(Quantity.of("11", data.helper.uomKg)) // i.e. two TUs and one CU
					.build();
			...
			// the LU now has an aggregate (ItemType='HA'), representing 99 TUs with 5 CUs each and a "real" (ItemType='HU') TU with 4
			MatcherAssert.assertThat(existingTUXML, hasXPath("HU-LU_Palet/Storage[@M_Product_Value='Tomato' and @C_UOM_Name='Kg']/@Qty", is("489.000")));
			MatcherAssert.assertThat(existingTUXML, hasXPath("HU-LU_Palet/Item[@ItemType='HA']/Storage[@M_Product_Value='Tomato' and @C_UOM_Name='Kg']/@Qty", is("485.000")));
			MatcherAssert.assertThat(existingTUXML, hasXPath("HU-LU_Palet/Item[@ItemType='HU']/HU-TU_IFCO/Storage[@M_Product_Value='Tomato' and @C_UOM_Name='Kg']/@Qty", is("4.000")));

			assertThat(newCUs).hasSize(3);

			final Node newCuXML = HUXmlConverter.toXml("HUs", newCUs);
			MatcherAssert.assertThat(newCuXML, hasXPath("count(HUs/HU-VirtualPI[@HUStatus='A'])", is("3")));
			MatcherAssert.assertThat(newCuXML, hasXPath("count(HUs/HU-VirtualPI/Storage[@M_Product_Value='Tomato' and @Qty='5.000' and @C_UOM_Name='Kg'])", is("2")));
			MatcherAssert.assertThat(newCuXML, hasXPath("count(HUs/HU-VirtualPI/Storage[@M_Product_Value='Tomato' and @Qty='1.000' and @C_UOM_Name='Kg'])", is("1")));
		}
		...
		public void husToNewCUs_LU_CU_QtyLessThanPer1VirtualTU()
		{
			...
			final Node existingTUXML = HUXmlConverter.toXml(topLevelParent);
			MatcherAssert.assertThat(existingTUXML, hasXPath("count(HU-LU_Palet[@HUStatus='A'])", is("1")));
			MatcherAssert.assertThat(existingTUXML, hasXPath("HU-LU_Palet/Storage[@M_Product_Value='Tomato' and @C_UOM_Name='Kg']/@Qty", is("4.000")));
		...
		public void husToNewCUs_LU_CU_QtyMoreThanPer1VirtualTU()
		{
			...
			final Node existingTUXML = HUXmlConverter.toXml(topLevelParent);
			MatcherAssert.assertThat(existingTUXML, hasXPath("count(HU-LU_Palet[@HUStatus='D'])", is("1")));
			MatcherAssert.assertThat(existingTUXML, hasXPath("HU-LU_Palet/Storage[@M_Product_Value='Tomato' and @C_UOM_Name='Kg']/@Qty", is("0.000")));

			final Node newCuXML = HUXmlConverter.toXml(CollectionUtils.singleElement(newCUs));
			MatcherAssert.assertThat(newCuXML, hasXPath("string(HU-VirtualPI/@HUStatus)", is("A")));
			MatcherAssert.assertThat(newCuXML, hasXPath("HU-VirtualPI/Storage[@M_Product_Value='Tomato' and @C_UOM_Name='Kg']/@Qty", is("5.000")));
		}
	}
	...
	class husToNewTUs
	{
		...
		public void husToNewTUs_mix_homogenouse_aggregate_and_non_aggregate()
		{
			...
			final HUsToNewTUsRequest request = HUsToNewTUsRequest.builder()
					...
					.qtyTU(QtyTU.ofInt(4)).build();

			final List<I_M_HU> extractedTUs = huTransformService.husToNewTUs(request);
			assertThat(extractedTUs)
					.hasSize(4)
					.allSatisfy(tu -> assertThat(handlingUnitsBL.isAggregateHU(tu)).isFalse())
					.as("realTU2 was just \"moved\" into the result set")
		...
		public void huToNewTUs_source_is_aggregated()
		{
			...
			final HUsToNewTUsRequest request = HUsToNewTUsRequest.builder().sourceHU(aggregateHU).qtyTU(QtyTU.ofInt(2)).build();
			final List<I_M_HU> extractedTUs = huTransformService.husToNewTUs(request);
		...
		public void huToNewTUs_source_is_not_aggregated(final boolean isOwnPackingMaterials)
		{
			...
			final HUsToNewTUsRequest request = HUsToNewTUsRequest.builder().sourceHU(luHU).qtyTU(QtyTU.ofInt(2)).build();
			final List<I_M_HU> extractedTUs = huTransformService.husToNewTUs(request);
		...
		public void doBeforeDestroyed()
		{
			...
			final Consumer<I_M_HU> consumer = hu -> huId2listenerFiredCounters.compute(hu.getM_HU_ID(), (k, v) -> (v == null) ? 1 : v + 1);
			...
			HUTransformService.newInstance(mutableHUContext)
					.husToNewTUs(HUsToNewTUsRequest.forSourceHuAndQty(aggregateHU1, QtyTU.ofInt(2)));

			refresh(aggregateHU1);
			assertThat(aggregateHU1.getHUStatus()).isEqualTo(X_M_HU.HUSTATUS_Destroyed);
			assertThat(huId2listenerFiredCounters).containsEntry(aggregateHU1.getM_HU_ID(), 1);
		}
	}
}
