...
/**
 * AC19: {@code M_ShipperTransportation_AddShipments} is a transport-order-only action - it must not be offered on a
 * delivery instruction, and a plain transport order must be completely unaffected (a no-op) by the added guard.
 */
class M_ShipperTransportation_AddShipmentsTest
{
	...
	private IProcessPreconditionsContext contextSelecting(final I_M_ShipperTransportation record)
	{
		...
		when(context.getSelectedModels(I_M_ShipperTransportation.class)).thenReturn(ImmutableList.of(record));
		return context;
	}
	...
	@Test
	void processedTransportOrder_stillRejectedForProcessedReason_notDocSubType()
	{
		// existing behaviour preserved: a processed transport order is still hidden, for the pre-existing reason
		final I_M_ShipperTransportation processedTransportOrder = shipperTransportation(null, true);

		final ProcessPreconditionsResolution resolution = process.checkPreconditionsApplicable(contextSelecting(processedTransportOrder));
