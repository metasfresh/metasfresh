...
public class CustomMFToExternalSystemMessageSender
{
	...
	public void send(@NonNull final JsonExternalSystemMessage externalSystemMessage)
	{
		final byte[] messageAsBytes;
		try
		{
			messageAsBytes = JsonObjectMapperHolder.sharedJsonObjectMapper().writeValueAsBytes(externalSystemMessage);
		}
		catch (final JsonProcessingException e)
		{
			throw new AdempiereException("Exception serializing externalSystemRequest", e)
					.appendParametersToMessage()
					.setParameter("externalSystemRequest", externalSystemMessage);
		}
