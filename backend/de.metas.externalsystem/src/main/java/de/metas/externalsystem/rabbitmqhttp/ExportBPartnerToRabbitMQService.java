...
public class ExportBPartnerToRabbitMQService extends ExportBPartnerToExternalSystem
{
	...
	public ExportBPartnerToRabbitMQService(
			...
			@NonNull final ExternalSystemMessageSender externalSystemMessageSender, final UserGroupRepository userGroupRepository)
	{
		super(externalSystemConfigRepo, externalSystemMessageSender, dataExportAuditLogRepository, dataExportAuditRepository);
	...
	{
		final ImmutableSet<ExternalSystemRabbitMQConfig> rabbitMQConfigs = externalSystemConfigRepo.getAllByType(getExternalSystemType())
				.stream()
		...
		return Optional.of(rabbitMQConfigs.stream()
	...
	@Override
	protected boolean isSendBPartnerAllowed(final @NonNull IExternalSystemChildConfig childConfig)
	{
		final ExternalSystemRabbitMQConfig rabbitMQConfig = ExternalSystemRabbitMQConfig.cast(childConfig);

		return rabbitMQConfig.isSendBPartnerAllowed();
	}
