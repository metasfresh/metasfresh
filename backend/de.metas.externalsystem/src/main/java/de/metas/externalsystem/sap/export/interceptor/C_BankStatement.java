...
public class C_BankStatement
{
	...
	public C_BankStatement(final @NonNull ExportAcctToSAPService exportAcctToSAPService)
	{
		this.exportAcctToSAPService = exportAcctToSAPService;
	}

	@DocValidate(timings = ModelValidator.TIMING_AFTER_POST)
	public void exportAfterPost(@NonNull final I_C_BankStatement bankStatement)
	{
		if (DocTypeId.ofRepoIdOrNull(bankStatement.getC_DocType_ID()) == null)
		{
			return;
		}

		trxManager.runAfterCommit(() -> exportAcctToSAPService.enqueueDocument(TableRecordReference.of(bankStatement)));
	}
}
