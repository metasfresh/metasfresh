...
public class C_Location
{
	...
	private final ITrxManager manager = Services.get(ITrxManager.class);

	private final RabbitMQExternalSystemService rabbitMQExternalSystemService;

	public C_Location(@NonNull final RabbitMQExternalSystemService rabbitMQExternalSystemService)
	{
		this.rabbitMQExternalSystemService = rabbitMQExternalSystemService;
	}

	@ModelChange(timings = ModelValidator.TYPE_AFTER_CHANGE)
	public void afterSave(@NonNull final I_C_Location location)
	{
		final List<I_C_BPartner_Location> bpartnerLocationList = bpartnerDAO.getBPartnerLocationsByLocationId(LocationId.ofRepoId(location.getC_Location_ID()));
