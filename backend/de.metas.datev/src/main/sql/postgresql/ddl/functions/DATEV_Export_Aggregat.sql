...
AS
$$
SELECT SUM(amt)               AS "Umsatz (ohne Soll-/Haben-Kennzeichen)",
       DebitOrCreditIndicator AS "Soll-/Haben-Kennzeichen",
       currency               AS "Währung",
       dr_account             AS Konto,
       CASE
...
GROUP BY DebitOrCreditIndicator, currency, dr_account, ex.dateacct, ex.documentno, rate, c.countrycode, tax.name, poreference
$$
;
