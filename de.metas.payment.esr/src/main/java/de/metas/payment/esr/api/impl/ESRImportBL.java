...
public class ESRImportBL implements IESRImportBL
{
	...
	public void evaluateLine(final I_ESR_Import esrImport, final I_ESR_ImportLine importLine)
	{
		...
		if (esrImport.getC_BP_BankAccount_ID() > 0) // might not be the case in unit tests.
		{
			...
			final I_C_BP_BankAccount bankAccount = InterfaceWrapperHelper.create(esrImport.getC_BP_BankAccount(), I_C_BP_BankAccount.class);
		...
		for (final I_ESR_ImportLine line : linesNoInvoices)
		{
			...
			Services.get(ITrxManager.class).run(trxName, trxRunConfig, new TrxRunnable()
			{
				...
				public void run(final String localTrxName) throws Exception
				{
					...
					final de.metas.banking.model.I_C_Payment paym = InterfaceWrapperHelper.create(payment, de.metas.banking.model.I_C_Payment.class);
