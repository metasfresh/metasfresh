...
SELECT
	...
	param_acct_value,
	param_acct_name,
	(SELECT enddate::date FROM C_Period WHERE C_Period_ID = $4 AND isActive = 'Y') AS Param_End_Date,
	(SELECT startdate::date FROM C_Period WHERE C_Period_ID = $3 AND isActive = 'Y') AS Param_Start_Date,
...
FROM
	(
		SELECT 
			...
			currencyrate(c.c_currency_id, (SELECT C_Currency_ID FROM C_Currency WHERE ISO_Code = 'EUR' AND isActive = 'Y') , (SELECT enddate::date FROM C_Period WHERE C_Period_ID = $4 AND isActive = 'Y'), (SELECT C_ConversionType_ID FROM C_ConversionType where value ='P' AND isActive = 'Y'), ci.AD_Client_ID, ci.ad_org_id) AS ConversionMultiplyRate,
		...
		FROM 
			...
			LEFT OUTER JOIN Fact_Acct fa ON fa.Account_ID = ev.C_ElementValue_ID
				AND DateAcct >= (SELECT startdate::date FROM C_Period WHERE C_Period_ID = $3 AND isActive = 'Y')
				AND DateAcct <= (SELECT enddate::date FROM C_Period WHERE C_Period_ID = $4 AND isActive = 'Y') 
	...
	LEFT OUTER JOIN C_Activity pa ON COALESCE( pa.C_Activity_ID = $5, false ) AND pa.isActive = 'Y'
WHERE
