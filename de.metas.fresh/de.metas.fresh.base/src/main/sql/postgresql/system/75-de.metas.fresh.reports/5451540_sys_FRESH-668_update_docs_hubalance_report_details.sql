...
FROM
	...
	INNER JOIN M_Material_Balance_Detail mbd ON mbc.M_Material_Balance_Config_ID = mbd.M_Material_Balance_Config_ID AND mbd.isActive = 'Y'
	INNER JOIN C_UOM uom ON mbd.C_UOM_ID = uom.C_UOM_ID AND uom.isActive = 'Y'
	LEFT OUTER JOIN C_UOM_Trl uomt ON uom.C_UOM_ID = uomt.C_UOM_ID AND uomt.AD_Language = $6 AND uomt.isActive = 'Y'
	INNER JOIN C_DocType dt ON mbd.C_DocType_ID = dt.C_DocType_ID AND dt.isActive = 'Y'
	LEFT OUTER JOIN C_DocType_Trl dtt ON dt.C_DocType_ID = dtt.C_DocType_ID AND dtt.AD_Language = $6 AND dtt.isActive = 'Y'
	INNER JOIN M_Product p ON mbd.M_Product_ID = p.M_Product_ID AND p.isActive = 'Y'
	...
	LEFT OUTER JOIN (
		...
		WHERE	MovementDate::date < $3 AND ( mbd.IsReset = 'N' OR ( mbd.IsReset = 'Y' AND mbd.ResetDateEffective > $5 )) AND mbd.isActive = 'Y'
