DROP FUNCTION IF EXISTS report.bpartner_pricing_conditions (IN C_BPartner_ID numeric, IN C_BPartner_Location_ID numeric, IN Date date, IN ad_language character varying);

CREATE OR REPLACE FUNCTION report.bpartner_pricing_conditions (IN C_BPartner_ID numeric, IN C_BPartner_Location_ID numeric, IN Date date, IN ad_language character varying)
...
(
	...
	LEFT JOIN M_Product_Trl pt ON p.M_Product_ID = pt.M_Product_ID AND pt.ad_language = $4
	...
	LEFT JOIN C_UOM_Trl uomt ON uom.C_UOM_ID = uomt.C_UOM_ID AND uomt.ad_language = $4
	...
	(
		...
		WHERE 
			plv.isActive = 'Y' AND plv.validFrom <= $3
	...
	WHERE 
		bp.C_BPartner_ID = $1
		AND bpl.C_BPartner_Location_ID = $2
)r
