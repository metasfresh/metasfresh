...
CREATE OR REPLACE FUNCTION de_metas_endcustomer_fresh_reports.Docs_Purchase_Order_Root(IN record_id numeric, IN ad_language Character Varying (6))
RETURNS TABLE 
	(
...
SELECT
	...
	CASE
		...
		EXISTS(
			...
			WHERE pc.M_Product_Category_ID IN (SELECT value::numeric FROM AD_SysConfig sc WHERE name = 'PackingMaterialProductCategoryID' AND isActive = 'Y' AND (sc.ad_org_id = ol.ad_org_id OR sc.ad_org_id = 0))
