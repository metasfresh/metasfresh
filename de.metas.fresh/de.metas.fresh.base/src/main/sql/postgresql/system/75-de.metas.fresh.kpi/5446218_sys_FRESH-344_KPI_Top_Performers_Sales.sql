DROP FUNCTION IF EXISTS de_metas_fresh_kpi.KPI_Top_Performers_Sales(IN startdate DATE, IN enddate DATE);
CREATE OR REPLACE FUNCTION de_metas_fresh_kpi.KPI_Top_Performers_Sales(IN startdate DATE, IN enddate DATE)
RETURNS TABLE
(
...
WHERE o.issotrx='Y' AND o.DocStatus NOT IN ('IP','VO') AND
...
WHERE io.issotrx='Y' AND io.DocStatus NOT IN ('IP','VO') AND
...
WHERE i.issotrx='Y' AND i.DocStatus NOT IN ('IP','VO') AND
