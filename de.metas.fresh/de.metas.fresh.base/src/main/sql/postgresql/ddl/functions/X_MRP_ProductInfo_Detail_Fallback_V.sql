...
CREATE OR REPLACE FUNCTION X_MRP_ProductInfo_Detail_Fallback_V(IN DateFrom date, IN DateTo date) RETURNS SETOF X_mrp_productinfo_detail_mv AS
$BODY$
SELECT data.* 
FROM
	(
	SELECT
		...
		,0::numeric AS "fresh_qtyonhand_ondate_stö2"
		,0::numeric AS fresh_qtyonhand_ondate_ind9
...
$BODY$
LANGUAGE sql STABLE;
COMMENT ON FUNCTION X_MRP_ProductInfo_Detail_Fallback_V(date, date) IS 'Supplemental function to X_MRP_ProductInfo_Detail_V that returns one "empty" row for 
