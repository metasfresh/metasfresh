	...
	FROM
		...
		LEFT JOIN (
			...
			WHERE	AD_Table_ID = (SELECT AD_Table_ID FROM AD_Table WHERE TableName = 'C_Invoice')
				AND rn.C_ReferenceNo_Type_ID = (SELECT C_ReferenceNo_Type_ID FROM C_ReferenceNo_Type WHERE name = 'InvoiceReference')
		...
		LEFT JOIN (
			...
			FROM	C_ReferenceNo_Doc rnd
				LEFT JOIN C_ReferenceNo rn	ON rnd.C_ReferenceNo_ID = rn.C_ReferenceNo_ID
			WHERE	AD_Table_ID = (SELECT AD_Table_ID FROM AD_Table WHERE TableName = 'C_Invoice')
				AND rn.C_ReferenceNo_Type_ID = (SELECT C_ReferenceNo_Type_ID FROM C_ReferenceNo_Type WHERE name = 'ESRReferenceNumber')
		...
		LEFT JOIN C_BP_Bankaccount bpb ON bp.C_BPartner_ID = bpb.C_BPartner_ID AND bpb.IsEsrAccount='Y'

		LEFT JOIN C_Dunning_Candidate cand ON i.C_Invoice_ID = cand.Record_ID AND cand.AD_Table_ID = Get_Table_ID('C_Invoice')
