...
FROM
	...
	LEFT OUTER JOIN C_AllocationLine sal ON i.C_Invoice_ID = sal.C_Invoice_ID
	LEFT OUTER JOIN C_AllocationHdr ah ON sal.C_AllocationHdr_ID = ah.C_AllocationHdr_ID
	LEFT OUTER JOIN C_AllocationLine tal ON sal.C_AllocationHdr_ID = tal.C_AllocationHdr_ID AND i.C_Invoice_ID != tal.C_Invoice_ID
	LEFT OUTER JOIN C_Invoice ai ON tal.C_Invoice_ID = ai.C_Invoice_ID
	LEFT OUTER JOIN C_DocType adt ON ai.C_DocType_ID = adt.C_DocType_ID
	LEFT OUTER JOIN C_Currency ac ON ah.C_Currency_ID = ac.C_Currency_ID
	LEFT OUTER JOIN C_BP_BankAccount ba ON ba.C_BP_BankAccount_ID=psl.C_BP_BankAccount_ID
...
$BODY$
LANGUAGE sql STABLE;
ALTER FUNCTION report.fresh_payselection_report_details(IN numeric, IN numeric, IN character varying) OWNER TO adempiere;
