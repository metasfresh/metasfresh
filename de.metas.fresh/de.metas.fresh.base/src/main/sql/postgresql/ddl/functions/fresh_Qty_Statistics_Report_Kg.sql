...
ALTER TABLE report.fresh_qty_statistics_report_kg OWNER TO adempiere;
...
$BODY$
	SELECT 
		*, 1 AS UnionOrder
	FROM 	
		report.fresh_statistics_kg ($1, $2, $3, $4, $5, $6, $7)
UNION ALL
	SELECT 
		...
		param_Activity, param_product, param_Product_Category, Param_Attributes,
		2 AS UnionOrder
	FROM 	
		report.fresh_statistics_kg ($1, $2, $3, $4, $5, $6, $7)
	GROUP BY
...
$BODY$
LANGUAGE sql STABLE;
ALTER FUNCTION report.fresh_qty_statistics_report_kg 
	(
		IN C_Period_ID numeric, 
		IN issotrx character varying,
		IN C_Activity_ID numeric,
