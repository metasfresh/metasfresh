...
ALTER TABLE report.fresh_qty_statistics_report OWNER TO adempiere;
...
$BODY$
	SELECT 
		*, 1 AS UnionOrder
	FROM 	
		report.fresh_statistics ($1, $2, $3, $4, $5, $6, $7)
UNION ALL
	SELECT 
		...
		StartDate, EndDate, param_IsSOTrx, param_bp, param_Activity, param_product, param_Product_Category, Param_Attributes,
		2 AS UnionOrder
	FROM 	
		report.fresh_statistics ($1, $2, $3, $4, $5, $6, $7)
	GROUP BY
...
$BODY$
LANGUAGE sql STABLE;
ALTER FUNCTION report.fresh_qty_statistics_report 
	(
		IN C_Period_ID numeric, 
		IN issotrx character varying,
		IN C_BPartner_ID numeric, 
