...
ALTER FUNCTION X_MRP_ProductInfo_Detail_MV_Refresh()
  OWNER TO adempiere;
COMMENT ON FUNCTION X_MRP_ProductInfo_Detail_MV_Refresh(date, date) IS 'tasks 08681 and 08682: refreshes the table X_MRP_ProductInfo_Detail_MV.
...
Note that all these steps are only done with records that have a DateGeneral range from 4 day back to 4 days into the future.
';
