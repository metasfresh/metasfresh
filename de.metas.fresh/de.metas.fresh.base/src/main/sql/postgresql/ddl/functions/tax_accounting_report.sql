DROP FUNCTION IF EXISTS report.tax_accounting_report(IN dateFrom date, IN dateTo date, IN vatcode varchar, IN account_id numeric, IN org_id numeric, IN showdetails character varying, IN ad_language character varying(6));
CREATE OR REPLACE FUNCTION report.tax_accounting_report(IN dateFrom date, IN dateTo date, IN vatcode varchar,
         ...
         SELECT ev.value                              AS kontono,
                ...
                (CASE
                     ...
                     ELSE (SELECT value || ' - ' || name
                           from C_ElementValue
                           WHERE C_ElementValue_ID = $4
                             and isActive = 'Y') END) AS param_konto,
                (CASE
                     WHEN $3 IS NULL
                         THEN NULL
                     ELSE $3 END)                     AS param_vatcode,
                (CASE
                     WHEN $5 IS NULL
                         THEN NULL
                     ELSE (SELECT name
