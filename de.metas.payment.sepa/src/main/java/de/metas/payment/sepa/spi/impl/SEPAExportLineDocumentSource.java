...
public class SEPAExportLineDocumentSource implements ISEPADocumentSource
{
	...
	public Iterator<ISEPADocument> iterate(Properties ctx, ISEPADocumentSourceQuery query, String trxName)
	{
		...
		{
			@Override
			public ISEPADocument convert(I_SEPA_Export_Line value)
			{
	...
	private ISEPADocument createSEPADocument(I_SEPA_Export_Line value)
	{
		final ISEPADocument sepaDocument = new SEPADocument(value.getAD_Client_ID(),
				value.getAD_Org_ID(), value.getAmt(),
				value.getDescription(), value.getIBAN(),
				value.getSwiftCode(), I_SEPA_Export_Line.Table_Name,
				value.getSEPA_MandateRefNo(), value.getSEPA_Export_Line_ID(), 
