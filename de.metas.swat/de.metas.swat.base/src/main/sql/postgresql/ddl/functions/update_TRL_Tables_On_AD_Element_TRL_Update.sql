...
-- AD_Field_TRL via AD_Elememtn <= AD_Column <= AD_Field
...
-- AD_Field_TRL; lookup via via AD_Element_ID = AD_Field.AD_Name_ID
	UPDATE  AD_Field_TRL ftrl
	SET name = x.name, isTranslated = x.isTranslated, description = x.description, help = x.help
	FROM
	(
...
-- AD_Process_Para_TRL
	...
	(
		...
		where e.AD_Element_ID = update_TRL_Tables_On_AD_Element_TRL_Update.AD_Element_ID 
			...
			and ppt.ad_language = etrl.ad_language 
			and etrl.ad_language =  update_TRL_Tables_On_AD_Element_TRL_Update.AD_Language
	) x
...
-- AD_PrintFormatItem_TRL
	...
	(
		...
		where e.AD_Element_ID = update_TRL_Tables_On_AD_Element_TRL_Update.AD_Element_ID 
			...
			and pfit.ad_language = etrl.ad_language 
			and etrl.ad_language =  update_TRL_Tables_On_AD_Element_TRL_Update.AD_Language
	) x
...
COMMENT ON FUNCTION public.update_trl_tables_on_ad_element_trl_update(numeric, character varying)
    IS 'When the AD_Element_trl has one of its values with IsTranslated=''Y'' changed, 
then that change shall also propagate to the linked table entries:
