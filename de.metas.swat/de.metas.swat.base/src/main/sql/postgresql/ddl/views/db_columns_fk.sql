...
SELECT
		...
		, v.DDL_NoForeignKey
		, (CASE WHEN
				v.DDL_NoForeignKey='Y'
				OR EXISTS (SELECT 1 FROM db_constraints c
					WHERE c.schemaName='adempiere' AND c.tablename=LOWER(v.tablename) AND c.columnname=LOWER(v.columnname)
					AND c.constraint_type IN ('p','f'))
			THEN 'N' ELSE 'Y' END) AS IsMissing
		--
...
WHERE 1=1
	...
	AND NOT EXISTS (
		...
		WHERE
		c.schemaName='adempiere' AND c.tablename=LOWER(v.tablename) AND c.columnname=LOWER(v.columnname)
		AND c.constraint_type IN ('p','f')
	)
	*/
