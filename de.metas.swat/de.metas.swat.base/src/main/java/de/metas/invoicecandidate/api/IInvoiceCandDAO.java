...
public interface IInvoiceCandDAO extends ISingletonService {
	 ...
	 *
	 * @param queryBuilder
	 * @return invoice candidate iterator ordered by
	 *         {@link I_C_Invoice_Candidate#COLUMNNAME_HeaderAggregationKey}
	 */
	...
	/**
	 * Returns those invoice candidates that have been tagged to be
	 * recomputed/updated by the given <code>recomputeTag</code>.
	 *
	 ...
	 * @return
	 */
	Iterator<I_C_Invoice_Candidate> fetchInvalidInvoiceCandidates(Properties ctx, InvoiceCandRecomputeTag recomputeTag,
			String trxName);
	...
	/**
	 * @return new tagger used to tag invoice candidates which were scheduled to be
	 *         recomputed.
	 */
	...
	I_C_Invoice_Line_Alloc retrieveIlaForIcAndIl(I_C_Invoice_Candidate invoiceCand,
			org.compiere.model.I_C_InvoiceLine invoiceLine);
	 ...
	 * @return
	 */
	Iterator<I_C_Invoice_Candidate> retrieveForHeaderAggregationKey(Properties ctx, String headerAggregationKey,
			String trxName);
	 ...
	 * Note that for more than one candidate, this method is more efficient than
	 * repeated calls of {@link #invalidateCand(I_C_Invoice_Candidate)}
	 */
	...
	/**
	 * Invalidates all candidates that have the same
	 * <code>(AD_Table_ID, Record_ID)</code> reference.
	 *
	 * @param ic
	...
	/**
	 * Load the invoice candidates whose <code>AD_Table_ID</code> and
	 * <code>Record_ID</code> columns match the given model.
	 */
	List<I_C_Invoice_Candidate> retrieveReferencing(Object model);

	/**
	 * Delete all invoice candidates (active or not) that reference the given
	 * {@code model} via their {@code AD_Table_ID} and {@code Record_ID}.
	 *
	...
	/**
	 * Similar to {@link #updateDateInvoiced(Timestamp, PInstanceId, boolean)}, but
	 * updates the <code>DateAcct</code> column.
	 *
	 * @task 08437
	 */
	...
	/**
	 * Similar to {@link #updateDateInvoiced(Timestamp, int, String)}, but updates
	 * the <code>POReference</code> column.
	 */
	void updatePOReference(String poReference, PInstanceId selectionId);

	/**
	 * Updates the {@link I_C_Invoice_Candidate#COLUMN_C_PaymentTerm_ID} of those
	 * candidates that don't have a payment term ID. The ID those ICs are updated
	 * with is taken from the selected IC with the smallest
	 * {@code C_Invoice_Candidate_ID} that has a {@code C_PaymentTerm_ID}.
	 *
	...
	/**
	 * Gets the sum of all {@link I_C_Invoice_Candidate#COLUMNNAME_NetAmtToInvoice}
	 * values of the invoice candidates that have the given bPartner and are
	 * invoiceable before or at the given date. The amounts are converted to the
	 * currency which is set in the accounting schema of the bPartner's clients
	 * AD_ClientInfo.
	 *
	 */
	BigDecimal retrieveInvoicableAmount(I_C_BPartner billBPartner, Timestamp date);

	BigDecimal retrieveInvoicableAmount(Properties ctx, IInvoiceCandidateQuery query, CurrencyId targetCurrencyId,
			int adClientId, int adOrgId, String amountColumnName, String trxName);
	...
	/**
	 * Retrieve all invoices which have an invoice candidate for given
	 * tableName/recordId. We can select unpaid invoices only or all invoices.
	 *
	 * @param ctx
	 * @param tableName
	 * @param recordId
	 ...
	 */
	<T extends org.compiere.model.I_C_Invoice> Map<Integer, T> retrieveInvoices(Properties ctx, String tableName,
			int recordId, Class<T> clazz, boolean onlyUnpaid, String trxName);

	/**
	 * @deprecated please use
	 *             {@link #retrieveICIOLAssociationsExclRE(InvoiceCandidateId)}
	 */
	@Deprecated
	 ...
	 * <li>are active</li>
	 * <li>belong to an {@code M_InOut} record that is active and completed or
	 * closed (i.e. <b>not</b> reversed)</li>
	 * </ul>
	 *
	...
	/**
	 * Retrieves those invoice candidates that belong to the given
	 * <code>inOutLine</code>.
	 *
	 * @param inOutLine
	 ...
	 * <p>
	 * <b>Important:</b> do not filter by the lines' <code>M_InOut.DocStatus</code>,
	 * i.e. also reversed lines are returned by this.
	 * <p>
	 * FIXME debug to see why c_invoicecandidate_inoutline have duplicates and take
	 * the inoutlines from there for now takes it via orderline.
	 *
	 * @param ic
	 * @param clazz
	 * @return
	 */
	<T extends org.compiere.model.I_M_InOutLine> List<T> retrieveInOutLinesForCandidate(I_C_Invoice_Candidate ic,
			Class<T> clazz);
	...
	/**
	 * Checks if the given <code>ic</code> is referenced by a
	 * <code>C_Invoice_Candidate_Recompute</code> record. The check is made within
	 * the ic's transaction.<br>
	 * Please use this method instead of calling the SQL-column based
	 * {@link I_C_Invoice_Candidate#isToRecompute()}.
	 *
	 * @param ic
	 * @return
	 */
	 ...
	 *
	 * If there were any errors encountered while saving, this method will save the
	 * errors fields directly in database.
	 *
	...
	/**
	 * Invalidate all ICs that have the given <code>aggregation</code> as either
	 * their <code>HeaderAggregationKeyBuilder_ID</code> or
	 * <code>LineAggregationKeyBuilder_ID</code>.
	 *
	 * @param aggregation
	 */
	...
	/**
	 * Delete given invoice candidate AND it will advice the framework to avoid
	 * scheduling a re-create.
	 *
	 * @param ic
	 */
	...
	/**
	 * @param ic invoice candidate
	 * @return true if re-create scheduling shall be avoided for given invoice
	 *         candidate
	 */
	...
	/**
	 * Return the default filter to be applied for retrieving invoice candidates, in
	 * String format.<br>
	 * This string is to be used in the hard-coded sql queries, in where
	 * clauses.<br>
	 * Note that this string does not start with "AND", but directly with the
	 * condition.<br>
	 *
	 * Default filters until now:
	 * <li>Only retrieve invoice candidates the user and role have access to.
	 ...
	 * @return
	 */
	IQueryBuilder<I_C_Invoice_Candidate> retrieveInvoiceCandidatesForInventoryLineQuery(
			I_M_InventoryLine inventoryLine);
