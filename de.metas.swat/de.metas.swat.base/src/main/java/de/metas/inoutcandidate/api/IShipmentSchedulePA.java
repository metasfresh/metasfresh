...
public interface IShipmentSchedulePA extends ISingletonService
{
	 ...
	 * <code>M_SipmentSchedule_Recompute</code> records that point to those scheds, are marked with the given <code>adPinstanceId</code>. Task 08727: Note that this "marking/tagging" is done
	 * out-of-trx so that the info is directly available.
	 * 
	 * <b>IMPORTANT:</b> is a shimpentschedule is locked (by a <code>T_Lock</code>) record, then that schedule is neither retrieved, nor is its <code>M_SipmentSchedule_Recompute</code> record marked
	 * with the given <code>adPinstanceId</code>.
	 * 
