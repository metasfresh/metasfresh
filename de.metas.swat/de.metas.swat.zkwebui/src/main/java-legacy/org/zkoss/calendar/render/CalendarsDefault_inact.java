 ...
 *
 * metas-dev: long time ago, we altered this class and trusted *this* version to be used instead of org.zkoss.calendar:calendar:jar:1.0.0-RC
 * If we want to use it again in future, make sure by creating an altered version of the jar.
 *
 * @author jumperchen
 *
 */
...
// metas
public class CalendarsDefault_inact implements ComponentRenderer
{
			...
			for (int day = 0; day < 7; ++day)
			{
				...
				for (final CalendarEvent ce : evts)
				{
					...
					if (bd.before(curDate) || btmp.get(Calendar.YEAR) != etmp.get(Calendar.YEAR)
							...
							|| ed.getTime() - bd.getTime() >= CalendarsDefault_inact.ONE_DAY * 1000 &&
							rests >= CalendarsDefault_inact.ONE_DAY)
					{
						for (double y = 1, x = rests / CalendarsDefault_inact.ONE_DAY; y < x; y++)
						{
							if (y + day >= 7)
							{
								...
								if (ed.after(endDate))
								{
									final String nextkey = sdfKey.format(new Date(endDate.getTime() + (long)CalendarsDefault_inact.ONE_DAY));
			...
			for (int r = 0; r < maxSize; r++)
			{
				...
				for (int total = 7, i = 0; i < 7; i++)
				{
					...
					if (evts.size() > 0)
					{
						...
						final Date bd = ce.getBeginDate();
						final Date ed = ce.getEndDate();
						final double rests = (ed.getTime() - curDate.getTime()) / 1000 / CalendarsDefault_inact.ONE_DAY;
						int colspan = 1;
	...
	private void getDayView(final Calendars self, final Writer out) throws IOException
	{
		...
		for (int day = 0; day < days; ++day)
		{
			...
			for (final CalendarEvent ce : evts)
			{
				...
				{
					daylongCnt[day]++;
					for (double y = 1, x = rests / CalendarsDefault_inact.ONE_DAY; y < x && y + day < days; y++)
					{
						final int size = day + (int)y;
						daylongCnt[size]++;
					}
		...
		for (int r = 0; r < maxSize; r++)
		{
			...
			for (final Iterator it = daylongList.iterator(); it.hasNext(); c++)
			{
				...
				if (list.size() > 0)
				{
					...
					int colspan = 1;
					for (double y = 1, x = rests / CalendarsDefault_inact.ONE_DAY; y < x && y + c < days; y++)
					{
						colspan++;
					}
	...
	{
		return CalendarsDefault_inact._defCompare;
	}
