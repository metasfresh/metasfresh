...
//     console.log('hover: ', props, monitor.getItem())
...
const cardTarget = {
  drop(props, monitor) {
    console.log('PROPS drop: ', props);

    props.onDrop && props.onDrop(monitor.getItem(), props.laneId);
  },
  hover(props, monitor) {
    console.log('PROPS hover: ', props, monitor.getItem());
...
const TargetIndicator = (props) => {
  ...
  if (laneId !== parentLaneId || index !== parentIndex) {
    return false;
  }
...
const Card = (props) => {
  ...
  const { laneId, targetIndicator, index } = props;
