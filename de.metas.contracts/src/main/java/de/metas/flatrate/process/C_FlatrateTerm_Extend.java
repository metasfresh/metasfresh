...
public class C_FlatrateTerm_Extend extends SvrProcess
{
	...
	protected void prepare()
	{
		for (final ProcessInfoParameter para : getParameter())
		{
			...
			else if (name.equals(I_C_Flatrate_Transition.COLUMNNAME_IsAutoCompleteNewTerm))
			{
				p_forceComplete = para.getParameterAsBoolean();
			}
		}
	}
	...
	protected String doIt() throws Exception
	{
		...
		{
			final I_C_Flatrate_Term termToExtend = InterfaceWrapperHelper.create(getCtx(), getRecord_ID(), I_C_Flatrate_Term.class, get_TrxName());
		...
		else
		{
			...
			final String wc =
					"COALESCE (" + I_C_Flatrate_Term.COLUMNNAME_AD_PInstance_EndOfTerm_ID + ",0)=0 AND "
			...
			final Iterator<I_C_Flatrate_Term> termsToExtend =
					new Query(getCtx(), I_C_Flatrate_Term.Table_Name, wc, get_TrxName())
