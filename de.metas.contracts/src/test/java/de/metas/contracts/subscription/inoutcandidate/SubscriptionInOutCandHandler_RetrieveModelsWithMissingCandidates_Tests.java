...
public class SubscriptionInOutCandHandler_RetrieveModelsWithMissingCandidates_Tests
{
	...
	public void retrieveModelsWithMissingCandidates_Different_Advance_Days_Values()
	{
		...
		// guard
		{
			sysConfigBL.setValue(SubscriptionInOutCandHandler.SYSCONFIG_CREATE_SHIPMENT_SCHEDULES_IN_ADVANCE_DAYS, 3, 0);
			assertOnlyFirstRecordIsReturned();
		}

		sysConfigBL.setValue(SubscriptionInOutCandHandler.SYSCONFIG_CREATE_SHIPMENT_SCHEDULES_IN_ADVANCE_DAYS, 4, 0);
		assertBothRecordsAreReturned();

		sysConfigBL.setValue(SubscriptionInOutCandHandler.SYSCONFIG_CREATE_SHIPMENT_SCHEDULES_IN_ADVANCE_DAYS, 5, 0);
		assertBothRecordsAreReturned();
	}

	private void assertOnlyFirstRecordIsReturned()
	{
		final List<Object> result = new SubscriptionInOutCandHandler().retrieveModelsWithMissingCandidates(Env.getCtx(), ITrx.TRXNAME_ThreadInherited);
	...
	{
		final List<Object> secondResult = new SubscriptionInOutCandHandler().retrieveModelsWithMissingCandidates(Env.getCtx(), ITrx.TRXNAME_ThreadInherited);
