...
describe('RawWidget component', () => {
  ...
  describe('generic tests using CostPrice widget:', () => {
    it('renders widget without errors', () => {
      const props = createDummyProps(
        {
      ...
      expect(wrapper.find('NumberInput').dive().find('input').length).toBe(1);
      expect(html).toContain(fixtures.costPrice.data1.value)
      expect(html).toContain(fixtures.costPrice.layout1.description)
    });
    ...
    it('renders nothing when widget is not displayed', () => {
      const props = createDummyProps(
        {
          ...fixtures.costPrice.layout1,
          widgetData: [{ ...fixtures.costPrice.data1, displayed: false }],
        },
      );
    ...
    it('calls `handlePatch` on `{enter}/{tab}` keydown', () => {
      ...
      numberInput.find('input').simulate(
        ...
        {
          key: 'Enter',
          target: { value: fixtures.costPrice.data1.value },
          preventDefault: jest.fn(),
        },
      );
    ...
    it('comma is replaced by dot when patching field', () => {
      const handlePatchSpy = jest.fn()
      const props = createDummyProps(
        {
          ...fixtures.costPrice.layout1,
