...
public class DistributionRun extends JavaProcess
{
			...
			if (order == null)
			{
				...
				if (!p_IsTest)
				{
					...
					if (counter && bp.getAD_OrgBP_ID() > 0)
					{
						...
						final I_AD_OrgInfo oi = Services.get(IOrgDAO.class).retrieveOrgInfo(getCtx(), bp.getAD_OrgBP_ID(), ITrx.TRXNAME_None);
	...
	private boolean distributionOrders()
	{
		...
		for (int i = 0; i < m_details.length; i++)
		{
			...
			final I_AD_OrgInfo oi_target = Services.get(IOrgDAO.class).retrieveOrgInfo(getCtx(), bp.getAD_OrgBP_ID(),ITrx.TRXNAME_None);
