...
public class ReplenishReport extends JavaProcess
{
	 ...
	 * @param wh warehouse
	 */
	private void fillTable(I_M_Warehouse wh) throws Exception
	{
	...
	private void createMovements()
	{
		...
		for (int i = 0; i < replenishs.length; i++)
		{
			...
			for (int j = 0; j < storages.length; j++)
			{
				...
				line.setM_LocatorTo_ID(M_LocatorTo_ID.getRepoId());					// to
	...
	private void createDO() throws Exception
	{
		...
		for (X_T_Replenish replenish : replenishs)
		{
					...
					|| M_Warehouse_ID != replenish.getM_Warehouse_ID())
			{
				...
				List<I_M_Warehouse> whsInTransit = warehousesRepo.getByOrgId(OrgId.ofRepoId(whSource.getAD_Org_ID()));
			...
			if (replenish.getQtyToOrder().compareTo(replenish.getQtyToOrder()) != 0)
				line.setDescription("Total: " + replenish.getQtyToOrder());
			line.setM_Locator_ID(M_Locator_ID.getRepoId());		// from
