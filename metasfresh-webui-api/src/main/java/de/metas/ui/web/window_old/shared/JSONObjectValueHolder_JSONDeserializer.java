...
public class JSONObjectValueHolder_JSONDeserializer extends JsonDeserializer<JSONObjectValueHolder>
{

	@Override
	public JSONObjectValueHolder deserialize(final JsonParser p, final DeserializationContext ctxt) throws IOException, JsonProcessingException
	{
		...
		{
			final JsonNode valueNode = root.get(JSONObjectValueHolder_JSONSerializer.PROPERTY_Value);
			value = mapper.treeToValue(valueNode, valueType);
		}

		return JSONObjectValueHolder.of(value);
	}

	private final String extractValueTypeName(final JsonNode root)
	{
		final JsonNode typeNode = root.get(JSONObjectValueHolder_JSONSerializer.PROPERTY_ValueType);
		if (typeNode == null)
		{
			return null;
		}
