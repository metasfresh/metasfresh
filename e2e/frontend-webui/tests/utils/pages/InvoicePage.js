 ...
 * Handles invoice document operations in metasfresh.
 */
export class InvoicePage {
  /**
   * Navigate to Invoice window and wait for it to fully load.
   */
  static async goto() {
    return await test.step('InvoicePage - Navigate to Invoice window', async () => {
      const page = getPage();
      await page.goto(`${FRONTEND_BASE_URL}/window/${SALES_INVOICE_WINDOW_ID}`);
  ...
  /**
   * Verify invoice window is visible and loaded.
   *
   * Assumes navigation via SalesOrderPage.openRelatedInvoice().
   * Waits for the invoice list view to be fully loaded.
   */
  static async expectVisible() {
    return await test.step('InvoicePage - Verify window is visible', async () => {
      const page = getPage();

      // Invoice opens as a list view (grid), not a detail window
  ...
  /**
   * Get the document number of the current invoice from the grid.
   *
   * Since invoice opens as a filtered list (not a detail window),
  ...
  static async getDocumentNo() {
    return await test.step('InvoicePage - Get document number from grid', async () => {
      const page = getPage();
  ...
  /**
   * Open the invoice detail view by double-clicking on the first row in the grid.
   * Required before printing from list view.
   */
  static async openDetailView() {
    return await test.step('InvoicePage - Open detail view', async () => {
      const page = getPage();
  ...
  static async validatePdfContent(download, expectedData) {
    return await test.step('InvoicePage - Validate PDF content', async () => {
  ...
  static async closePrintModal() {
    return await test.step('InvoicePage - Close print modal', async () => {
      const page = getPage();
