...
// Columns with AD_Field.IsDisplayedGrid='Y' on tab 549365 (verified via
// AD_Field/AD_Column locally) — these render as grid column headers regardless
// of whether any row exists. The remaining columns from the table
// (C_BPartner_Location_ID, ReturnedName, ReturnedAddress, RawResponse) are
// detail/advanced-form-only fields (IsDisplayedGrid='N') and are not asserted
// here: they only render inside an opened record, which does not exist yet.
const GRID_COLUMNS = [
  'C_BPartner_ID',
  'VATaxID',
  'VATaxIDStatus',
...
2. Every grid-displayed column the feature depends on
   (C_BPartner_ID, VATaxID, VATaxIDStatus, RequestDate, ResponseDate,
   RequestIdentifier) is present as a rendered grid column header.
