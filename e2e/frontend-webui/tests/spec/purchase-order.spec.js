 ...
 * - Language-independent selectors using database column names
...
test.describe('Purchase Order', () => {
  test('Create and complete purchase order', async ({ page }) => {
    // Step 1: Create master data via Backend API
    // - User with login credentials
    ...
    console.log('Purchase Order created and completed:', purchaseOrderNo);
  ...
  test('Create purchase order and verify it appears in the list', async ({ page }) => {
    // Simpler test: Just create a PO and verify it's visible
    ...
    await PurchaseOrderPage.selectBusinessPartner(masterdata.bpartners.Vendor1.bpartnerCode);
    await PurchaseOrderPage.setOrderDate(30); // Select day 30 from date picker
    // Note: Batch entry button is already visible - no need to switch tabs
    await PurchaseOrderPage.addOrderLine({
      product: masterdata.products.Product1.productName,
      quantity: 5,
    });
    ...
    console.log('Purchase Order created successfully:', docNo);
  });
});
