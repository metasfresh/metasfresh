...
describe('BPartner relations', function () {
  ...
  it('Create discount schema,customers and a sales order', function () {
    cy.fixture('discount/discountschema.json').then((discountschemaJson) => {
      Object.assign(new DiscountSchema(), discountschemaJson).setName(discountSchemaName).apply();
    });

    cy.fixture('sales/simple_customer.json').then((customerJson) => {
      new BPartner({ ...customerJson, name: customer1Name }).setCustomerDiscountSchema(discountSchemaName).setCustomerPricingSystem(priceListName).clearContacts().apply();
    });

    cy.fixture('sales/simple_customer.json').then((customerJson) => {
      new BPartner({ ...customerJson, name: customer2Name }).setCustomerDiscountSchema(discountSchemaName).setCustomerPricingSystem(priceListName).clearContacts().apply();
    });

    cy.fixture('sales/sales_order.json').then((orderJson) => {
      new SalesOrder({ ...orderJson, reference: salesOrder1Name }).setBPartner(customer1Name).setInvoicePartner(customer2Name).apply();
    });
  });
