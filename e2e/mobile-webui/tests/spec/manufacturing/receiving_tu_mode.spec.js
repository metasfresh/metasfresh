...
const createMasterdata = async () => {
    return await Backend.createMasterdata({
        ...
        request: {
            ...
            // Sales order with packing instructions so the PP_Order gets linked to a C_OrderLine
            // that carries M_HU_PI_Item_Product_ID — required for TU mode to resolve the TU PI Item Product
            salesOrders: {
                "SO1": {
                    bpartner: 'BP1',
                    warehouse: 'wh',
                    datePromised: '2025-03-01T00:00:00.000+02:00',
                    lines: [{ product: 'BOM', qty: 80, piItemProduct: 'TU' }],
                },
            },
            manufacturingOrders: {
                "PP1": {
...
test('Receive in TU mode to a new TU target', async ({ page }) => {
    // === ALLURE METADATA ===
    allure.epic('E0160: Manufacturing Execution');
    allure.tag('F8030: MobileUI Manufacturing');
    allure.tag('F8030');
    allure.story('Receive main products in TU mode');
...
// noinspection JSUnusedLocalSymbols
test('Receive partial TUs in TU mode to a new LU target', async ({ page }) => {
    // === ALLURE METADATA ===
    allure.epic('E0160: Manufacturing Execution');
    allure.tag('F8030: MobileUI Manufacturing');
    allure.tag('F8030');
    ...
    // Receive 5 TUs to a new LU target
    await ManufacturingJobScreen.clickReceiveButton({ index: 1 });
