Feature: metasfresh communicates with the procurement-WebUI via RabbitMQ
  ...
  Scenario: metasfresh provides bpartner data to the procurement-WebUI
    ...
    And metasfresh contains C_BPartners:
      | RecordIdentifier | Name                   | OPT.IsVendor | OPT.IsCustomer |
      | bpartner_1       | ProcurementPartner1    | Y            | Y              |
      | bpartner_2       | ProcurementPartner2    | Y            | N              |
      | bpartner_3       | NotAProcurementPartner | N            | Y              |
