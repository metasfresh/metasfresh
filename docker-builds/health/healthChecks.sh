...
timeout 300s sh -c 'until docker ps | grep db | grep -q healthy; do echo "Waiting for container to be healthy..."; sleep 30; done'
status=$?
...
then
  ...
  echo ""
  echo "---------------------------------------------------------"
  echo " start unhealthy db log"
  echo "---------------------------------------------------------"
  echo docker-compose logs db --tail 250
  echo "---------------------------------------------------------"
  echo " end unhealthy db log"
  echo "---------------------------------------------------------"
else
  echo ""
  echo "OK"
...
timeout 300s sh -c 'until docker ps | grep app | grep -q healthy; do echo "Waiting for container to be healthy..."; sleep 30; done'
status=$?
if [ $status -eq 124 ] #timed out
then
  hasErrors=true
  ...
  echo ""
  echo "---------------------------------------------------------"
  echo " start unhealthy app log"
  echo "---------------------------------------------------------"
  echo docker-compose logs app --tail 250
  echo "---------------------------------------------------------"
  echo " end unhealthy app log"
  echo "---------------------------------------------------------"
  echo ""
else
  echo ""
...
echo ""
echo "========================================================="
echo " webapi health check ..."
echo "---------------------------------------------------------"

timeout 300s sh -c 'until docker ps | grep webapi | grep -q healthy; do echo "Waiting for container to be healthy..."; sleep 30; done'
status=$?
if [ $status -eq 124 ] #timed out
then
  hasErrors=true
  ...
  echo ""
  echo "---------------------------------------------------------"
  echo " start unhealthy webapi log"
  echo "---------------------------------------------------------"
  echo docker-compose logs app --tail 250
  echo ""
  echo "---------------------------------------------------------"
  echo " end unhealthy webapi log"
  echo "---------------------------------------------------------"
  echo ""
else
  echo ""
