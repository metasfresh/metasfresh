...
public class ChangeASIAction extends ProcessPopupAction
{
	...
	private int selectASIID(BOMLineWrapper line)
	{
		...
		final MProduct product = new MProduct(Env.getCtx(), line.getM_Product_ID(), null);
		final ProductId productId = ProductId.ofRepoIdOrNull(line.getM_Product_ID());
		final WarehouseId warehouseId = WarehouseId.ofRepoIdOrNull(getParameterValueAsInt("M_Warehouse_ID"));
		final BPartnerId bpartnerId = null;
		final PAttributeInstance pai = new PAttributeInstance((JFrame)null, product.getName(), warehouseId, 0, productId, bpartnerId);
